...
export function getToolUseGuidelinesSection(codeIndexManager?: CodeIndexManager): string {
  ...
  - Any other relevant feedback or information related to the tool use.`)
	guidelinesList.push(
		`${itemNumber++}. Wait for tool use results before proceeding. Use discovery tools intelligently in sequence to build understanding. For exploration and discovery tasks, you may chain multiple discovery tools (codebase_search, list_files, search_files, read_file) to efficiently gather context before asking the user anything.`,
	)
