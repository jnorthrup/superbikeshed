...
suspend fun demonstrateTrikeShedPatterns() {
    ...
    val cursor = DatabaseCursor(3 j { i ->
        val row: RowVec = 2 j { j ->
            when (j) {
                0 -> "capability-$i" j { "string-meta" }
                1 -> i.toString() j { "int-meta" }
                else -> "default" j { "default-meta" }
            }
        }
        row
    })
    
    println("      🗄️ Database cursor: ${cursor.size} rows of architectural data")
