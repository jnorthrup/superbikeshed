...
class Http3Connection(
    internal val quicConnection: QuicConnection,
    internal val role: Role = Role.CLIENT
) {
    enum class Role { CLIENT, SERVER }
    
    // Control streams
    internal var controlStream: QuicStream? = null
    internal var qpackEncoderStream: QuicStream? = null
    internal var qpackDecoderStream: QuicStream? = null
    
    // QPACK codec
    internal val qpackEncoder = QpackEncoder()
    internal val qpackDecoder = QpackDecoder()
    ...
    internal val remoteSettings = mutableMapOf<Http3SettingId, Http3SettingValue>()
    
    // Request/Response handling
    internal val activeRequests = mutableMapOf<Http3StreamId, Http3Request>()
     ...
     */
    internal suspend fun processIncomingStreams() {
        GlobalScope.launch {
     ...
     */
    internal suspend fun handleIncomingStream(stream: QuicStream) {
        try {
     ...
     */
    internal suspend fun handleControlStream(stream: QuicStream) {
     ...
     */
    internal suspend fun handleRequestStream(stream: QuicStream) {
     ...
     * Send settings
     */
    internal suspend fun sendSettings(stream: QuicStream) {
     ...
     * Handle GOAWAY frame
     */
    internal fun handleGoaway(payload: Http3FramePayload) {
     ...
     */
    internal fun handleMaxPushId(payload: Http3FramePayload) {
     ...
     * Send stream type
     */
    internal suspend fun sendStreamType(stream: QuicStream, type: Http3StreamType) {
     ...
     */
    internal fun isUnidirectionalStream(streamId: Http3StreamId): Boolean {
     ...
     */
    internal suspend fun readVarInt(stream: QuicStream): Long? {
     ...
     */
    internal suspend fun readAllBytes(stream: QuicStream): Indexed<Byte> {
     ...
     * Handle push stream
     */
    internal suspend fun handlePushStream(stream: QuicStream) {
        // Read push ID
     ...
     */
    internal suspend fun handleQpackEncoderStream(stream: QuicStream) {
        while (stream.hasData()) {
            val data = readAllBytes(stream)
     ...
     */
    internal suspend fun handleQpackDecoderStream(stream: QuicStream) {
        while (stream.hasData()) {
            val data = readAllBytes(stream)
 ...
 */
class QpackEncoder {
    internal var maxTableCapacity = DynamicTableCapacity(0)
    internal val dynamicTable = mutableListOf<Join<String, String>>()
    internal var currentCapacity = 0
    ...
    internal fun encodeStringLiteral(bytes: ByteArray): List<Byte> {
...
class QpackDecoder {
    ...
    internal fun decodeStringLiteral(
        data: Http3FramePayload,
        startOffset: Int
    ): Pair<String, Int> {
 ...
 */
internal data class Http3Request(
 ...
 */
internal fun encodeVarInt(value: Long, prefixBits: Int = 62): List<Byte> {
    return when {
