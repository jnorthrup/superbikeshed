...
class HttpServer(
    ...
    internal val handler: CcekHttpHandler
) {
    internal lateinit var serverChannel: ServerChannel
...
class HttpConnectionHandler(
    internal val config: HttpServerConfig,
    internal val reactor: Reactor,
    internal val channel: ClientChannel,
    ...
    internal suspend fun sendErrorResponse(code: Int, phrase: String) {
...
internal fun String.toIndexed(): Indexed<Char> = this.length j { this[it] }


// ===== CONNECTION MANAGEMENT (RFC 7230 Section 6) =====

class HttpConnectionManager(internal val config: HttpServerConfig) {
...
internal fun ByteArray.toIndexed(): Indexed<Byte> = size j { this[it] }
internal fun List<Char>.toIndexed(): Indexed<Char> = size j { this[it] }
