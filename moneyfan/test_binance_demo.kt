...
/**
 * Simple test for Binance archive data loading
 */
fun main() {
    println("🔥 Testing Binance Archive Data Loading")
    println("=" .repeat(50))
    
    // Test 1: Download and parse a sample Binance archive
    println("\n📊 Test 1: Downloading sample Binance archive...")
    ...
    if (sampleData.isNotEmpty()) {
        ...
        // Test 2: Save as CSV for inspection
        println("\n📊 Test 2: Saving as CSV...")
        saveToCsv(sampleData, "test_binance_data.csv")
        println("✅ Saved to test_binance_data.csv")
        
        // Test 3: Parse CSV and filter by date range
        println("\n📊 Test 3: Parsing CSV with date filtering...")
        val filteredData = parseCsvWithDateRange("test_binance_data.csv", 
        ...
        // Test 4: Run simple backtesting
        println("\n📊 Test 4: Running backtesting...")
        ...
        // Test 5: Calculate basic technical indicators
...
fun downloadSampleBinanceData(): List<Kline> {
    return try {
        // Download a sample DOGEUSDT 1d kline file from Binance Data Vision (smaller file)
        val url = "https://data.binance.vision/data/spot/monthly/klines/DOGEUSDT/1d/DOGEUSDT-1d-2022-01.zip"
        ...
        println("Download completed, file size: ${tempFile.length()} bytes")
        ...
        ZipInputStream(FileInputStream(tempFile)).use { zipStream ->
            ...
            while (entry != null) {
                if (!entry.isDirectory && entry.name.endsWith(".csv")) {
                    println("Processing CSV file: ${entry.name}")
        ...
        tempFile.delete()
        println("Parsed ${klines.size} klines from archive")
        klines
...
fun parseKlineCsvLine(line: String): Kline? {
...
fun saveToCsv(klines: List<Kline>, filename: String) {
...
fun parseCsvWithDateRange(filename: String, startTime: Long, endTime: Long): List<Kline> {
    val klines = mutableListOf<Kline>()
...
fun runSimpleBacktest(klines: List<Kline>, initialCapital: Double): BacktestResult {
...
fun calculateTechnicalIndicators(klines: List<Kline>): TechnicalIndicators {
    if (klines.size < 20) {
...
fun formatKline(kline: Kline): String {
