...
class SwingMDIInterface : JFrame("Moneyfan MDI Trading Interface") {
    internal val desktop = JDesktopPane()
    internal val mdiInterface = MDIInterface()
    internal val scope = CoroutineScope(Dispatchers.Default + SupervisorJob())
    internal val windows = mutableMapOf<String, JInternalFrame>()
    internal var binanceIngest: BinanceAttentionIngest? = null
    internal var isIngestRunning = false
    
    // Trace and data areas
    internal val traceArea = JTextArea(10, 50).apply {
        isEditable = false
    ...
    internal val binanceArea = JTextArea(10, 50).apply {
        isEditable = false
        background = Color.BLACK
    ...
    internal fun setupFunctionKeys() {
    ...
    internal fun handleFunctionKey(keyNum: Int) {
    ...
    internal fun createInitialWindows() {
        openAttentionWindow()
        openPortfolioWindow()
    }
    
    internal fun openAttentionWindow() {
    ...
    internal fun openPortfolioWindow() {
    ...
    internal fun openTechnicalWindow() {
    ...
    internal fun openBacktestWindow() {
    ...
    internal fun openChartWindow(symbol: String) {
    ...
    internal fun createInternalFrame(title: String, content: JComponent, x: Int, y: Int, width: Int, height: Int): JInternalFrame {
    ...
    internal fun focusTraceLog() {
        traceArea.requestFocus()
    }
    
    internal fun clearTraceLog() {
        traceArea.text = ""
    }
    ...
    internal fun initializeStrategyBridge() {
        scope.launch {
            try {
    ...
    internal fun getCurrentCryptoPrice(symbol: String): String {
        return when (symbol) {
            "BTC" -> "$43,127.98"
            "ETH" -> "$2,847.82"
            "ADA" -> "$0.387"
