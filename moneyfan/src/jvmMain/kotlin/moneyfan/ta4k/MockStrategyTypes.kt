...
class HistoricalDataLoader {
    internal val dataDir = File(System.getProperty("user.home"), "mpdata/klines")
...
class AttentionSystem {
    internal val attentionData = mutableMapOf<String, AttentionReading>()
    ...
    internal fun calculateAttention(klines: List<Kline>, symbol: String): AttentionReading {
    ...
    internal fun calculateVolatility(prices: List<Double>): Double {
...
class StrategyOrchestrator {
    internal val dataLoader = HistoricalDataLoader()
    internal val attentionSystem = AttentionSystem()
    internal val symbols = listOf("BTCUSDT", "ETHUSDT", "ADAUSDT", "SOLUSDT", "DOTUSDT", "LTCUSDT", "DOGEUSDT")
    internal val scope = CoroutineScope(Dispatchers.Default + SupervisorJob())
    
    internal fun calculateRSI(pricesSeries: Indexed<Price>, period: Int): Double {
        TODO("Calculate RSI using TrikeShed Indexed<Price> with α transformations")
    }
    
    internal fun calculateSMA(pricesSeries: Indexed<Price>, period: Int): Price {
