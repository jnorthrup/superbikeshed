...
class SpaceGraphMasterLoader : JPanel(BorderLayout()) {
    ...
    internal val loadingPhases = listOf(
    ...
    internal val loadingStates = mutableMapOf<LoadingPhase, LoadingProgress>()
    internal val scope = CoroutineScope(Dispatchers.Default + SupervisorJob())
    internal var isLoading = false
    
    // Disk Utility style components
    internal val progressBars = mutableMapOf<LoadingPhase, JProgressBar>()
    internal val statusLabels = mutableMapOf<LoadingPhase, JLabel>()
    internal val controlPanel = JPanel(FlowLayout())
    internal val statusLabel = JLabel("Ready to load historical data")
    ...
    internal fun setupSpaceGraphUI() {
    ...
    internal fun createProgressBarsPanel(): JPanel {
    ...
    internal suspend fun simulatePhaseLoading(phase: LoadingPhase, latency: LatencyScale) {
        val steps = 100
    ...
    internal fun updateProgressBar(phase: LoadingPhase, progress: LoadingProgress) {
    ...
    internal fun updateStatusIfComplete() {
    ...
    internal fun showSpaceGraphVisualization() {
    ...
    internal fun createSpaceGraphWindow(): JInternalFrame {
