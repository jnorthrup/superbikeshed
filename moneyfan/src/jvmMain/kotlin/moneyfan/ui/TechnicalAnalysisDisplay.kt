...
class TechnicalAnalysisDisplay(
    internal val orchestrator: StrategyOrchestrator
) : JPanel(BorderLayout()) {
    ...
    internal var currentMode = StrategyMode("Combined")
    internal val availableModes = listOf(
    ...
    // UI Components
    internal val modeSelector = JComboBox(availableModes.map { it.name }.toTypedArray())
    internal val statusLabel = JLabel("Initializing...")
    internal val signalTable = JTable()
    internal val detailsArea = JTextArea(8, 40)
    internal val analysisScope = CoroutineScope(Dispatchers.Default + SupervisorJob())
    
    // Analysis update timer
    internal var isUpdating = false
    internal var lastUpdate = Clock.System.now()
    ...
    internal fun startAnalysisUpdates() {
        analysisScope.launch {
            while (isActive) {
                try {
                    if (!isUpdating) {
    ...
    internal suspend fun updateAnalysisDisplay() {
        isUpdating = true
    ...
    internal suspend fun getCarlosOnlyAnalysis(): Indexed<CombinedAnalysis> {
    ...
    internal suspend fun getSkimmerOnlyAnalysis(): Indexed<CombinedAnalysis> {
        val topSymbols = orchestrator.analyzeTopSymbols(10)
        return topSymbols.map { analysis ->
            analysis.copy(
    ...
    internal suspend fun getAttentionOnlyAnalysis(): Indexed<CombinedAnalysis> {
        val topSymbols = orchestrator.analyzeTopSymbols(10)
        return topSymbols.map { analysis ->
    ...
    internal fun convertCarlosToSignal(carlosSignal: TradeSignal): CombinedSignal {
    ...
    internal fun convertSkimmerToSignal(skimmerAction: SkimmerAction): CombinedSignal {
    ...
    internal fun convertSkimmerActionToSignal(skimmerAction: SkimmerAction): TradeSignal {
        return when (skimmerAction) {
    ...
    internal fun updateSignalTable(analyses: Indexed<CombinedAnalysis>) {
    ...
    internal fun updateDetailsForSelectedRow(rowIndex: Int) {
        try {
            val model = signalTable.model as DefaultTableModel
    ...
    internal fun updateStatus(message: String) {
        SwingUtilities.invokeLater {
    ...
    internal class SignalCellRenderer : DefaultTableCellRenderer() {
