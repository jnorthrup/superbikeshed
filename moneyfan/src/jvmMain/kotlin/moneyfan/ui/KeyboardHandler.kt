 ...
 */
class KeyboardHandler(private val mdiInterface: MDIInterface) {
    private val keyPressedStates = mutableMapOf<Int, Boolean>()
    private val keyHoldJobs = mutableMapOf<Int, Job>()
    ...
    private suspend fun simulateKeyboardEvents() {
    ...
    private fun simulateKeyHover(key: FunctionKey, isHovering: Boolean) {
        mdiInterface.onFunctionKeyHover(key, isHovering)
    }
    
    private fun simulateKeyPress(key: FunctionKey) {
    ...
    private fun simulateKeyRelease(key: FunctionKey) {
        val keyCode = key.keyCode
