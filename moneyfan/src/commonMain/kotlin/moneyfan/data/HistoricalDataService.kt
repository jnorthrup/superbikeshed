...
class HistoricalDataService(
    internal val fileContentProvider: FileContentProvider
) {
    // In-memory cache for Kline series. Keyed by file path for simplicity.
    internal val klineCache = mutableMapOf<String, Indexed<Kline>>()
    internal val cacheMutex = Mutex()

    // Instantiate the archive reader with the provided file content provider.
    internal val archiveReader = BinanceDataArchiveReader(fileContentProvider)
