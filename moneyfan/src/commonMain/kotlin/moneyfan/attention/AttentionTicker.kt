...
class AttentionBasedTicker {
    internal val tickerScope = CoroutineScope(Dispatchers.Default + SupervisorJob())
    internal val engine = TradingEngine()
    
    // Base attention span - dynamically adjusted by gauges
    internal val baseAttentionSpan = AttentionSpan(5000) // 5 second base
    
    // Current attention state with variable time gauge
    internal var currentAttention: AttentionState = 
    ...
    internal val watchedPairs = mutableMapOf<Symbol, PairWeight>()
    internal val dynamicSpans = mutableMapOf<Symbol, AttentionSpan>()
    
    // Time series attention tracking with gauge history
    internal val attentionWindows = mutableMapOf<Symbol, MutableList<AttentionWindow>>()
    internal val gaugeHistory = mutableMapOf<Symbol, MutableList<GaugeReading>>()
    ...
    internal fun updateAttentionWindowsWithGauges(currentTime: Instant): AttentionSeries {
    ...
    internal fun calculateVolatility(prices: List<Price>): Decimal {
    ...
    internal fun recomputeAttentionWithGauges(windows: AttentionSeries) {
...
class AttentionGUI {
    ...
    internal fun displayAttentionUpdate(windows: AttentionSeries) {
