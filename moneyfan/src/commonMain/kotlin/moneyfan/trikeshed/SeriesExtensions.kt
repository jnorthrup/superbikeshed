...
// --- Low-hanging fruit: Simple test for ffill() ---
#if TEST
...
class SeriesExtensionsTest {
    @Test
    fun testFfill() {
        val input = Indexed.of(5) { i -> if (i == 0 || i == 3) null else i }
        val result = input.ffill()
        // Expected: [null, 1, 2, 2, 4]
        assertEquals(listOf(null, 1, 2, 2, 4), List(result.size) { result[it] })
    }
}
#endif
