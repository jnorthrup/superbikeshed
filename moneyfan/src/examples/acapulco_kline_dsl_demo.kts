...
class AcapulcoKlineDSL {
    ...
    suspend fun execute(): Cursor {
        ...
        return try {
            ...
            telemetry.trackStage("store", isamCursor.rows)
...
class KlineParser {
    ...
    suspend fun parse(csvLines: List<String>): List<KlineData> {
...
class KlineTransformer {
    ...
    suspend fun transform(klines: List<KlineData>): List<KlineData> {
        return withContext(Dispatchers.Default) {
    ...
    private fun applyFilters(klines: List<KlineData>): List<KlineData> {
    ...
    private fun validateData(klines: List<KlineData>): List<KlineData> {
        return klines.filter { kline ->
    ...
    private fun aggregateData(klines: List<KlineData>): List<KlineData> {
        // Group by time window and aggregate
        val grouped = klines.groupBy { kline ->
            kline.openTime / aggregationWindow
        }
        
        return grouped.map { (_, groupKlines) ->
    ...
    private fun addCalculatedFields(klines: List<KlineData>): List<KlineData> {
...
class IsamStorage {
    ...
    suspend fun store(klines: List<KlineData>): Cursor {
        val path = outputPath ?: throw IllegalStateException("Output path not set")
        
        return withContext(Dispatchers.IO) {
            // Create a simple cursor implementation for demo
            SimpleCursor(klines)
        }
    }
}
...
/**
 * Simple cursor implementation for demo
 */
class SimpleCursor(private val data: List<KlineData>) : Cursor {
    override val rows: Int get() = data.size
    override val cols: Int get() = 11
    
    override fun row(index: Int): RowVec {
        require(index >= 0 && index < data.size) { "Row index $index out of bounds [0, $data.size)" }
        val kline = data[index]
        
        return SimpleRowVec(listOf(
            kline.openTime,
            kline.open,
            kline.high,
            kline.low,
...
/**
 * Simple row vector implementation
 */
class SimpleRowVec(private val values: List<Any?>) : RowVec {
    override val size: Int get() = values.size
    
    override fun get(index: Int): Any? {
        require(index >= 0 && index < values.size) { "Index $index out of bounds [0, $values.size)" }
        return values[index]
    }
}
...
class KlineAnalysis(private val cursor: Cursor) {
    ...
    fun getSummary(): KlineSummary {
        ...
        for (i in 0 until rows) {
            ...
            prices.add(row.get(4) as BigDecimal) // close price
            volumes.add(row.get(5) as BigDecimal) // volume
        }
    ...
    private fun getDateRange(): Pair<Long, Long> {
        ...
        val lastRow = cursor.row(cursor.rows - 1)
        
        val startTime = firstRow.get(0) as Long
        val endTime = lastRow.get(6) as Long // close_time
...
fun main() = runBlocking {
    ...
    // Create sample data
    createSampleCsv("data/klines.csv", 100)
