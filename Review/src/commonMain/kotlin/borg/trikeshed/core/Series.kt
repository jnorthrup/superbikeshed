...
infix operator fun IntRange.div(denominator: Int): Series<IntRange> =
    (this j ((last - first + 1) / denominator)).let { (_: IntRange, subSize: Int) ->
        denominator j { x: Int ->
            (subSize * x).let { lower ->
...
//series map
fun <T, R> Series<T>.map(transform: (T) -> R): Series<R> = size j { i -> transform(this[i]) }
...
object EmptySeries : Series<Nothing> by 0 j { x: Int -> throw IndexOutOfBoundsException("empty Series Access Violation") }
...
fun <T> Series<T>.compareTo(other: Series<T>, comparator: Comparator<T>): Int {
    ...
    while (i < shortestLength) {
        val c = comparator.compare(this[i]!!, other[i]!!).also { if (it != 0) return it }
        i++
    }
