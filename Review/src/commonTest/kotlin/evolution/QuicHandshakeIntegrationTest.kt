...
class QuicHandshakeIntegrationTest {
    ...
    fun testPartialHandshakeWithPublicServer() = runTest(timeout = 30000L) { // Generous timeout for network ops
        ...
        val cryptoDataToSendQueue = mutableListOf<Pair<ByteArray, EncryptionLevel>>()
        ...
        val tlsHandler = QuicTlsHandler(
            ...
            onHandshakeDataToSendCallback = { data, level ->
                ...
                cryptoDataToSendQueue.add(Pair(data, level))
            },
        ...
        try {
            ...
            println("Integration Test: Bound to ${localBoundAddress.host}:${localBoundAddress.port}")
            ...
            tlsHandler.startClientHandshake(targetHost, listOf("h3", "h3-29")) // Common ALPNs for QUIC/HTTP3

            // Send initial packets
            if (cryptoDataToSendQueue.isNotEmpty()) {
                val (clientHelloData, clientHelloLevel) = cryptoDataToSendQueue.removeFirst()
                ...
                println("Integration Test: Sending Initial packet (${protectedInitialPacketBytes.size} bytes) to $remoteAddress")
                networkService.send(borg.trikeshed.io.network.DatagramPacket(protectedInitialPacketBytes, remoteAddress))
            ...
            job = launch(testSpecificContext) { // Launch in the test context
                for (i in 1..5) { // Try to receive a few packets
                    ...
                    println("Integration Test: Received ${receivedDatagram.data.size} bytes from ${receivedDatagram.address}")
                    ...
                    for (frame in frames) {
                        if (frame is QuicFrame.CryptoFrame) {
                            ...
                            while(cryptoDataToSendQueue.isNotEmpty()) {
                                val (dataToSend, sendLevel) = cryptoDataToSendQueue.removeFirst()
                                ...
                                networkService.send(borg.trikeshed.io.network.DatagramPacket(protectedBytes, remoteAddress!!))
