...
kotlin {
    ...
    linuxX64 {
        binaries {
            executable() // Or framework(), sharedLib(), staticLib() as appropriate
        }

        compilations.main.cinterops {
            val asyncIoLinux by creating { // Changed name to be more specific
                defFile(project.file("src/linuxX64Main/cinterop/async_io_linux.def"))
                packageName("com.example.trikeshed.asyncio.native.linux")
                // Adjust includeDirs if the .h file is not found relative to the .def file's location
                // by the cinterop tool. The .def file specifies a relative path, which should work.
                // includeDirs.allHeaders(project.file("src/nativeInterop/cinterop"))
            }
        }

        // Add C source file to the compilation
        compilations.getByName("main").cSources.from(files("src/nativeInterop/cinterop/async_io_linux.c"))
    }

    listOf(macosX64(), macosArm64()).forEach { target -> // Use direct target functions
        target.binaries {
            executable() // Or framework(), sharedLib(), staticLib() as appropriate for trikeshed-core
        }

        target.compilations.getByName("main").cinterops { // Access main compilation directly
            val asyncIoMacos by creating {
                defFile(project.file("src/macosMain/cinterop/async_io_macos.def"))
                packageName("com.example.trikeshed.asyncio.native.macos")
                // The header file's path is relative from the .def file, so explicit includeDirs
                // for the cinterop task itself regarding the .h location might not be strictly needed here,
                // but if issues arise, it would be:
                // includeDirs.allHeaders(project.file("src/nativeInterop/cinterop"))
            }
        }

        target.compilations.getByName("main").cSources.from(files("src/nativeInterop/cinterop/async_io_macos.c"))
    }
