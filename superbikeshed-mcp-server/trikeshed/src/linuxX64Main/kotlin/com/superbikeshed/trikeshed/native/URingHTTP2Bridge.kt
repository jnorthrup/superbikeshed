...
class URingHTTP2Bridge(
    ...
    private val connections = mutableMapOf<Int, URingHTTP2Connection>()
    private val acceptChannel = Channel<Int>(Channel.UNLIMITED)
    private var requestHandler: (suspend (HTTP2Request) -> HTTP2Response)? = null
...
class URingHTTP2Connection(
    private val transport: URingTransport,
    ...
    // private val h2protocol = Http2Protocol.Http2Connection(transport as QuicConnection, isServer)
    ...
    private suspend fun sendServerPreface() {
        // Send SETTINGS frame
        val settings = buildSettingsFrame()
        transport.write(settings)
    }
    
    private fun buildSettingsFrame(): ByteArray {
    ...
    private fun encodeFrame(type: Byte, flags: Byte, streamId: Int, payload: ByteArray): ByteArray {
    ...
    private suspend fun handleSettings(frame: Frame) {
    ...
    private suspend fun handlePing(frame: Frame) {
        if (frame.flags and FLAG_ACK == 0.toByte()) {
            // Echo with ACK
    ...
    private suspend fun sendResponse(streamId: Int, response: HTTP2Response) {
    ...
    private suspend fun handleWindowUpdate(frame: Frame) {
        // Handle window updates
    }
    
    private fun handleGoAway(frame: Frame) {
        // Connection closing
    }
    
    private fun handleRstStream(frame: Frame) {
        // Stream reset
    }
...
private fun io_uring_prep_accept(
