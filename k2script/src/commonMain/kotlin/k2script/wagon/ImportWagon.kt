...
class ImportWagon(
    internal val repositories: List<Repository> = listOf(
        Repository("central", "https://repo1.maven.org/maven2"),
        Repository("gradle-plugins", "https://plugins.gradle.org/m2")
    ),
    internal val cacheDir: String = "${System.getProperty("user.home")}/.k2script/imports"
) {
    ...
    internal fun parseCoordinate(coordinate: String): Artifact {
    ...
    internal fun resolveToLocalPath(artifact: Artifact): String {
        val groupPath = artifact.groupId.replace('.', '/')
        return "$cacheDir/$groupPath/${artifact.artifactId}/${artifact.version}/${artifact.fileName}"
    }
    
    internal fun isCached(localPath: String): Boolean {
        return IOMemento.exists(localPath)
    }
    
    internal suspend fun downloadArtifact(artifact: Artifact, localPath: String) {
    ...
    internal suspend fun downloadFromUrl(url: String): ByteIndexed {
...
class ScriptImportProcessor(
    internal val wagon: ImportWagon = ImportWagon()
) {
    ...
    internal fun extractImports(scriptContent: String): List<ImportAnnotation> {
    ...
    internal fun removeImportAnnotations(scriptContent: String): String {
