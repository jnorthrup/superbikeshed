...
class TrikeShedTransport(
    ...
    private suspend fun fetchHttp(url: String): ByteIndexed {
    ...
    private suspend fun fetchQuic(url: String): ByteIndexed {
    ...
    private suspend fun fetchFile(url: String): ByteIndexed {
...
class RepositoryCache(
    ...
    private val metadataCache = mutableMapOf<String, RepositoryMetadata>()
    ...
    private suspend fun loadMetadata(repo: Repository): RepositoryMetadata {
...
class TrikeShedWagon(
    ...
    private val cache: RepositoryCache = RepositoryCache(),
    private val reactor: Reactor = Reactor()
) {
    ...
    private suspend fun fetchFromRepository(repo: Repository, artifact: Artifact): ByteIndexed {
    ...
    private suspend fun cacheArtifact(artifact: Artifact, data: ByteIndexed) {
...
class BatchWagon(
    private val wagon: TrikeShedWagon,
    private val reactor: Reactor = Reactor()
) {
    ...
    suspend fun resolveBatch(artifacts: Indexed<Artifact>): Indexed<ResolveResult> {
        ...
        val tasks = artifacts.a j { i ->
            ...
            reactor.spawn {
                try {
                    val data = wagon.resolve(artifact)
                    ResolveResult.Success(artifact, data)
                } catch (e: Exception) {
                    ResolveResult.Failure(artifact, e.message ?: "Unknown error")
                }
            }
        }
...
private fun ByteIndexed.extractBody(): ByteIndexed {
...
private fun ByteArray.toByteIndexed(): ByteIndexed = 
    ByteIndexed(size j { this[it] })

private fun String.toByteIndexed(): ByteIndexed =
