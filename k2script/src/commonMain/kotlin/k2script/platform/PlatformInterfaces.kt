...
interface FileSystemOperations {
    ...
    suspend fun fileExists(file: File): Boolean = TrikeshedFiles.exists(file.path)
    suspend fun isDirectory(file: File): Boolean = TrikeshedFiles.isDirectory(file.path)
    suspend fun readText(file: File): String = TrikeshedFiles.readString(file.path)
    suspend fun writeText(file: File, text: String) = TrikeshedFiles.write(file.path, text.encodeToByteArray())
