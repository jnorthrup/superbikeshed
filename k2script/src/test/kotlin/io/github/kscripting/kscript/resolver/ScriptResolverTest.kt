...
class ScriptResolverTest {
    internal val currentNativeOsType = OsType.native
    internal val testProperties = Properties()
    internal val testEnv = mutableMapOf<String, String>()

    internal val testHome = OsPath.createOrThrow(OsType.native, "build/tmp/script_resolver_test")
    internal val config = ConfigBuilder(currentNativeOsType, testProperties, testEnv).apply {
        tempDir = testHome.resolve("temp")
        userHomeDir = testHome.resolve("home")
        kotlinHomeDir = testHome.resolve("kotlin")
    }.build()

    internal val cache = Cache(testHome.resolve("cache"))
    internal val inputOutputResolver = InputOutputResolver(config.osConfig, cache)
    internal val scriptingConfig = ScriptingConfig("", "", "", "", "", null)
    internal val sectionResolver = SectionResolver(inputOutputResolver, Parser(), scriptingConfig)
    internal val scriptResolver = ScriptResolver(inputOutputResolver, sectionResolver, scriptingConfig)

    internal val defaultPackageName = PackageName("kscript.scriplet")
    ...
    internal fun String.discardEmptyLines(): String = this.lines().filterNot { it.isBlank() }.joinToString("\n")
}
