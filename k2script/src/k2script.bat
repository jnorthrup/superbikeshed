...
rem k2script - Having fun with Kotlin 2.x scripting
...
rem Set K2SCRIPT_HOME and other k2script environment variables
set K2SCRIPT_HOME=%~dp0..
set K2SCRIPT_FILE=%1
...
if !_java_major_version! geq 9 (
  ...
  rem Enable native access for all unnamed modules (required for modern JVMs in 2025)
  set JAVA_OPTS=!JAVA_OPTS! "--enable-native-access=ALL-UNNAMED"
)

set CLASS_PATH=%_BIN_DIR%\*

java !JAVA_OPTS! -classpath %CLASS_PATH% k2script.K2scriptKt windows %KOTLIN_OPTS%
