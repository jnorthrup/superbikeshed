...
class DefaultScriptEngine : Handler {
    ...
    internal val internalRegistry = mutableMapOf<String, Any>()
    internal val dependencyResolvers = mutableListOf<DependencyResolver>()
     ...
     */
    internal fun registerWithRouter() {
        engineScope.launch {
    ...
    internal fun validateScript(scriptFile: File): List<String> {
        return emptyList()
    }

    internal fun parseDependencies(scriptFile: File): borg.trikeshed.lib.Indexed<String> {
        return borg.trikeshed.lib.emptySeries()
    }

    internal fun executeScript(scriptFile: File, scriptArgs: Array<String>): Boolean {
        return true
    }
}
...
internal class DependencyParser(internal val resolvers: List<DependencyResolver>) {
