...
class TrikeshedDependencyServer(
    internal val socketPath: String = "/tmp/trikeshed.sock",
    internal val port: Int = 8080,
    internal val enableRealtime: Boolean = true,
    internal val persistToRepo: Boolean = true,
    internal val mavenRepo: Path = Paths.get(System.getProperty("user.home"), ".m2", "repository")
) {
    
    internal val serverScope = CoroutineScope(Dispatchers.IO + SupervisorJob())
    internal val isRunning = AtomicBoolean(false)
    internal val activeTransfers = ConcurrentHashMap<String, TransferSession>()
    internal val dependencyCache = ConcurrentHashMap<String, CachedDependency>()
     ...
     */
    internal fun launchUnixSocketServer() {
        serverScope.launch {
            try {
     ...
     */
    internal fun handleClientConnection(clientSocket: SocketChannel) {
        serverScope.launch {
            try {
     ...
     */
    internal fun handleTcpClientConnection(clientSocket: Socket) {
        serverScope.launch {
            try {
     ...
     */
    internal fun handleLegacyTcpConnection(clientSocket: Socket) {
        serverScope.launch {
            try {
                // Read transfer request
     ...
     */
    internal suspend fun processTransfer(session: TransferSession): TransferResponse {
     ...
     */
    internal suspend fun materializeDependency(coordinate: String, realtime: Boolean): CachedDependency? {
     ...
     */
    internal suspend fun cureSocket(session: TransferSession, response: TransferResponse) {
     ...
     */
    internal suspend fun persistToMavenRepo(coordinates: List<String>) {
     ...
     */
    internal suspend fun readTransferRequest(socket: SocketChannel): TransferRequest {
     ...
     */
    internal suspend fun readLegacyTransferRequest(socket: Socket): TransferRequest {
     ...
     */
    internal suspend fun sendTransferResponse(socket: SocketChannel, response: TransferResponse) {
     ...
     */
    internal suspend fun sendLegacyTransferResponse(socket: Socket, response: TransferResponse) {
