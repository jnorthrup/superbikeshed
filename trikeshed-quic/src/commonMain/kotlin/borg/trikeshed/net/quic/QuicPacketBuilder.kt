...
object QuicPacketBuilder {
    ...
    fun buildHttp3Response200(): QuicPacket {
        ...
        return QuicPacket(
            header = QuicHeader(
                ...
                packetNumber = (kotlinx.datetime.Clock.System.now().toEpochMilliseconds() and 0xFFFFFFL)
            ),
            frames = 1 j {
                StreamFrame(
                    streamId = 0,
    ...
    internal fun buildHttp3Headers(): Indexed<Byte> {
    ...
    fun serializePacket(packet: QuicPacket): Indexed<Byte> {
        ...
        val pn = packet.header.packetNumber.toInt()
        when {
            pn < 0x40 -> {
