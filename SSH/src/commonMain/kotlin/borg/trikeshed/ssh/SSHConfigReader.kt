...
enum class SSHConfigOption(val key: String) {
    ...
    companion object {
        private val map = values().associateBy { it.key.lowercase() }
...
class DefaultSSHConfigParser : SSHConfigParser {
    ...
    private fun parseKeyOptions(optionString: String): SSHKeyOptions {
...
class SSHConfigResolver(
    private val parser: SSHConfigParser = DefaultSSHConfigParser()
) {
    ...
    private fun matchesHostPattern(hostname: String, pattern: String): Boolean {
        // Handle wildcards
        val regex = pattern
            .replace(".", "\\.")
            .replace("*", ".*")
    ...
    private fun parseIdentityFiles(value: String?): List<String> {
...
class OpenSSHKeyParser : SSHKeyReader {
    override suspend fun readPrivateKey(path: String, passphrase: String?): SSHPrivateKey = coroutineScope {
        ...
        when (format) {
            ...
            else -> throw IllegalArgumentException("Unsupported private key format: $format")
        }
    }
    ...
    private suspend fun parseOpenSSHPrivateKey(content: String, passphrase: String?): SSHPrivateKey {
    ...
    private suspend fun parseRSAPrivateKey(content: String, passphrase: String?): SSHPrivateKey {
        // PEM format RSA key
        val base64 = content
    ...
    private suspend fun parseECPrivateKey(content: String, passphrase: String?): SSHPrivateKey {
        // PEM format EC key
        val base64 = content
    ...
    private fun derivePublicKey(privateKey: SSHPrivateKey): SSHPublicKey {
        // Derive public key from private key
...
class SSHFileLocator {
    ...
    suspend fun findIdentityFiles(): List<String> = coroutineScope {
        ...
        files.filter { file ->
            // Only return private keys (not .pub files)
