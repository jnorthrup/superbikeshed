...
class JvmSSHAgent(private val socketPath: String) : SSHAgent {
    ...
    private suspend fun <T> withAgentConnection(block: suspend (SocketChannel) -> T): T {
    ...
    private fun isUnixDomainSocketSupported(): Boolean {
        return try {
    ...
    private fun createAgentMessage(messageType: Byte, data: Indexed<Byte>? = null): ByteBuffer {
    ...
    private suspend fun readAgentMessage(channel: SocketChannel): AgentMessage {
        // Read length
    ...
    private fun readUInt32(data: Indexed<Byte>, offset: Int): Int {
    ...
    private fun readSSHString(data: Indexed<Byte>, offset: Int): Join<Indexed<Byte>, Int>? {
    ...
    private data class AgentMessage(
...
class WindowsSSHAgent(private val pipeName: String = "\\\\.\\pipe\\openssh-ssh-agent") : SSHAgent {
...
object SSHKeyUtils {
    ...
    private fun computeSHA256(data: Indexed<Byte>): ByteArray {
        // Would use actual SHA256
        return ByteArray(32) { i -> (i * 7).toByte() }
    }
    
    private fun computeMD5(data: Indexed<Byte>): ByteArray {
