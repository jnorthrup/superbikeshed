...
value class Parser<T>(val parse: (Indexed<Token>) -> ParseResult<T>?) {
    companion object {
        ...
        fun token(type: TokenType): Parser<Token> = Parser { tokens ->
            if (tokens.size == 0) null
            else {
                val token = tokens[0]
                if (token.type == type) ParseResult(token, tokens.size - 1 j { i -> tokens[i + 1] })
                else null
            }
        }
    ...
    fun many(): Parser<Indexed<T>> = Parser { tokens ->
...
object BashParsers {
    ...
    val literal: Parser<String> = Parser { tokens ->
        if (tokens.size == 0) null
            ...
            val token = tokens[0]
            if (token.type == TokenType.LITERAL) ParseResult(token.literal, tokens.size - 1 j { i -> tokens[i + 1] })
            else null
        }
    }
    ...
    val sequenceExpr: Parser<Indexed<String>> = Parser.memoize(
        lbrace.flatMap { _ ->
    ...
    val listExpr: Parser<Indexed<String>> = Parser.memoize(
        lbrace.flatMap { _ ->
    ...
    val braceExpr: Parser<Indexed<String>> = Parser.memoize(
    ...
    val fullExpr: Parser<Indexed<String>> = Parser.memoize(
        prefix.flatMap { pre ->
