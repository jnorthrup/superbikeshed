...
object RequestFactoryBroker {
    @JvmInline value class ServiceToken(val value: String)
    @JvmInline value class MethodToken(val value: String)
    @JvmInline value class EntityProxyId(val value: String)
        ...
        suspend fun handleResponse(responseBytes: Series<Byte>) {
            ...
            when (val result = parseResponse(response)) {
                ...
                is Response.EntityUpdated -> {
                    val id = EntityProxyId(getStringField(response, "entityId"))
    ...
    class Server {
        ...
        fun registerService(token: String, service: Any) {
            services[ServiceToken(token)] = service
        }

        fun registerValidator(methodToken: String, validator: (Series<Any?>) -> Boolean) {
            validators[MethodToken(methodToken)] = validator
        }
        ...
        private fun handleCreate(request: Request.Create): Response {
            ...
            val id = EntityProxyId(generateEntityId())
    ...
    private fun parseRequest(json: Any?): Request {
        ...
        val serviceToken = ServiceToken(getStringField(obj, "serviceToken"))
        val methodToken = MethodToken(getStringField(obj, "methodToken"))
        
        return when (val type = getStringField(obj, "type")) {
            "invoke" -> Request.Invoke(
                serviceToken,
                methodToken,
                getArrayField(obj, "args").size j { i -> getArrayField(obj, "args")[i] }
            )
            ...
            "update" -> Request.Update(
                ...
                getStringField(obj, "entityToken"),
                EntityDelta(
                    EntityProxyId(getStringField(obj, "entityId")),
    ...
    private fun parseResponse(json: Any?): Response {
        ...
        return when (val type = getStringField(obj, "type")) {
            ...
            "entityCreated" -> Response.EntityCreated(
                getStringField(obj, "entityToken"),
                EntityProxyId(getStringField(obj, "id")),
                getLongField(obj, "version")
            )
    ...
    private fun serializeRequest(request: Request): Map<String, Any?> {
        val base = mapOf(
            "serviceToken" to request.serviceToken.value,
            "methodToken" to request.methodToken.value
        )
        ...
        return when (request) {
            ...
            is Request.Update -> base + mapOf(
                "type" to "update",
                "entityToken" to request.entityToken,
                "entityId" to request.delta.id.value,
            ...
            is Request.Delete -> base + mapOf(
                "type" to "delete",
                "entityToken" to request.entityToken,
                "entityId" to request.version.id.value,
                "version" to request.version.version
            )
        }
    }
    ...
    private fun generateEntityId(): String = 
        System.currentTimeMillis().toString(16) + (0..0xFFFF).random().toString(16)
} 