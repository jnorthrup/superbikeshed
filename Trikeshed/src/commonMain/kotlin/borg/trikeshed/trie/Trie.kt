 ...
 */
public class Trie(var root: Map<String, Node> = linkedMapOf()) {
    public var freeze: Boolean = false
    public fun add(v: Int, vararg values: String) {
        var children = root
    ...
    public fun contains(vararg values: String): Boolean = search(*values) != null

    public operator fun get(vararg key: String): Int? = search(*key)?.payload

    public fun frez(n: Node) {
//        if (n.leaf) return
    ...
    public fun search(vararg segments: String): Node? {
