...
/**
 * Creates a new series with the given size and function.
 */
infix fun <T> Int.j(f: (Int) -> T): Series<T> = object : Series<T> {
    override val a: Int = this@j
    override val b: (Int) -> T = f
}

/**
 * Gets the size of a series.
 */
val <T> Series<T>.size: Int get() = a

/**
 * Gets an element from a series.
 */
 ...
 */
fun <T> Series<T>.toList(): List<T> = List(size) { get(it) }

/**
 * Converts a series to an array.
 */
inline fun <reified T> Series<T>.toArray(): Array<T> = Array(size) { get(it) }

/**
 * Maps a series to a new series.
 */
inline fun <T, R> Series<T>.map(transform: (T) -> R): Series<R> = size j { transform(get(it)) }
 ...
 * Folds a series.
 */
inline fun <T, R> Series<T>.fold(initial: R, operation: (R, T) -> R): R {
 ...
 * Reduces a series.
 */
inline fun <T> Series<T>.reduce(operation: (T, T) -> T): T {
    if (size == 0) throw UnsupportedOperationException("Empty series can't be reduced.")
    var accumulator = get(0)
 ...
 */
inline fun <T> Series<T>.find(predicate: (T) -> Boolean): T? {
    for (i in 0 until size) {
        val element = get(i)
        if (predicate(element)) {
            return element
 ...
 */
inline fun <T> Series<T>.findLast(predicate: (T) -> Boolean): T? {
 ...
 */
inline fun <T> Series<T>.any(predicate: (T) -> Boolean): Boolean {
    for (i in 0 until size) {
        if (predicate(get(i))) {
            return true
        }
 ...
 */
inline fun <T> Series<T>.all(predicate: (T) -> Boolean): Boolean {
    for (i in 0 until size) {
        if (!predicate(get(i))) {
            return false
        }
...
/**
 * Checks if none of the elements match the predicate.
 */
inline fun <T> Series<T>.none(predicate: (T) -> Boolean): Boolean {
    for (i in 0 until size) {
        if (predicate(get(i))) {
            return false
        }
 ...
 */
inline fun <T> Series<T>.count(predicate: (T) -> Boolean): Int {
    var count = 0
    for (i in 0 until size) {
        if (predicate(get(i))) {
            count++
...
/**
 * Gets the first element or null if the series is empty.
 */
fun <T> Series<T>.firstOrNull(): T? = if (size == 0) null else get(0)

/**
 * Gets the last element or null if the series is empty.
 */
fun <T> Series<T>.lastOrNull(): T? = if (size == 0) null else get(size - 1)

/**
 * Gets the element at the given index or null if the index is out of bounds.
 */
fun <T> Series<T>.getOrNull(index: Int): T? = if (index in 0 until size) get(index) else null

/**
 * Gets the element at the given index or the default value if the index is out of bounds.
 */
fun <T> Series<T>.getOrElse(index: Int, defaultValue: (Int) -> T): T = if (index in 0 until size) get(index) else defaultValue(index)

/**
 * Gets the element at the given index or the default value if the index is out of bounds.
 */
fun <T> Series<T>.getOrElse(index: Int, defaultValue: T): T = if (index in 0 until size) get(index) else defaultValue

/**
 * Gets the element at the given index or throws an exception if the index is out of bounds.
 */
fun <T> Series<T>.elementAt(index: Int): T {
    if (index !in 0 until size) throw IndexOutOfBoundsException("Index $index out of bounds for series of size $size")
    return get(index)
}
 ...
 * Gets the element at the given index or null if the index is out of bounds.
 */
fun <T> Series<T>.elementAtOrNull(index: Int): T? = if (index in 0 until size) get(index) else null

/**
 * Gets the element at the given index or the default value if the index is out of bounds.
 */
fun <T> Series<T>.elementAtOrElse(index: Int, defaultValue: (Int) -> T): T = if (index in 0 until size) get(index) else defaultValue(index)

/**
 * Gets the element at the given index or the default value if the index is out of bounds.
 */
fun <T> Series<T>.elementAtOrElse(index: Int, defaultValue: T): T = if (index in 0 until size) get(index) else defaultValue

/**
 * Gets the index of the first element matching the predicate.
 */
inline fun <T> Series<T>.indexOfFirst(predicate: (T) -> Boolean): Int {
    for (i in 0 until size) {
        if (predicate(get(i))) {
            return i
        }
...
/**
 * Gets the index of the last element matching the predicate.
 */
inline fun <T> Series<T>.indexOfLast(predicate: (T) -> Boolean): Int {
    for (i in size - 1 downTo 0) {
        if (predicate(get(i))) {
            return i
        }
 ...
 */
fun <T> Series<T>.indexOf(element: T): Int {
    for (i in 0 until size) {
        if (get(i) == element) {
            return i
        }
 ...
 */
fun <T> Series<T>.lastIndexOf(element: T): Int {
    for (i in size - 1 downTo 0) {
        if (get(i) == element) {
            return i
        }
 ...
 */
val <T> Series<T>.indices: IntRange get() = 0 until size

/**
 * Checks if the series is empty.
 */
val <T> Series<T>.isEmpty: Boolean get() = size == 0

/**
 * Checks if the series is not empty.
 */
val <T> Series<T>.isNotEmpty: Boolean get() = size > 0

/**
 * Gets the first element matching the predicate.
 */
inline fun <T> Series<T>.first(predicate: (T) -> Boolean): T {
    for (i in 0 until size) {
        val element = get(i)
        if (predicate(element)) {
            return element
...
/**
 * Gets the last element matching the predicate.
 */
inline fun <T> Series<T>.last(predicate: (T) -> Boolean): T {
    for (i in size - 1 downTo 0) {
        val element = get(i)
        if (predicate(element)) {
            return element
...
/**
 * Gets the first element matching the predicate or null if no such element exists.
 */
inline fun <T> Series<T>.firstOrNull(predicate: (T) -> Boolean): T? {
    for (i in 0 until size) {
        val element = get(i)
        if (predicate(element)) {
            return element
...
/**
 * Gets the last element matching the predicate or null if no such element exists.
 */
inline fun <T> Series<T>.lastOrNull(predicate: (T) -> Boolean): T? {
    for (i in size - 1 downTo 0) {
        val element = get(i)
        if (predicate(element)) {
            return element
...
/**
 * Gets the single element matching the predicate or throws an exception if there is not exactly one such element.
 */
inline fun <T> Series<T>.single(predicate: (T) -> Boolean): T {
    var single: T? = null
    var found = false
    for (i in 0 until size) {
        val element = get(i)
        if (predicate(element)) {
            if (found) throw IllegalArgumentException("More than one element matches the predicate.")
            single = element
            found = true
        }
    }
    if (!found) throw NoSuchElementException("No element matches the predicate.")
    return single!!
}

/**
 * Gets the single element matching the predicate or null if there is not exactly one such element.
 */
inline fun <T> Series<T>.singleOrNull(predicate: (T) -> Boolean): T? {
    var single: T? = null
    var found = false
    for (i in 0 until size) {
        val element = get(i)
    ...
    return if (found) single else null
}

/**
 * Gets the single element or throws an exception if there is not exactly one element.
 */
fun <T> Series<T>.single(): T {
    if (size == 0) throw NoSuchElementException("Series is empty.")
    if (size > 1) throw IllegalArgumentException("Series has more than one element.")
    return get(0)
}

/**
 * Gets the single element or null if there is not exactly one element.
 */
fun <T> Series<T>.singleOrNull(): T? {
