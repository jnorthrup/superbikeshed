...
class TreeMap<K, V>(private val comparator: Comparator<in K>? = null) : NavigableMap<K, V> {
    ...
    // NavigableMap interface methods that need to be implemented
    override fun subMap(fromKey: K, fromInclusive: Boolean, toKey: K, toInclusive: Boolean): NavigableMap<K, V> {
        ...
        return SubMapView(this, fromKey, fromInclusive, toKey, toInclusive, false)
    }

    override fun headMap(toKey: K, inclusive: Boolean): NavigableMap<K, V> {
        return SubMapView(this, null, false, toKey, inclusive, false)
    }

    override fun tailMap(fromKey: K, inclusive: Boolean): NavigableMap<K, V> {
        return SubMapView(this, fromKey, inclusive, null, false, false)
    }
    ...
    private var descendingMapView: NavigableMap<K, V>? = null
    ...
    private inner class KeySet : AbstractMutableSet<K>() {
        ...
        override fun add(element: K): Boolean {
            throw UnsupportedOperationException("Cannot add to a map's key set")
        }
    }
    ...
    private inner class ValueCollection : AbstractMutableCollection<V>() {
        ...
        override fun add(element: V): Boolean {
            throw UnsupportedOperationException("Cannot add to a map's value collection")
        }
    }
    ...
    private class NavigableKeySet<K, V>(private val map: TreeMap<K, V>) : AbstractMutableSet<K>(), NavigableSet<K> {
        ...
        override fun descendingIterator(): MutableIterator<K> {
                // Returns a MutableIterator
                return map.DescendingKeyIterator()
        }
    ...
    private class DescendingTreeMapView<K, V>(
        ...
        // NavigableMap overloads with inclusive flags
        override fun subMap(fromKey: K, fromInclusive: Boolean, toKey: K, toInclusive: Boolean): NavigableMap<K, V> = 
        ...
        override fun navigableKeySet(): NavigableSet<K> {
            ...
            // For now, let's create a simple wrapper around original NavigableKeySet but use descending iterator.
             return object : AbstractMutableSet<K>(), NavigableSet<K> {
                ...
                override fun comparator(): Comparator<in K>? = reverseComparator
                override fun descendingIterator(): MutableIterator<K> = m.KeyIterator() // Reversed again is original
        ...
        private inner class DescendingEntrySetView : AbstractMutableSet<MutableMap.MutableEntry<K,V>>() {
            ...
            override fun clear() = m.clear()
            override fun add(element: MutableMap.MutableEntry<K, V>): Boolean {
                m.put(element.key, element.value)
                return true
            }
        }
    }
    ...
    private class SubMapView<K, V>(
        ...
        override fun subMap(newFromKey: K, newFromInclusive: Boolean, newToKey: K, newToInclusive: Boolean): NavigableMap<K, V> {
        ...
        override fun headMap(newToKey: K, newToInclusive: Boolean): NavigableMap<K, V> {
            val (effFrom, effFromIncl) = getRestrictedFromKey(null, false) // Use current lowBound
            val (effTo, effToIncl) = getRestrictedToKey(newToKey, newToInclusive)
            return SubMapView(m, effFrom, effFromIncl, effTo, effToIncl, descending)
        }

        override fun tailMap(newFromKey: K, newFromInclusive: Boolean): NavigableMap<K, V> {
            val (effFrom, effFromIncl) = getRestrictedFromKey(newFromKey, newFromInclusive)
    ...
    private inner class SubMapKeySet : AbstractMutableSet<K>() {
        ...
        override fun add(element: K): Boolean = throw UnsupportedOperationException("Cannot add to a key set")
    }
    private inner class SubMapValueCollection : AbstractMutableCollection<V>() {
        ...
        override fun add(element: V): Boolean = throw UnsupportedOperationException("Cannot add to a value collection")
    }
  }
    ...
    private class SubMapNavigableKeySet<K,V>(
        ...
        override fun comparator(): Comparator<in K>? = subMapView.comparator()

        override fun iterator(): MutableIterator<K> = subMapView.keys.iterator() // Uses SubMapView's key set iterator
