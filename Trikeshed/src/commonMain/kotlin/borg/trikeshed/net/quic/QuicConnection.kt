...
class QuicConnection(
    ...
    suspend fun createStream(priority: Int? = null): QuicStream? {
        ...
        // Ensure socket and its remote address are available before creating a stream
        val currentSocket = socket
        val remoteAddr = (currentSocket?.remoteAddress as? InetSocketAddress)
            ?: (currentSocket?.inetAddress as? InetSocketAddress) // Fallback for locally bound server sockets if applicable

        if (remoteAddr == null && currentSocket?.port == null) {
            // Cannot determine remote address, critical for sending packets via stream.
            // This might happen if connect was not called or failed.
            println("Cannot create stream: Remote address unknown.")
            return null
        }

        // If remoteAddr is still null but socket has a port, it might be a server scenario.
        // For now, we assume client role primarily, where remoteAddress is the server.
        // A more robust solution would handle server-side stream creation context.
        val streamRemoteAddress = remoteAddr ?: InetSocketAddress(currentSocket!!.inetAddress, currentSocket.port)
    ...
    suspend fun sendData(streamId: Long, data: ByteBuffer): Boolean = withContext(Dispatchers.IO) {
        ...
        val packetBuffer = ByteBuffer.allocate(STREAM_ID_HEADER_SIZE + dataSize)
    ...
    private suspend fun startPacketReceiver() {
        ...
        val receiveBuffer = ByteArray(streamBufferSize + STREAM_ID_HEADER_SIZE) // Max possible size
        ...
        while (currentSocket.isConnected && !currentSocket.isClosed) {
            try {
                ...
                val receivedBuffer = ByteBuffer.wrap(datagramPacket.data, 0, datagramPacket.length)
                val streamId = receivedBuffer.getLong()

                val stream = activeStreams[streamId]
                if (stream != null && !stream.isClosed()) {
                    // restante data for the stream (after stream ID)
                    val streamData = ByteBuffer.allocate(receivedBuffer.remaining())
                    streamData.put(receivedBuffer)
                    streamData.flip()
    ...
    private suspend fun establishRegularConnection(
        ...
        val sessionTicket = extractSessionTicket(responsePacket)
        if (sessionTicket != null) {
            val sessionData = QuicSessionData(
                serverAddress,
                port,
                "SESSION_ID".toByteArray(), // Placeholder for session ID
                sessionTicket,
                System.currentTimeMillis() + (3600 * 1000) // Example: ticket valid for 1 hour
            )
            sessionCache.storeSession(serverAddress, port, sessionData)
        }
        
        return@withContext socket
    }

    private fun extractSessionTicket(packet: DatagramPacket): ByteArray? {
        ...
        if (packet.length > 0 && !packetData.contentEquals("INITIAL_PACKET".toByteArray())) {
            return packetData
        }
        return null
    }
...
class QuicStream(
    ...
    internal val internalReceiveChannel: Channel<ByteBuffer> = Channel(Channel.BUFFERED)
) {
    @Volatile
    private var closed = false
    var remoteAddress: InetSocketAddress? = null // This should be set when stream is created or by first packet
