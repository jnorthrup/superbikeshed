...
class Socks5Server(
    ...
    // Server state
    private var serverChannel: AsyncChannel? = null
    private var serverJob: Job? = null
    private val activeConnections = mutableMapOf<String, SocksConnection>()
    
    // Channels for connection management
    private val acceptChannel = Channel<AsyncChannel>(Channel.UNLIMITED)
    private val commandChannel = Channel<SocksCommand>(Channel.UNLIMITED)
    
    // io_uring batch processor
    private val batchProcessor = Channel<BatchOperation>(Channel.UNLIMITED)
    
    /**
     * Connection state for each SOCKS client with CCEK context
     */
    private data class SocksConnection(
        val id: String,
    ...
    private sealed class SocksCommand {
     ...
     */
    private suspend fun handleConnection(connection: SocksConnection) {
        try {
     ...
     */
    private suspend fun negotiateAuth(connection: SocksConnection): Boolean = 
     ...
     */
    private suspend fun sendAuthResponse(connection: SocksConnection, method: Byte) {
     ...
     */
    private suspend fun processRequest(connection: SocksConnection) {
     ...
     */
    private suspend fun relayData(connection: SocksConnection) = coroutineScope {
        val batchSize = 16
     ...
     */
    private suspend fun relayBatch(
        from: AsyncChannel,
        to: AsyncChannel,
     ...
     * Relay UDP packets
     */
    private suspend fun relayUdp(connection: SocksConnection) {
     ...
     */
    private fun parseLocalAddress(addr: String): Pair<String, Int> {
     ...
     */
    private fun cleanupConnection(connection: SocksConnection) {
     ...
     */
    private suspend fun uringBatchProcessor() {
     ...
     */
    private suspend fun createUringServerChannel(host: String, port: Int): AsyncChannel {
        return UringServerChannel(host, port, ioContext)
    }
    
    private suspend fun createUringChannel(fd: Int): AsyncChannel {
        return UringChannel(fd, ioContext)
    }
    
    private suspend fun createUringClientChannel(host: String, port: Int): AsyncChannel {
        return UringClientChannel(host, port, ioContext)
    }
    
    private suspend fun createUringUdpChannel(): AsyncChannel {
        return UringUdpChannel(ioContext)
    }

    // CCEK-based connection handler using channel chain context
    private suspend fun handleConnectionWithCCEK(connection: SocksConnection, ccek: CcekContext) {
...
class UringChannel(
    private val fd: Int,
    private val context: IOContext.UringContext
) : AsyncChannel {
