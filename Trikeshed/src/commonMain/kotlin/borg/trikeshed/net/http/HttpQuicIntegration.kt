...
class HttpQuicIntegration(
     ...
     */
    private suspend fun handleHttpOverQuic(connection: QuicConnection) {
     ...
     * Handle HTTP stream
     */
    private suspend fun handleHttpStream(stream: QuicStream) {
        try {
     ...
     */
    private fun detectHttpVersion(requestLine: String): String? {
        return when {
     ...
     */
    private suspend fun handleUnknownVersion(stream: QuicStream) {
     ...
     */
    private suspend fun readFullRequest(stream: QuicStream, buffer: MutableList<Byte>): ByteArray {
     ...
     */
    private fun hasKeepAlive(headers: Indexed<Join<HttpHeaderName, HttpHeaderValue>>): Boolean {
     ...
     */
    private fun hasConnectionClose(headers: Indexed<Join<HttpHeaderName, HttpHeaderValue>>): Boolean {
        for (i in 0 until headers.a) {
            val header = headers[i]
            if (header.a.value.equals("Connection", ignoreCase = true) &&
