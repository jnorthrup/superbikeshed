    ...
    suspend fun initialize(): TrikeshedIntegrationResult {
        return try {
            ...
            when (dbResult) {
                ...
                else -> println("Unknown database creation result")
            }
        ...
        return try {
            ...
            val document = CouchDocument(
                data = buildJsonObject { 
                    put("content", content)
                    put("created_at", kotlinx.datetime.Clock.System.now().toEpochMilliseconds())
                }
            )
                ...
                is Either.Right -> {
                    ...
                    val event = IntegrationEvent(
                        ...
                        data = mapOf(
                            ...
                            "created_at" to kotlinx.datetime.Clock.System.now().toEpochMilliseconds()
                        )
                    )
                    network.emit("integration", event)
    ...
    private suspend fun getDocument(id: String): IntegrationDocumentResult {
        return try {
            ...
            when (couchResult) {
                is Either.Right -> {
                    val doc = couchResult.value
                    val content = doc.data["content"] ?: return Either.Left(IntegrationError("No content found"))
                    val ipfsHash = doc.data["ipfs_hash"]
                            ...
                            if (contentBytes.contentEquals(ipfsBytes)) {
                                Either.Right(
                                    IntegrationDocument(
                                        ...
                                        ipfsHash = ipfsHash,
                                        metadata = mapOf(
                                            "retrieved_at" to kotlinx.datetime.Clock.System.now().toEpochMilliseconds(),
                                            "verified" to true
                                        )
                                    )
                                )
                ...
                is Either.Left -> Either.Left(IntegrationError(couchResult.value))
                else -> Either.Left(IntegrationError("Unknown result type"))
            }
        } catch (e: Exception) {
            Either.Left(IntegrationError(e.message ?: "Unknown error"))
        }
...
private fun generateEventId(): EventId = "event_${kotlinx.datetime.Clock.System.now().toEpochMilliseconds()}"
...
data class IntegrationEvent(
    ...
    val timestamp: EventTimestamp = kotlinx.datetime.Clock.System.now().toEpochMilliseconds()
)
