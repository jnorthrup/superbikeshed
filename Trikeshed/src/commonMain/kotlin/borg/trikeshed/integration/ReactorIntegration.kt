    ...
    private suspend fun retrieveWithReactor(id: StorageId): ReactorStorageResult {
        try {
            ...
            when (couchResult) {
                is Either.Right -> {
                    val doc = couchResult.value
                    val content = doc.data["content"] ?: return ReactorStorageResult.Error("No content found")
...
data class ReactorEvent(
    ...
    val timestamp: ReactorEventTimestamp = kotlinx.datetime.Clock.System.now().toEpochMilliseconds()
)

// === UTILITY FUNCTIONS ===

private fun generateEventId(): ReactorEventId = "event_${kotlinx.datetime.Clock.System.now().toEpochMilliseconds()}"
