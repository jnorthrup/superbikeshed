...
class NativeHubExample {
    ...
    class DHTNode(
        ...
        private val channels = mutableMapOf<NodeID, IPCChannel>()
        ...
        private suspend fun handleMessage(message: IPCMessage) {
            when (message.type) {
        ...
        private suspend fun getOrCreateChannel(target: NodeID): IPCChannel {
        ...
        private suspend fun handleQuery(message: IPCMessage) {
            // Simplified query handling
            println("Node ${nodeId.toHex()} handling query from ${message.source}")
        }
        
        private suspend fun handleStore(message: IPCMessage) {
            println("Node ${nodeId.toHex()} storing data from ${message.source}")
        }
        
        private suspend fun handleFindNode(message: IPCMessage) {
        ...
        private fun serializeNodes(nodes: List<WeightedNode>): Indexed<Byte> {
...
// Extension functions
private fun Indexed<Byte>.toByteArray(): ByteArray = ByteArray(a) { b(it) }
private fun ByteArray.toIdx(): Indexed<Byte> = this.size j { i: Int -> this[i] }
