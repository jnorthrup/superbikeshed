    ...
    fun testColumnMetadata() {
        fileBuffer(target).use { fileBuf ->
            ...
            meta.forEachIndexed { index, columnMeta ->
                assertEquals(expectedTypeForColumn(index), columnMeta.type, "Column $index type mismatch")
            }
            // Add a helper function if needed, but keeping it simple for this diff
