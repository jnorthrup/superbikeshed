...
class TLS13Connection(
    ...
    internal val transport: QuicConnection? = null,
    ...
    internal val transcriptHash = mutableListOf<Byte>()
    
    // Crypto state
    internal var selectedCipherSuite: CipherSuite? = null
    internal var tls13Crypto: TLS13Crypto? = null
    internal var handshakeSecrets: HandshakeSecrets? = null
    internal var applicationSecrets: ApplicationSecrets? = null
    
    // Key exchange
    internal var keyExchange: CommonCrypto.KeyExchange? = null
    internal var localKeyPair: Join<PublicKey, PrivateKey>? = null
    internal var sharedSecret: SessionKey? = null
    
    // Extensions state
    internal val supportedGroups = listOf(
        TLS13Protocol.NamedGroups.X25519,
        TLS13Protocol.NamedGroups.SECP256R1,
        TLS13Protocol.NamedGroups.SECP384R1
    )
    
    internal val supportedSignatures = listOf(
     ...
     * Send ClientHello
     */
    internal suspend fun sendClientHello() {
        // Generate random
     ...
     * Process ServerHello
     */
    internal suspend fun processServerHello(body: Indexed<Byte>) {
     ...
     */
    internal fun processEncryptedExtensions(body: Indexed<Byte>) {
     ...
     * Process Certificate
     */
    internal fun processCertificate(body: Indexed<Byte>) {
     ...
     */
    internal fun processCertificateVerify(body: Indexed<Byte>) {
     ...
     * Process Finished
     */
    internal suspend fun processFinished(body: Indexed<Byte>) {
     ...
     */
    internal suspend fun sendFinished() {
     ...
     */
    internal suspend fun sendHandshakeMessage(message: HandshakeMessage) {
     ...
     * Update transcript hash
     */
    internal fun updateTranscript(data: Indexed<Byte>) {
        for (i in 0 until data.a) {
     ...
     */
    internal suspend fun processClientHello(body: Indexed<Byte>) {
     ...
     */
    internal suspend fun sendServerHello() {
