...
class GradleBuildParser {
    ...
    private val groupRegex = Pattern.compile("""^\s*group\s*[=:]?\s*['"]([^'"]+)['"]""", Pattern.MULTILINE)
    private val versionRegex = Pattern.compile("""^\s*version\s*[=:]?\s*['"]([^'"]+)['"]""", Pattern.MULTILINE)
    ...
    private val depRegexString = Pattern.compile(
    ...
    private val depRegexMap = Pattern.compile(
    ...
    private val depRegexPlatform = Pattern.compile(
    ...
    private fun extractValue(content: String, pattern: Pattern): String? {
        val matcher = pattern.matcher(content)
        return if (matcher.find()) matcher.group(1) else null
    }

    private fun configToScope(config: String): String {
