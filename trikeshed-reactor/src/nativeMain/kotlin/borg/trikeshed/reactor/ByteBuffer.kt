...
/**
 * Native platform implementation using ByteArray with manual position tracking.
 * This provides a zero-copy abstraction over native memory management.
 */
...
actual class ByteBuffer internal constructor(
    ...
    /**
     * Native-specific: Access to backing array with current position/limit.
     */
    fun arrayWithBounds(): Triple<ByteArray, Int, Int> = Triple(backing, _position, _limit)
    
    /**
     * Native-specific: Pin memory for C interop.
     */
    fun pin(): Pinned<ByteArray> = backing.pin()
}