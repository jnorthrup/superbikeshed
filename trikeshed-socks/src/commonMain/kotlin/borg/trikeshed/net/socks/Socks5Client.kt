...
class Socks5Client(
    ...
    internal val proxyPort: Int,
    internal val ioContext: IOContext
) {
    internal var proxyChannel: SelectableChannel? = null
    ...
    internal fun buildGreeting(authMethod: AuthMethod): ByteArray {
    ...
    internal suspend fun authenticateUserPass(username: String, password: String) {
    ...
    internal fun buildConnectRequest(host: String, port: Int): ByteArray {
        return buildRequest(0x01, host, port)  // CMD = CONNECT
    }
    
    internal fun buildBindRequest(host: String, port: Int): ByteArray {
        return buildRequest(0x02, host, port)  // CMD = BIND
    }
    
    internal fun buildUDPAssociateRequest(host: String, port: Int): ByteArray {
        return buildRequest(0x03, host, port)  // CMD = UDP ASSOCIATE
    }
    
    internal fun buildRequest(cmd: Byte, host: String, port: Int): ByteArray {
    ...
    internal suspend fun readConnectResponse(): ConnectResponse {
    ...
    internal fun getErrorMessage(status: Byte): String = when (status.toInt() and 0xFF) {
    ...
    internal suspend fun createChannel(host: String, port: Int): SelectableChannel {
...
class SocksConnection(
    internal val channel: SelectableChannel,
 ...
 */
class SocksUDP(
    internal val tcpChannel: SelectableChannel,
    val relayAddress: String,
    val relayPort: Int,
    internal val ioContext: IOContext
) {
    internal var udpSocket: Any? = null  // Platform-specific UDP socket
    ...
    internal fun encapsulateUDP(data: ByteArray, host: String, port: Int): ByteArray {
...
class Socks5ClientBuilder {
    internal var proxyHost: String = ""
    internal var proxyPort: Int = 1080
