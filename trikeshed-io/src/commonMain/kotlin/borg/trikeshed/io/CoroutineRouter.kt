 ...
 */
class CoroutineRouter internal constructor(
    internal val parent: CoroutineRouter? = null,
    internal val context: CoroutineContext = EmptyCoroutineContext,
    internal val handlers: MutableMap<Any, suspend (Any) -> Any> = mutableMapOf(),
    internal val interceptors: MutableList<suspend (Any) -> Any> = mutableListOf(),
    internal val name: String = "router"
) {
    ...
    class RouterBuilder {
        ...
        internal var name: String = "router"
        internal val handlers = mutableMapOf<Any, suspend (Any) -> Any>()
        internal val interceptors = mutableListOf<suspend (Any) -> Any>()
     ...
     */
    internal suspend fun <T> withTiming(
        metricsHandler: suspend (String, Long) -> Unit,
        block: suspend () -> T
    ): T {
    ...
    internal suspend fun <T> withLogging(
        logger: suspend (String) -> Unit,
        block: suspend () -> T
    ): T {
    ...
    internal suspend fun <T> withObservability(
        observer: suspend (String, Any, Any?) -> Unit,
        block: suspend () -> T
    ): T {
 ...
 */
class RateLimiter(internal val permitsPerSecond: Int) {
    internal var lastCheck = 0L // TODO: Platform-specific timestamp
    internal var available = permitsPerSecond
