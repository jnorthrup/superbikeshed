...
export class FormationControls {
    ...
    render() {
        if (!this.visible || this.selectedUnits.size === 0) return;
        ...
        ctx.fillRect(
            startX - this.padding,
            startY - this.padding,
            (this.buttonSize + this.padding) * this.formationShapes.length + this.padding * 2,
            this.buttonSize + this.padding * 2
        );
    ...
    drawFormationOptions(formation) {
        ...
        this.drawSlider(
            ...
            180,
            'Spacing',
            formation.options.spacing,
            COMMAND_CONFIG.COMMAND_RANGES.INDIVIDUAL,
            COMMAND_CONFIG.COMMAND_RANGES.STRATEGIC
        );
