...
class MockComputroniumUnit {
    constructor(id, typeDetails = {}) {
        this.id = id;
        this.type = Object.assign({ name: typeDetails.name || 'TestDrone', hasComputroniumCore: true, maxEnergyShields: typeDetails.maxEnergyShields || 50, speed: typeDetails.speed || 50 }, typeDetails // Allow overriding other type properties
        );
        this.computronium = {
...
class ComputroniumSystem {
    ...
    collectFunctionRequests(entity, focusModeValue, requestedForks) {
        const getPrio = (funcName) => {
            const prioritiesByMode = this.functionPriorities[funcName];
            if (prioritiesByMode)
                return prioritiesByMode[focusModeValue] || 0;
            return 0;
        };
    ...
    applyPerformanceModifiers(entity, deltaTime) {
        ...
        if (entity.movementSystemData) {
            ...
            entity.movementSystemData.accelerationFactor = 1.0;
        ...
        if (entity.sensorSystemData) {
            ...
            if (basicSensorStatus === 'DEGRADED')
                entity.sensorSystemData.rangeFactor = 0.7;
            else if (basicSensorStatus === 'STARVED')
        ...
        if (sas === 'DEGRADED') {
            ...
            entity.specialAbilityDurationFactor = 0.8;
        }
        else if (sas === 'STARVED') {
    ...
    setCoreFocusMode(entity, modeKey) {
        ...
        if (!modeValue) {
            console.warn(`Invalid focus mode key: ${modeKey}`);
            return false;
        }
