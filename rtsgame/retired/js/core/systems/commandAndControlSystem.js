...
exports.CommandAndControlSystem = void 0;
class CommandAndControlSystem {
    ...
    processCommandHierarchy(entities) {
        // Find all commanders
        const commanders = entities.filter(e => e.hasComponent('CommandComponent') &&
    ...
    processCommandChain(commander, subordinates) {
        ...
        if (command.strategicDirective) {
            ...
            subordinates.forEach(subordinate => {
                this.issueTacticalOrders(subordinate, command.strategicDirective, scope, this.calculateCommandLatency(commander, subordinate), chainEfficiency);
            });
    ...
    calculateCommandLatency(commander, target) {
        ...
        return (distance * this.LATENCY_FACTORS.BASE_DISTANCE_LATENCY +
    ...
    updateLatencyHistory() {
        if (this.networkState.latencyHistory.length < 2)
            return;
        ...
        const variance = this.networkState.latencyHistory.reduce((sum, entry) => sum + Math.pow(entry.rtt - avgLatency, 2), 0) / this.networkState.latencyHistory.length;
    ...
    simplifyFormationOrder(order) {
        ...
        if (!simpleFormations.includes(order.formation)) {
            return Object.assign(Object.assign({}, order), { formation: simpleFormations[Math.floor(Math.random() * simpleFormations.length)] });
        }
        return order;
    }
    attemptResync() {
    ...
    getAverageLatency() {
        if (this.networkState.latencyHistory.length === 0)
            return 0;
    ...
    processCommandHierarchy(entities) {
        // Find all commanders
        const commanders = entities.filter(e => e.hasComponent('RankComponent') &&
    ...
    findSubordinateCommanders(commander, scope) {
        return this.findUnitsInRange(commander, scope).filter(unit => {
            var _a;
            const unitRank = ((_a = unit.getComponent('RankComponent')) === null || _a === void 0 ? void 0 : _a.level) || 0;
    ...
    processTacticalOrders(commander, subordinates, commandLatency) {
        var _a;
        const strategicDirective = (_a = commander.getComponent('CommandComponent')) === null || _a === void 0 ? void 0 : _a.currentDirective;
        if (!strategicDirective)
            return;
        subordinates.forEach(subordinate => {
    ...
    getAvailableTacticalOptions(command, computronium) {
        return Object.entries(this.TACTICAL_OPTIONS)
            ...
            .map(([key, option]) => (Object.assign({ key }, option)));
    }
    selectTacticalOption(availableOptions, strategicDirective) {
        // Score each option based on strategic directive
        const scoredOptions = availableOptions.map(option => (Object.assign(Object.assign({}, option), { score: this.scoreTacticalOption(option, strategicDirective) })));
        // Select highest scoring option
        return scoredOptions.reduce((best, current) => current.score > best.score ? current : best);
    }
    ...
    calculateTacticalEffects(tacticalOption, chainEfficiency) {
        const effects = Object.assign({}, tacticalOption.effects);
    ...
    applyTacticalEffects(unit, effects) {
        ...
        if (unit.hasComponent('CombatComponent')) {
            ...
            if (effects.damageBonus)
                combat.damage *= effects.damageBonus;
            if (effects.accuracyBonus)
                combat.accuracy *= effects.accuracyBonus;
        }
        // Apply defensive effects
        if (unit.hasComponent('ShieldComponent')) {
            const shield = unit.getComponent('ShieldComponent');
            if (effects.shieldBonus)
                shield.maxHP *= effects.shieldBonus;
        }
        if (unit.hasComponent('ArmorComponent')) {
            const armor = unit.getComponent('ArmorComponent');
            if (effects.armorBonus)
                armor.value *= effects.armorBonus;
        }
        ...
        if (unit.hasComponent('FormationComponent')) {
            ...
            if (effects.formationBonus)
                formation.efficiency *= effects.formationBonus;
            if (effects.coordinationBonus)
                formation.coordination *= effects.coordinationBonus;
        }
    }
    ...
    findSquadLeader(squad) {
        ...
        return units.reduce((leader, unit) => {
            ...
            if (!leader)
                return unit;
            const leaderRank = ((_a = leader.getComponent('RankComponent')) === null || _a === void 0 ? void 0 : _a.level) || 0;
            const unitRank = ((_b = unit.getComponent('RankComponent')) === null || _b === void 0 ? void 0 : _b.level) || 0;
    ...
    detectThreats(squad, gameContext) {
        ...
        units.forEach(unit => {
            var _a;
            const sensorRange = ((_a = unit.getComponent('SensorComponent')) === null || _a === void 0 ? void 0 : _a.range) || 0;
    ...
    calculateUnitTypeThreat(enemy, unit) {
        ...
        const enemyType = (_a = enemy.getComponent('UnitTypeComponent')) === null || _a === void 0 ? void 0 : _a.type;
        const unitType = (_b = unit.getComponent('UnitTypeComponent')) === null || _b === void 0 ? void 0 : _b.type;
        const matchups = {
            'ANTI_AIR': { 'AIR': 0.5 },
            'ANTI_ARMOR': { 'HEAVY': 0.4 },
            'ANTI_INFANTRY': { 'LIGHT': 0.3 }
        };
        return ((_c = matchups[enemyType]) === null || _c === void 0 ? void 0 : _c[unitType]) || 0;
    }
    ...
    predictEntityState(entity, deltaTime) {
        ...
        const predictedState = {
            ...
            velocity: Object.assign({}, entity.velocity),
    ...
    predictMovementState(state, order, deltaTime, predictionSteps, predictionWindow) {
        ...
        for (let step = 0; step < predictionSteps; step++) {
            ...
            if (distance > moveDistance) {
                ...
                state.position = Object.assign({}, targetPos);
                break;
            }
        }
        // Predict velocity
    ...
    applyServerState(entity, serverState) {
        ...
        entity.position = Object.assign({}, serverState.position);
        entity.velocity = Object.assign({}, serverState.velocity);
        entity.rotation = serverState.rotation;
        // Update command component if needed
        const command = entity.getComponent('CommandComponent');
        if (command && serverState.commandState) {
            command.currentOrder = Object.assign({}, serverState.commandState);
        }
    }
    ...
    storeEntityState(entity) {
        ...
        const state = {
            ...
            rotation: entity.rotation,
            timestamp: Date.now(),
            commandState: (_a = entity.getComponent('CommandComponent')) === null || _a === void 0 ? void 0 : _a.currentOrder
        };
        this.predictionState.entityStates.set(entity.id, state);
    }
    applyPredictedState(entity, predictedState) {
        // Apply predicted position
        entity.position = Object.assign({}, predictedState.position);
        // Apply predicted velocity
        entity.velocity = Object.assign({}, predictedState.velocity);
    ...
    calculatePredictionWindow(computronium, dimensionalFactor) {
        if (!computronium)
            return 1000; // Base window without Computronium
    ...
    handleReadMiss(computronium, message) {
        ...
        if (owner && owner !== computronium) {
            ...
            cacheState.l1Cache.set(line, Object.assign(Object.assign({}, cacheState.l1Cache.get(line)), { state: this.CACHE_COHERENCE_STATES.SHARED }));
        }
        else {
            // No owner, fetch from memory and set to Exclusive
            this.fetchFromMemory(computronium, line);
            cacheState.l1Cache.set(line, Object.assign(Object.assign({}, cacheState.l1Cache.get(line)), { state: this.CACHE_COHERENCE_STATES.EXCLUSIVE }));
        }
    }
    handleWriteMiss(computronium, message) {
        ...
        this.fetchFromMemory(computronium, line);
        cacheState.l1Cache.set(line, Object.assign(Object.assign({}, cacheState.l1Cache.get(line)), { state: this.CACHE_COHERENCE_STATES.MODIFIED }));
    }
    ...
    renderCacheVisualization(ctx, entity) {
        ...
        cacheLevels.forEach((cache, index) => {
            ...
            ctx.fillStyle = 'white';
            ctx.fillText(`${cache.level}: ${coherenceStates.MODIFIED}M ${coherenceStates.SHARED}S ${coherenceStates.EXCLUSIVE}E`, position.x - 40, y);
        });
    ...
    calculateLatencyDistribution(computronium) {
        const cacheState = computronium.cacheState;
        if (!cacheState.latencyHistory)
            return { l1: 0, l2: 0, l3: 0, memory: 0 };
    ...
    determineContext(access) {
        ...
        if (access.type === 'COMBAT_DATA')
            return 'COMBAT';
        if (access.type === 'COMMAND_DATA')
            return 'COMMAND';
        return 'GENERAL';
    }
...
exports.CommandAndControlSystem = CommandAndControlSystem;
// Tactical Options
CommandAndControlSystem.TACTICAL_OPTIONS = {
