...
exports.IntrospectionManager = exports.TalentRendererFactory = exports.FactoryRenderer = exports.CombatUnitRenderer = exports.EngineerRenderer = exports.CommanderRenderer = exports.BaseIntrospectionRenderer = void 0;
const borderLayout_js_1 = require("./borderLayout.js");
// Base introspection renderer - provides common functionality
class BaseIntrospectionRenderer {
    ...
    createWindow(x, y, gameContext) {
        const window = new borderLayout_js_1.BorderLayoutContainer(x, y, this.width, this.height, {
            title: this.getTitle(),
    ...
    addHeader(window) {
        ...
        window.addComponent(new borderLayout_js_1.TextComponent(headerText, {
            backgroundColor: 'rgba(60, 60, 80, 0.8)',
            alignment: 'center',
            fontSize: 11,
            color: '#ffffff'
        }), borderLayout_js_1.BorderRegion.NORTH);
        window.setRegionSize(borderLayout_js_1.BorderRegion.NORTH, 35);
    }
    addMainContent(window, gameContext) {
        const contentText = this.getMainContentText(gameContext);
        window.addComponent(new borderLayout_js_1.TextComponent(contentText, {
    ...
    addFooter(window) {
        ...
        if (footerText) {
            window.addComponent(new borderLayout_js_1.TextComponent(footerText, {
                backgroundColor: 'rgba(70, 60, 50, 0.8)',
                alignment: 'center',
                fontSize: 9,
                color: '#aaaaaa'
            }), borderLayout_js_1.BorderRegion.SOUTH);
            window.setRegionSize(borderLayout_js_1.BorderRegion.SOUTH, 25);
        }
    }
    getHeaderText() {
    ...
    updateContent(window, gameContext) {
        ...
        if (now - this.lastUpdate < this.updateInterval)
            return;
        // Update main content
        const centerComponent = window.regions.get(borderLayout_js_1.BorderRegion.CENTER);
        if (centerComponent) {
            centerComponent.text = this.getMainContentText(gameContext);
        }
        // Update footer
        const southComponent = window.regions.get(borderLayout_js_1.BorderRegion.SOUTH);
        if (southComponent) {
...
exports.BaseIntrospectionRenderer = BaseIntrospectionRenderer;
// Commander-specific renderer
class CommanderRenderer extends BaseIntrospectionRenderer {
    constructor(target, options = {}) {
    ...
    setupLayout(window, gameContext) {
        ...
        window.setRegionSize(borderLayout_js_1.BorderRegion.EAST, 120);
    }
    addAbilitiesPanel(window) {
        const abilities = this.getAbilitiesText();
        window.addComponent(new borderLayout_js_1.TextComponent(abilities, {
...
exports.CommanderRenderer = CommanderRenderer;
// Engineer-specific renderer
class EngineerRenderer extends BaseIntrospectionRenderer {
    constructor(target, options = {}) {
        super(target, options);
        this.width = 350;
        this.height = 280;
    ...
    setupLayout(window, gameContext) {
        ...
        window.setRegionSize(borderLayout_js_1.BorderRegion.WEST, 100);
    }
    addTaskPanel(window, gameContext) {
        const tasks = this.getTasksText(gameContext);
        window.addComponent(new borderLayout_js_1.TextComponent(tasks, {
    ...
    getMainContentText(gameContext) {
        ...
        const nearbyBuildings = gameContext.buildings.filter(b => b.team === this.target.team &&
            b.hp < b.maxHp &&
            Math.sqrt(Math.pow((b.x - this.target.x), 2) + Math.pow((b.y - this.target.y), 2)) < 200);
        ...
        const nearbyNodes = ((_a = gameContext.resourceNodes) === null || _a === void 0 ? void 0 : _a.filter(n => !n.occupied &&
            n.amount > 0 &&
            Math.sqrt(Math.pow((n.x - this.target.x), 2) + Math.pow((n.y - this.target.y), 2)) < 300)) || [];
        if (nearbyNodes.length > 0) {
            content += `\n\n⛏️ BUILD OPPORTUNITIES:\n`;
            nearbyNodes.slice(0, 2).forEach(n => {
                const distance = Math.floor(Math.sqrt(Math.pow((n.x - this.target.x), 2) + Math.pow((n.y - this.target.y), 2)));
...
exports.EngineerRenderer = EngineerRenderer;
// Combat unit renderer
class CombatUnitRenderer extends BaseIntrospectionRenderer {
    constructor(target, options = {}) {
        super(target, options);
        this.width = 320;
        this.height = 270;
    ...
    setupLayout(window, gameContext) {
        ...
        window.setRegionSize(borderLayout_js_1.BorderRegion.EAST, 110);
    }
    addCombatPanel(window) {
        const stats = this.getCombatStatsText();
        window.addComponent(new borderLayout_js_1.TextComponent(stats, {
            backgroundColor: 'rgba(80, 50, 50, 0.8)',
            fontSize: 9,
            padding: 6,
            color: '#ffdddd'
    ...
    getMainContentText(gameContext) {
        ...
        if (this.target.target) {
            const distance = Math.floor(Math.sqrt(Math.pow((this.target.target.x - this.target.x), 2) +
                Math.pow((this.target.target.y - this.target.y), 2)));
...
exports.CombatUnitRenderer = CombatUnitRenderer;
// Factory building renderer
class FactoryRenderer extends BaseIntrospectionRenderer {
    constructor(target, options = {}) {
        super(target, options);
        this.width = 380;
        this.height = 300;
    ...
    addProductionPanel(window, gameContext) {
        ...
        window.addComponent(new borderLayout_js_1.TextComponent(production, {
    ...
    getMainContentText(gameContext) {
        ...
        if (this.target.type.produces) {
            ...
            content += `\nCost: ${((_a = this.target.type.produces.cost) === null || _a === void 0 ? void 0 : _a.mass) || 0}M / ${((_b = this.target.type.produces.cost) === null || _b === void 0 ? void 0 : _b.energy) || 0}E`;
        }
...
exports.FactoryRenderer = FactoryRenderer;
// Talent renderer factory
class TalentRendererFactory {
...
exports.TalentRendererFactory = TalentRendererFactory;
...
exports.IntrospectionManager = IntrospectionManager;
