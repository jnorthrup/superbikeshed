...
exports.CommandStatusRenderer = void 0;
const borderLayout_js_1 = require("./borderLayout.js");
const commandHierarchy_js_1 = require("../ai/commandHierarchy.js");
class CommandStatusRenderer {
    constructor(team, gameContext) {
        ...
        if (!gameContext.commandHierarchies.has(team)) {
            ...
            gameContext.commandHierarchies.set(team, new commandHierarchy_js_1.CommandHierarchy(team));
        }
    ...
    createWindow(x, y) {
        this.window = new borderLayout_js_1.BorderLayoutContainer(x, y, 500, 600, {
    ...
    addCommandSummary() {
        ...
        this.window.addComponent(new borderLayout_js_1.TextComponent(summary, {
        ...
        this.window.setRegionSize(borderLayout_js_1.BorderRegion.NORTH, 60);
    }
    addRankDistribution() {
        const rankInfo = this.getRankDistribution();
        this.window.addComponent(new borderLayout_js_1.TextComponent(rankInfo, {
            backgroundColor: this.team === 'blue' ? 'rgba(50, 40, 70, 0.9)' : 'rgba(70, 40, 50, 0.9)',
            fontSize: 9,
            color: this.team === 'blue' ? '#ddddff' : '#ffdddd',
            padding: 6
        }), borderLayout_js_1.BorderRegion.WEST);
        this.window.setRegionSize(borderLayout_js_1.BorderRegion.WEST, 140);
    }
    addMissionGroups() {
        const groupInfo = this.getMissionGroupsInfo();
        this.window.addComponent(new borderLayout_js_1.TextComponent(groupInfo, {
    ...
    addUnitStatus() {
        ...
        this.window.addComponent(new borderLayout_js_1.TextComponent(unitStatus, {
        ...
        this.window.setRegionSize(borderLayout_js_1.BorderRegion.EAST, 120);
    }
    addActivityLog() {
        const activity = this.getActivityLog();
        this.window.addComponent(new borderLayout_js_1.TextComponent(activity, {
            backgroundColor: 'rgba(50, 50, 30, 0.9)',
            fontSize: 8,
            color: '#ffffcc',
            padding: 6
        }), borderLayout_js_1.BorderRegion.SOUTH);
        this.window.setRegionSize(borderLayout_js_1.BorderRegion.SOUTH, 80);
    }
    getCommandSummary() {
    ...
    getActivityLog() {
        ...
        const relevantEvents = events
            .filter(event => event.message.toLowerCase().includes(this.team) ||
    ...
    updateContent() {
        ...
        const northComponent = this.window.regions.get(borderLayout_js_1.BorderRegion.NORTH);
        if (northComponent) {
            northComponent.text = this.getCommandSummary();
        }
        const westComponent = this.window.regions.get(borderLayout_js_1.BorderRegion.WEST);
        if (westComponent) {
            westComponent.text = this.getRankDistribution();
        }
        const centerComponent = this.window.regions.get(borderLayout_js_1.BorderRegion.CENTER);
        if (centerComponent) {
            centerComponent.text = this.getMissionGroupsInfo();
        }
        const eastComponent = this.window.regions.get(borderLayout_js_1.BorderRegion.EAST);
        if (eastComponent) {
            eastComponent.text = this.getUnitStatusInfo();
        }
        const southComponent = this.window.regions.get(borderLayout_js_1.BorderRegion.SOUTH);
        if (southComponent) {
...
exports.CommandStatusRenderer = CommandStatusRenderer;
