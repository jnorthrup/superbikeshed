...
const gameConstants_js_1 = require("../config/gameConstants.js"); // Assuming these are available and correctly pathed
class PathNode {
...
function isTraversable(gridX, gridY, gameContext, unitMovementType) {
    ...
    if (unitMovementType === 'land') {
        const isWater = terrainTypeAtNode === gameConstants_js_1.TERRAIN_TYPES.WATER;
        const isMountain = terrainTypeAtNode === gameConstants_js_1.TERRAIN_TYPES.MOUNTAIN;
...
function retracePath(startNode, endNode) {
    ...
    return path.map(node => ({
        x: node.x * gameConstants_js_1.TILE_SIZE + gameConstants_js_1.TILE_SIZE / 2,
        y: node.y * gameConstants_js_1.TILE_SIZE + gameConstants_js_1.TILE_SIZE / 2
    }));
}
function findPath(startCoords, endCoords, gameContext, unitMovementType = 'land') {
    // Convert world coordinates to grid coordinates
    const startGridX = Math.floor(startCoords.x / gameConstants_js_1.TILE_SIZE);
    const startGridY = Math.floor(startCoords.y / gameConstants_js_1.TILE_SIZE);
    const endGridX = Math.floor(endCoords.x / gameConstants_js_1.TILE_SIZE);
    const endGridY = Math.floor(endCoords.y / gameConstants_js_1.TILE_SIZE);
    ...
    const MAX_ITERATIONS = gameConstants_js_1.GRID_SIZE * gameConstants_js_1.GRID_SIZE * 2; // Safety break for performance
