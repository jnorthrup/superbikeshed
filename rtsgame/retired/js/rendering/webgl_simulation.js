...
const webgl_2d_renderer_js_1 = require("./webgl_2d_renderer.js");
const gameConstants_js_1 = require("../config/gameConstants.js");
const canvas = document.getElementById('game-canvas');
const renderer = new webgl_2d_renderer_js_1.WebGL2DRenderer(canvas);
...
function initializeTerrain(size) {
    ...
    for (let x = 0; x < size; x++) {
        ...
        for (let y = 0; y < size; y++) {
            ...
            if (rand < 0.2) {
                terrain[x][y] = gameConstants_js_1.TERRAIN_TYPES.WATER; // 0
            }
            else if (rand < 0.4) {
                terrain[x][y] = gameConstants_js_1.TERRAIN_TYPES.MOUNTAIN; // 2
            }
            else {
                terrain[x][y] = gameConstants_js_1.TERRAIN_TYPES.LAND; // 1
            }
        }
    }
    return terrain;
...
function updateBot(bot, terrain, bots, time, events) {
    ...
    if (actionChance < 0.3 && distToOther < 3) {
        ...
        if (direction === 0 && bot.y > 0)
            newY--;
        else if (direction === 1 && bot.x < terrain.length - 1)
            newX++;
        else if (direction === 2 && bot.y < terrain.length - 1)
            newY++;
        else if (direction === 3 && bot.x > 0)
            newX--;
        if (newX >= 0 && newX < gameConstants_js_1.GRID_SIZE && newY >= 0 && newY < gameConstants_js_1.GRID_SIZE && terrain[newX][newY] !== gameConstants_js_1.TERRAIN_TYPES.WATER) { // Use numerical WATER type
            bot.x = newX;
            bot.y = newY;
            bot.action = 'move';
            events.push({
...
function simulateMatch(duration) {
    let gameTime = 0;
    const events = [];
    const terrain = initializeTerrain(gameConstants_js_1.GRID_SIZE);
    const bots = [
    ...
    function renderScene(terrain, bots) {
        ...
        for (let x = 0; x < gameConstants_js_1.GRID_SIZE; x++) {
            for (let y = 0; y < gameConstants_js_1.GRID_SIZE; y++) {
                ...
                renderer.drawRect(x * gameConstants_js_1.TILE_SIZE, y * gameConstants_js_1.TILE_SIZE, gameConstants_js_1.TILE_SIZE, gameConstants_js_1.TILE_SIZE, color);
            }
        }
        // Render bots (as squares for simplicity in 2D WebGL)
        bots.forEach(bot => {
            const botSize = gameConstants_js_1.TILE_SIZE * 0.6; // Make bot a bit smaller than a tile
