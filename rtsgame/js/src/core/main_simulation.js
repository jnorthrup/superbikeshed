...
const webgl_2d_renderer_js_1 = require("../rendering/webgl_2d_renderer.js");
const webglRenderer_js_1 = require("../rendering/webglRenderer.js");
...
const TERRAIN_COLORS = {
    [gameConstants_js_1.TERRAIN_TYPES.WATER]: [0.0, 0.2, 0.5, 1.0], // Blue
    [gameConstants_js_1.TERRAIN_TYPES.LAND]: [0.2, 0.6, 0.2, 1.0], // Green
    [gameConstants_js_1.TERRAIN_TYPES.MOUNTAIN]: [0.4, 0.4, 0.4, 1.0] // Gray
};
const TEAM_COLORS = {
...
function initializeTerrain(size) {
    ...
    for (let x = 0; x < size; x++) {
        ...
        for (let y = 0; y < size; y++) {
            ...
            if (resourceMap[x][y]) {
                newTerrain[x][y] = {
                    type: gameConstants_js_1.TERRAIN_TYPES.RESOURCE,
                    elevation: elevation,
                    hasResource: true
                };
            }
                ...
                newTerrain[x][y] = {
                    type: gameConstants_js_1.TERRAIN_TYPES.WATER,
                    elevation: elevation,
                    hasResource: false
                };
            }
                ...
                newTerrain[x][y] = {
                    type: gameConstants_js_1.TERRAIN_TYPES.MOUNTAIN,
                    elevation: elevation,
                    hasResource: false
                };
            }
                ...
                newTerrain[x][y] = {
                    type: gameConstants_js_1.TERRAIN_TYPES.LAND,
                    elevation: elevation,
                    hasResource: false
                };
            }
...
function updateBot(bot, currentTerrain, allBots, time, eventLog) {
    ...
    if (actionChance < 0.3 && distToOther < 3) {
        ...
        let newX = bot.x;
        let newY = bot.y;
        if (direction === 0 && bot.y > 0)
            newY--;
        else if (direction === 1 && bot.x < currentTerrain.length - 1)
            newX++;
        else if (direction === 2 && bot.y < currentTerrain.length - 1)
            newY++;
        else if (direction === 3 && bot.x > 0)
            newX--;
        if (newX >= 0 && newX < gameConstants_js_1.GRID_SIZE && newY >= 0 && newY < gameConstants_js_1.GRID_SIZE && currentTerrain[newX][newY].type !== gameConstants_js_1.TERRAIN_TYPES.WATER) {
            bot.x = newX;
            bot.y = newY;
            bot.action = 'move';
            eventLog.push({
    ...
    if (rendererType === 'svg') {
        ...
        for (let x = 0; x < gameConstants_js_1.GRID_SIZE; x++) {
            for (let y = 0; y < gameConstants_js_1.GRID_SIZE; y++) {
                ...
                rect.setAttribute('x', x * gameConstants_js_1.TILE_SIZE);
                rect.setAttribute('y', y * gameConstants_js_1.TILE_SIZE);
                rect.setAttribute('width', gameConstants_js_1.TILE_SIZE);
                rect.setAttribute('height', gameConstants_js_1.TILE_SIZE);
                ...
                switch (currentTerrain[x][y].type) {
                    ...
                    case gameConstants_js_1.TERRAIN_TYPES.LAND:
                        rect.classList.add('land');
                        break;
                    case gameConstants_js_1.TERRAIN_TYPES.MOUNTAIN:
                        rect.classList.add('mountain');
                        break;
                    case gameConstants_js_1.TERRAIN_TYPES.RESOURCE:
                        rect.classList.add('resource');
                        break;
                }
        ...
        currentBots.forEach(bot => {
            ...
            circle.setAttribute('cx', bot.x * gameConstants_js_1.TILE_SIZE + gameConstants_js_1.TILE_SIZE / 2);
            circle.setAttribute('cy', bot.y * gameConstants_js_1.TILE_SIZE + gameConstants_js_1.TILE_SIZE / 2);
            circle.setAttribute('r', gameConstants_js_1.TILE_SIZE / 3);
        ...
        for (let x = 0; x < gameConstants_js_1.GRID_SIZE; x++) {
            for (let y = 0; y < gameConstants_js_1.GRID_SIZE; y++) {
                ...
                switch (terrainType) {
                    case gameConstants_js_1.TERRAIN_TYPES.WATER:
                        color = `rgba(0, ${Math.floor(51 + elevation * 50)}, 128, 1.0)`;
                        break; // Varying blue depth
                    case gameConstants_js_1.TERRAIN_TYPES.LAND:
                        color = `rgba(${Math.floor(51 + elevation * 100)}, 153, 51, 1.0)`;
                        break; // Varying green height
                    case gameConstants_js_1.TERRAIN_TYPES.MOUNTAIN:
                        color = `rgba(${Math.floor(102 + elevation * 100)}, ${Math.floor(102 + elevation * 100)}, ${Math.floor(102 + elevation * 100)}, 1.0)`;
                        break; // Varying gray
                    case gameConstants_js_1.TERRAIN_TYPES.RESOURCE:
                        color = 'rgba(255, 215, 0, 1.0)';
                        break; // Gold for resources
                    default: color = 'rgba(0, 0, 0, 1.0)'; // Black
                }
                ctx.fillStyle = color;
                ctx.fillRect(x * gameConstants_js_1.TILE_SIZE, y * gameConstants_js_1.TILE_SIZE, gameConstants_js_1.TILE_SIZE, gameConstants_js_1.TILE_SIZE);
            }
        }
        // Render bots
        currentBots.forEach(bot => {
            const botSize = gameConstants_js_1.TILE_SIZE * 0.6;
            const botColor = bot.team === 'blue' ? 'rgba(25, 25, 204, 1.0)' : 'rgba(204, 25, 25, 1.0)'; // Darker colors
            const centerX = bot.x * gameConstants_js_1.TILE_SIZE + gameConstants_js_1.TILE_SIZE / 2;
            const centerY = bot.y * gameConstants_js_1.TILE_SIZE + gameConstants_js_1.TILE_SIZE / 2;
        ...
        for (let x = 0; x < gameConstants_js_1.GRID_SIZE; x++) {
            for (let y = 0; y < gameConstants_js_1.GRID_SIZE; y++) {
                ...
                currentRenderer.drawRect(x * gameConstants_js_1.TILE_SIZE, y * gameConstants_js_1.TILE_SIZE, gameConstants_js_1.TILE_SIZE, gameConstants_js_1.TILE_SIZE, color);
            }
        }
...
function startSimulation() {
    ...
    if (simulationRandom.seed <= 0)
        simulationRandom.seed += 2147483646;
    gameTime = 0;
    events = [];
    terrain = initializeTerrain(gameConstants_js_1.GRID_SIZE);
    bots = [
    ...
    if (selectedRenderer === 'svg') {
        ...
        if (selectedRenderer === 'webgl2d') {
            currentRenderer = new webgl_2d_renderer_js_1.WebGL2DRenderer(gameCanvas);
        }
        else if (selectedRenderer === 'webgl3d') {
            currentRenderer = new webglRenderer_js_1.WebGLRenderer(gameCanvas);
...
function processKeyboardInput() {
    ...
    // WASD movement
    if (keys.w)
        camera.targetY -= moveAmount; // Move up
    if (keys.s)
        camera.targetY += moveAmount; // Move down
    if (keys.a)
        camera.targetX -= moveAmount; // Move left
    if (keys.d)
        camera.targetX += moveAmount; // Move right
...
function renderTraitOverlays() {
    if (camera.zoom > 8.0) { // Show traits when zoomed in close
        ...
        bots.forEach(bot => {
            ...
            const screenX = (bot.x * gameConstants_js_1.TILE_SIZE - camera.x) * camera.zoom + gameCanvas.width / 2;
            const screenY = (bot.y * gameConstants_js_1.TILE_SIZE - camera.y) * camera.zoom + gameCanvas.height / 2;
