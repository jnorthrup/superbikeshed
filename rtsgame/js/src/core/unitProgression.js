...
exports.UnitProgression = void 0;
...
class UnitProgression {
    constructor(unit) {
        ...
        // Command fitness
        this.commandFitness = gameConstants_js_1.COMMAND_FITNESS_LEVELS.FULL_COMMAND;
        this.lastAuthorityUpdate = 0;
        this.commandSuccesses = 0;
        this.commandFailures = 0;
        // Veterancy
        this.veterancyLevel = gameConstants_js_1.VETERANCY_LEVELS.GREEN;
        this.experienceThresholds = {
            [gameConstants_js_1.VETERANCY_LEVELS.GREEN]: 0,
            [gameConstants_js_1.VETERANCY_LEVELS.REGULAR]: 25,
            [gameConstants_js_1.VETERANCY_LEVELS.VETERAN]: 75,
            [gameConstants_js_1.VETERANCY_LEVELS.ELITE]: 150,
            [gameConstants_js_1.VETERANCY_LEVELS.HERO]: 300
        };
    }
    /**
    ...
    isUnderFire(gameContext) {
        ...
        const nearbyEnemies = gameContext.units.filter(u => u && u.team !== this.unit.team &&
            this.unit.getDistance && this.unit.getDistance(u) < 200);
        return nearbyEnemies.some(enemy => enemy.target === this.unit ||
    ...
    updateHealthAuthority(healthRatio) {
        if (healthRatio >= 0.8) {
            ...
            this.commandFitness = gameConstants_js_1.COMMAND_FITNESS_LEVELS.REDUCED_AUTHORITY;
        }
        else if (healthRatio >= 0.4) {
            this.healthAuthorityModifier = -2;
            this.commandFitness = gameConstants_js_1.COMMAND_FITNESS_LEVELS.COMPROMISED_COMMAND;
        }
        else if (healthRatio >= 0.2) {
            this.healthAuthorityModifier = -5;
            this.commandFitness = gameConstants_js_1.COMMAND_FITNESS_LEVELS.CRITICAL_STATUS;
        }
        else {
            this.healthAuthorityModifier = -10;
            this.commandFitness = gameConstants_js_1.COMMAND_FITNESS_LEVELS.COMBAT_INEFFECTIVE;
        }
    }
    /**
    ...
    updateVeterancyAuthority() {
        ...
        if (experiencePoints >= this.experienceThresholds[gameConstants_js_1.VETERANCY_LEVELS.HERO]) {
            this.veterancyLevel = gameConstants_js_1.VETERANCY_LEVELS.HERO;
            this.veterancyAuthorityModifier = 15;
        }
        else if (experiencePoints >= this.experienceThresholds[gameConstants_js_1.VETERANCY_LEVELS.ELITE]) {
            this.veterancyLevel = gameConstants_js_1.VETERANCY_LEVELS.ELITE;
            this.veterancyAuthorityModifier = 10;
        }
        else if (experiencePoints >= this.experienceThresholds[gameConstants_js_1.VETERANCY_LEVELS.VETERAN]) {
            this.veterancyLevel = gameConstants_js_1.VETERANCY_LEVELS.VETERAN;
            this.veterancyAuthorityModifier = 5;
        }
        else if (experiencePoints >= this.experienceThresholds[gameConstants_js_1.VETERANCY_LEVELS.REGULAR]) {
            this.veterancyLevel = gameConstants_js_1.VETERANCY_LEVELS.REGULAR;
    ...
    processPromotion(oldLevel) {
        ...
        switch (this.veterancyLevel) {
            case gameConstants_js_1.VETERANCY_LEVELS.REGULAR:
                this.unit.maxHp *= 1.1;
                this.unit.hp = this.unit.maxHp;
                break;
            case gameConstants_js_1.VETERANCY_LEVELS.VETERAN:
                this.unit.maxHp *= 1.2;
                this.unit.hp = this.unit.maxHp;
                this.unit.attackDamage *= 1.15;
                break;
            case gameConstants_js_1.VETERANCY_LEVELS.ELITE:
                this.unit.maxHp *= 1.3;
                this.unit.hp = this.unit.maxHp;
                this.unit.attackDamage *= 1.25;
                this.unit.attackRange *= 1.1;
                break;
            case gameConstants_js_1.VETERANCY_LEVELS.HERO:
                this.unit.maxHp *= 1.5;
                this.unit.hp = this.unit.maxHp;
    ...
    recordKill(target) {
        ...
        else if (target.veterancyLevel === gameConstants_js_1.VETERANCY_LEVELS.HERO) {
            this.addCombatExperience(5);
        }
        else if (target.veterancyLevel === gameConstants_js_1.VETERANCY_LEVELS.ELITE) {
...
exports.UnitProgression = UnitProgression;
