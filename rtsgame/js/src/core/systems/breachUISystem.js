    ...
    getBreachEffects(breach) {
        ...
        switch (breach.type) {
            case 'command_injection':
                effects.push({ name: 'Command Latency', value: `+${Math.round(breach.severity * 100)}%` }, { name: 'Command Accuracy', value: `-${Math.round(breach.severity * 50)}%` });
                break;
            case 'data_exfiltration':
                effects.push({ name: 'Network Security', value: `-${Math.round(breach.severity * 70)}%` }, { name: 'Intel Leak Rate', value: `${Math.round(breach.severity * 50)}%` });
                break;
            case 'network_disruption':
                effects.push({ name: 'Network Stability', value: `-${Math.round(breach.severity * 80)}%` }, { name: 'Packet Loss', value: `${Math.round(breach.severity * 60)}%` });
                break;
            case 'core_compromise':
                effects.push({ name: 'Core Efficiency', value: `-${Math.round(breach.severity * 90)}%` }, { name: 'PoW Contribution', value: `-${Math.round(breach.severity * 70)}%` });
                break;
        }
        return effects;
    }
...
exports.BreachUISystem = BreachUISystem;
