...
class ResourceSystem {
    ...
    updateNetworkConnections(entity, gameContext) {
        if (!entity.resource.connectedTo)
            return;
    ...
    transferResources(source, target, gameContext) {
        ...
        if (!source.resource.isActive || !target.resource.isActive)
            return;
        // Calculate transfer amount based on source generation and target capacity
        const transferAmount = Math.min(source.resource.generationRate * gameContext.deltaTime, target.resource.capacity - target.resource.currentAmount);
        ...
        if (gameContext.factions) {
            for (const faction of Object.values(gameContext.factions)) {
                ...
                for (const [resourceType, amount] of Object.entries(faction.resources)) {
                    ...
                    const income = ((_a = faction.income) === null || _a === void 0 ? void 0 : _a[resourceType]) || 0;
                    const consumption = ((_b = faction.consumption) === null || _b === void 0 ? void 0 : _b[resourceType]) || 0;
    ...
    findEntityById(gameContext, id) {
        var _a, _b, _c;
        // Search in units
        const unit = (_a = gameContext.units) === null || _a === void 0 ? void 0 : _a.find(u => u.id === id);
        if (unit)
            return unit;
        // Search in buildings
        const building = (_b = gameContext.buildings) === null || _b === void 0 ? void 0 : _b.find(b => b.id === id);
        if (building)
            return building;
        // Search in resource nodes
        const node = (_c = gameContext.resourceNodes) === null || _c === void 0 ? void 0 : _c.find(n => n.id === id);
        if (node)
            return node;
        return null;
    }
    ...
    getGlobalResourceStatus(gameContext, factionId) {
        var _a;
        const faction = (_a = gameContext.factions) === null || _a === void 0 ? void 0 : _a[factionId];
        if (!faction)
            return null;
        return {
