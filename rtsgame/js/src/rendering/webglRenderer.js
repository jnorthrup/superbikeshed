 ...
 */
const gl_matrix_1 = require("gl-matrix"); // Import gl-matrix
const gameConstants_js_1 = require("../config/gameConstants.js");
const modelDefaults_js_1 = require("../config/modelDefaults.js");
...
class WebGLRenderer {
    ...
    initializeModels() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            for (const [key, modelConfig] of Object.entries(modelDefaults_js_1.UNIT_MODELS)) {
                const model = {
                ...
                loadPromises.push(this.loadModel(modelConfig.modelPath)
                    .then(geometry => {
            ...
            for (const [key, modelConfig] of Object.entries(modelDefaults_js_1.BUILDING_MODELS)) {
                const model = {
                    modelPath: modelConfig.modelPath,
                    scale: modelConfig.scale,
                    size: modelConfig.size,
            ...
            for (const [key, modelConfig] of Object.entries(modelDefaults_js_1.TERRAIN_MODELS)) {
                const model = {
                    modelPath: modelConfig.modelPath,
                    scale: modelConfig.scale,
                    size: modelConfig.size,
    ...
    loadModel(modelPath) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                const response = yield fetch(modelPath);
                if (!response.ok) {
    ...
    parseOBJ(objText) {
        ...
        for (const face of faces) {
            ...
            if (face.length === 3) { // Triangle
                indices.push(getOrAddVertex(face[0][0], face[0][1], face[0][2]), getOrAddVertex(face[1][0], face[1][1], face[1][2]), getOrAddVertex(face[2][0], face[2][1], face[2][2]));
            }
    ...
    render(simulation, gameContext) {
        ...
        let terrainData = simulation.terrain || ((_a = simulation.gameContext) === null || _a === void 0 ? void 0 : _a.terrain) || gameContext.terrain;
    ...
    setupMatrices() {
        ...
        const zNear = 1.0;
        const zFar = 50000;
        gl_matrix_1.mat4.perspective(projectionMatrix, fieldOfViewY, aspect, zNear, zFar);
        ...
        const eye = gl_matrix_1.vec3.fromValues(eyeX, eyeY, eyeZ);
        const center = gl_matrix_1.vec3.fromValues(cam.x, cam.y, 0); // Look at target point
        const up = gl_matrix_1.vec3.fromValues(0, 0, 1); // Z is up
        gl_matrix_1.mat4.lookAt(viewMatrix, eye, center, up);
        ...
        const globalNormalMatrix = gl_matrix_1.mat4.create();
        gl_matrix_1.mat4.invert(globalNormalMatrix, viewMatrix);
        gl_matrix_1.mat4.transpose(globalNormalMatrix, globalNormalMatrix);
    ...
    renderTerrain(terrainData, left, right, top, bottom, viewMatrix) {
        ...
        const startX = 0;
        const endX = gameConstants_js_1.GRID_SIZE;
        const startY = 0;
        const endY = gameConstants_js_1.GRID_SIZE;
        let vertices = [];
        let indices = [];
        let colors = [];
        let texCoords = [];
        ...
        const getHeight = (x, y) => {
            if (x < 0 || x >= gameConstants_js_1.GRID_SIZE || y < 0 || y >= gameConstants_js_1.GRID_SIZE) {
        ...
        const calculateNormal = (gx, gy) => {
            ...
            const normalUnnormalized = gl_matrix_1.vec3.fromValues(dX, dY, gameConstants_js_1.TILE_SIZE * 2); // Z-up normal
            const normal = gl_matrix_1.vec3.create();
            gl_matrix_1.vec3.normalize(normal, normalUnnormalized);
            return normal;
        };
        let indexOffset = 0;
        for (let x = startX; x < endX; x++) {
            for (let y = startY; y < endY; y++) {
                ...
                if (terrainData[x][y].type === gameConstants_js_1.TERRAIN_TYPES.RESOURCE) {
                    continue;
                }
                const posX = x * gameConstants_js_1.TILE_SIZE;
                const posY = y * gameConstants_js_1.TILE_SIZE;
                const size = gameConstants_js_1.TILE_SIZE;
                ...
                if (elevation < modelDefaults_js_1.SEA_LEVEL) {
                    terrainType = gameConstants_js_1.TERRAIN_TYPES.WATER;
                }
                let color;
                switch (terrainType) {
                    case gameConstants_js_1.TERRAIN_TYPES.WATER:
                        // Water colors - deeper blues for lower elevations
                        const waterDepth = modelDefaults_js_1.SEA_LEVEL - elevation;
                        ...
                        break;
                    case gameConstants_js_1.TERRAIN_TYPES.LAND:
                        // Land colors - greener for higher elevations
                        const landHeight = elevation - modelDefaults_js_1.SEA_LEVEL;
                        color = [0.2 + landHeight * 0.2, 0.6 + landHeight * 0.1, 0.2 + landHeight * 0.2, 1.0];
                        break;
                    case gameConstants_js_1.TERRAIN_TYPES.MOUNTAIN:
                        // Mountain colors - grayer for higher elevations
                        const mountainHeight = elevation - modelDefaults_js_1.SEA_LEVEL;
    ...
    renderTerrainModels(terrain, left, right, top, bottom, viewMatrix) {
        ...
        for (let x = 0; x < gameConstants_js_1.GRID_SIZE; x++) {
            for (let y = 0; y < gameConstants_js_1.GRID_SIZE; y++) {
                if (terrain[x] && terrain[x][y].type === gameConstants_js_1.TERRAIN_TYPES.RESOURCE) { // Changed to .type
                    ...
                    const objWorldX = x * gameConstants_js_1.TILE_SIZE;
                    const objWorldY = y * gameConstants_js_1.TILE_SIZE;
                    ...
                    let modelMatrix = gl_matrix_1.mat4.create();
                    // Create translation matrix
                    const translateVec = gl_matrix_1.vec3.fromValues(objWorldX, objWorldY, modelZ);
                    gl_matrix_1.mat4.fromTranslation(modelMatrix, translateVec);
                    // Create scaling matrix and apply it
                    const scaleVec = gl_matrix_1.vec3.fromValues(scaleFactor, scaleFactor, scaleFactor);
                    gl_matrix_1.mat4.scale(modelMatrix, modelMatrix, scaleVec); // Scale existing modelMatrix
                    let modelViewMatrix = gl_matrix_1.mat4.create();
                    gl_matrix_1.mat4.multiply(modelViewMatrix, viewMatrix, modelMatrix);
                    this.gl.uniformMatrix4fv(this.uniforms.modelViewMatrix, false, modelViewMatrix);
                    const normalMatrixRender = gl_matrix_1.mat4.create(); // Renamed to avoid conflict with 'normalMatrix' in parent scope
                    gl_matrix_1.mat4.invert(normalMatrixRender, modelViewMatrix);
                    gl_matrix_1.mat4.transpose(normalMatrixRender, normalMatrixRender);
    ...
    renderEntities(renderContext, left, right, top, bottom, viewMatrix) {
        ...
        const isVisible = (obj, modelSize) => {
            ...
            const objWorldX = obj.x * gameConstants_js_1.TILE_SIZE;
            const objWorldY = obj.y * gameConstants_js_1.TILE_SIZE;
        ...
        for (const [modelKey, modelInfo] of Object.entries(this.models.units)) {
            ...
            renderContext.units.forEach(unit => {
                let unitModelKey = Object.keys(modelDefaults_js_1.UNIT_MODELS).find(key => modelDefaults_js_1.UNIT_MODELS[key].name === unit.type.name);
                if (!unitModelKey)
                    unitModelKey = 'scout'; // Default fallback
                ...
                if (unitModelKey !== modelKey)
                    return; // Only render units of the current model type
                const effectiveSize = modelInfo.size * modelInfo.scale;
                ...
                const translationMatrix = [
                    1, 0, 0, unit.x * gameConstants_js_1.TILE_SIZE,
                    0, 1, 0, unit.y * gameConstants_js_1.TILE_SIZE,
                    0, 0, 1, posZ,
                    0, 0, 0, 1
                ];
                // Scaling matrix
                ...
                // Combine with view matrix
                let modelMatrix = gl_matrix_1.mat4.create();
                const translateVecUnit = gl_matrix_1.vec3.fromValues(unit.x * gameConstants_js_1.TILE_SIZE, unit.y * gameConstants_js_1.TILE_SIZE, posZ);
                gl_matrix_1.mat4.fromTranslation(modelMatrix, translateVecUnit);
                ...
                const scaleVecUnit = gl_matrix_1.vec3.fromValues(unitScale, unitScale, unitScale);
                gl_matrix_1.mat4.scale(modelMatrix, modelMatrix, scaleVecUnit);
                let modelViewMatrix = gl_matrix_1.mat4.create();
                gl_matrix_1.mat4.multiply(modelViewMatrix, viewMatrix, modelMatrix);
                this.gl.uniformMatrix4fv(this.uniforms.modelViewMatrix, false, modelViewMatrix);
                const normalMatrixRender = gl_matrix_1.mat4.create();
                gl_matrix_1.mat4.invert(normalMatrixRender, modelViewMatrix);
                gl_matrix_1.mat4.transpose(normalMatrixRender, normalMatrixRender);
                this.gl.uniformMatrix4fv(this.uniforms.normalMatrix, false, normalMatrixRender);
                this.gl.drawElements(this.gl.TRIANGLES, geometry.indices.length, this.gl.UNSIGNED_SHORT, 0);
        ...
        for (const [modelKey, modelInfo] of Object.entries(this.models.buildings)) {
            ...
            renderContext.buildings.forEach(building => {
                let buildingModelKey = Object.keys(modelDefaults_js_1.BUILDING_MODELS).find(key => modelDefaults_js_1.BUILDING_MODELS[key].name === building.type.name);
                if (!buildingModelKey)
                    buildingModelKey = 'landFactory'; // Default fallback
                if (buildingModelKey !== modelKey)
                    return;
                const effectiveSize = modelInfo.size * modelInfo.scale;
                if (!isVisible(building, effectiveSize))
                    return;
                ...
                const translationMatrix = [
                    1, 0, 0, building.x * gameConstants_js_1.TILE_SIZE,
                    0, 1, 0, building.y * gameConstants_js_1.TILE_SIZE,
                    0, 0, 1, posZ,
                    0, 0, 0, 1
                ];
                // Scaling matrix
                ...
                // Combine with view matrix
                let modelMatrix = gl_matrix_1.mat4.create();
                const translateVecBuilding = gl_matrix_1.vec3.fromValues(building.x * gameConstants_js_1.TILE_SIZE, building.y * gameConstants_js_1.TILE_SIZE, posZ);
                gl_matrix_1.mat4.fromTranslation(modelMatrix, translateVecBuilding);
                // TODO: Add building rotation if applicable
                const scaleVecBuilding = gl_matrix_1.vec3.fromValues(buildingScale, buildingScale, buildingScale);
                gl_matrix_1.mat4.scale(modelMatrix, modelMatrix, scaleVecBuilding);
                let modelViewMatrix = gl_matrix_1.mat4.create();
                gl_matrix_1.mat4.multiply(modelViewMatrix, viewMatrix, modelMatrix);
                this.gl.uniformMatrix4fv(this.uniforms.modelViewMatrix, false, modelViewMatrix);
                const normalMatrixRender = gl_matrix_1.mat4.create();
