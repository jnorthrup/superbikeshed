export class ComputroniumSystem {
    ...
    collectFunctionRequests(entity, focusMode, requestedForks) {
        ...
        requestedForks.push({ // All units typically have basic sensors
    ...
    applyPerformanceModifiers(entity, deltaTime) {
        ...
        if(entity.sensorSystemData) { // Assuming a specific data object for basic sensor performance
            ...
            if (sensorStatus === 'DEGRADED') entity.sensorSystemData.rangeFactor = 0.7;
            else if (sensorStatus === 'STARVED') entity.sensorSystemData.rangeFactor = 0.4;
        }

        // Apply modifiers for new functions
        const constructionStatus = core.allocatedFunctions['CONSTRUCTION_SYSTEMS'];
        if (constructionStatus === 'DEGRADED') entity.constructionRateFactor = 0.6;
        else if (constructionStatus === 'STARVED') entity.constructionRateFactor = 0.3;

        const repairStatus = core.allocatedFunctions['REPAIR_SYSTEMS'];
        if (repairStatus === 'DEGRADED') entity.repairRateFactor = 0.6;
        else if (repairStatus === 'STARVED') entity.repairRateFactor = 0.3;
    ...
    setCoreFocusMode(entity, modeKey) { // modeKey should be like 'OFFENSIVE', 'DEFENSIVE'
