...
import { CommandBlocks, APMComposer, BlockLayout } from './TacticsDSL.js';

export class VisualTacticsEditor {
    constructor(canvasElement, gameState) {
        this.canvas = canvasElement;
        this.ctx = this.canvas.getContext('2d');
        this.gameState = gameState;
        this.composer = new APMComposer();
    ...
    attemptAutoConnection(newBlock) {
        ...
        for (const [id, existingBlock] of this.blocks) {
            if (id === newBlock.id) continue;
            
            const distance = Math.sqrt(
                Math.pow(newBlock.x - existingBlock.x, 2) + 
    ...
    render() {
        ...
        // Render overlays
        if (this.hotKeyMode) this.renderHotkeyOverlay();
        if (this.quickCommandMode) this.renderQuickCommandPanel();
    ...
    createConnection(fromBlockId, toBlockId) {
        ...
        if (!existing) {
            ...
            if (fromBlock) fromBlock.connections.outputs.push(toBlockId);
            if (toBlock) toBlock.connections.inputs.push(fromBlockId);
    ...
    exportProgram() {
        ...
        const rootBlocks = Array.from(this.blocks.values()).filter(
            block => block.connections.inputs.length === 0
        );
    ...
    blockToExpression(block) {
        ...
        return new CommandBlocks[block.type]();
    }
...
class APMCounter {
    ...
    getEfficiency() {
        ...
        if (this.actions.length < 2) return 1.0;
    ...
    render(ctx, blocks, connections) {
        ...
        for (const connection of connections) {
            ...
            if (fromBlock && toBlock) {
                ctx.beginPath();
                ctx.moveTo(
                    this.x + fromBlock.x * scaleX,
                    this.y + fromBlock.y * scaleY
                );
                ctx.lineTo(
                    this.x + toBlock.x * scaleX,
                    this.y + toBlock.y * scaleY
                );
                ctx.stroke();
            }
        }
