...
class WebGPUOptimizedRenderer {
    ...
    private lateinit var instanceBuffer: GPUBuffer
    private lateinit var indirectDrawBuffer: GPUBuffer
    private lateinit var cullingBuffer: GPUBuffer
    
    // Render pipelines
    private lateinit var unitPipeline: GPURenderPipeline
    private lateinit var buildingPipeline: GPURenderPipeline
    private lateinit var terrainPipeline: GPURenderPipeline
    private lateinit var particlePipeline: GPURenderPipeline
    
    // Compute pipelines
    private lateinit var cullingPipeline: GPUComputePipeline
    private lateinit var frustumCullingPipeline: GPUComputePipeline
    private lateinit var lodPipeline: GPUComputePipeline
    
    // Instance data
    private val maxInstances = 65536
    private val instanceData = FloatArray(maxInstances * INSTANCE_STRIDE)
    ...
    fun render(world: ECSWorld, deltaTime: Float) {
        ...
        // Update metrics
        gpuTime = (System.nanoTime() - startTime) / 1_000_000f
    }
    
    private fun prepareInstanceData(world: ECSWorld) {
        instanceCount = 0
        drawCalls = 0
    ...
    private fun fillInstanceData(index: Int, instance: InstanceData) {
    ...
    private fun performGPUCulling(commandEncoder: GPUCommandEncoder) {
    ...
    private fun drawUnits(renderPass: GPURenderPassEncoder) {
    ...
    private fun drawTerrain(renderPass: GPURenderPassEncoder) {
        // Terrain rendering with LOD
        renderPass.setPipeline(terrainPipeline)
        // Implementation...
    }
    
    private fun drawBuildings(renderPass: GPURenderPassEncoder, world: ECSWorld) {
        // Building rendering
        renderPass.setPipeline(buildingPipeline)
        // Implementation...
    }
    
    private fun drawParticles(renderPass: GPURenderPassEncoder) {
        // Particle rendering
    ...
    private fun getTeamColor(teamId: Int): Color {
        return when (teamId) {
