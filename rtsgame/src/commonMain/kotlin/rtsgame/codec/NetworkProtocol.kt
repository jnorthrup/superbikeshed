...
class ConnectionState(
    ...
    private val sentMessages = mutableMapOf<Long, Long>() // messageId -> timestamp
    private val receivedAcks = mutableSetOf<Long>()
    
    fun trackSentMessage(messageId: Long) {
        sentMessages[messageId] = currentTimeMillis()
    }
    
    fun trackReceivedAck(messageId: Long) {
        val sentTime = sentMessages[messageId]
        if (sentTime != null) {
            latencyMs = currentTimeMillis() - sentTime
