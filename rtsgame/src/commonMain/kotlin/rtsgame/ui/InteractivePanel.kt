...
class InteractiveWebGPUPanel {
    ...
    private var gameState = createDemoGameState()
    private var panelState = createControlPanel()
    private var mouseState = MouseState(0f, 0f)
    ...
    private fun isPointInButton(x: Float, y: Float, button: ButtonState): Boolean {
    ...
    private fun handleButtonClick(buttonId: ButtonId) {
        when (buttonId.value) {
            "start_stop" -> {
                isRunning = !isRunning
    ...
    private fun updateButtonLabel(panel: PanelState, buttonId: ButtonId, newLabel: String): PanelState {
    ...
    private fun createDemoGameState(): GameState {
    ...
    private fun addRandomUnit(currentState: GameState): GameState {
        val newUnit = Entity(
            id = EntityId("unit_${currentTimeMillis()}"),
            position = Position(
