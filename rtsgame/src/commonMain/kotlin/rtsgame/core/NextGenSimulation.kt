...
class TacticalAISystem : System {
    ...
    private fun processCommand(world: ECSWorld, entity: EntityId, ai: UnitAIComponent, command: Command) {
        when (command) {
            is MoveCommand -> {
    ...
    private fun findNewTask(world: ECSWorld, entity: EntityId, ai: UnitAIComponent, 
    ...
    private fun validateTarget(world: ECSWorld, entity: EntityId, ai: UnitAIComponent) {
    ...
    private fun findEnemiesInRange(world: ECSWorld, pos: PositionComponent, 
    ...
    private fun distanceSq(a: PositionComponent, b: PositionComponent): Float {
        val dx = a.x - b.x
        val dy = a.y - b.y
        return dx * dx + dy * dy
    }
...
class StrategicAISystem : System {
    private val teamStrategies = mutableMapOf<Int, Strategy>()
    
    override fun update(world: ECSWorld, deltaTime: Float) {
        // Update strategic AI every second
        if (System.currentTimeMillis() % 1000 > 50) return
    ...
    private fun analyzeTeams(world: ECSWorld): Map<Int, TeamAnalysis> {
    ...
    private fun determineStrategy(analysis: TeamAnalysis): Strategy {
        return when {
    ...
    private fun executeStrategy(world: ECSWorld, teamId: Int, strategy: Strategy, analysis: TeamAnalysis) {
    ...
    private fun issueProductionOrders(world: ECSWorld, teamId: Int, unitType: UnitType, count: Int) {
        // Find production buildings and queue units
        // Implementation details...
    }
    
    private fun coordinateAttack(world: ECSWorld, teamId: Int) {
