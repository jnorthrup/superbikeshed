...
abstract class ComponentStorage<T : Component> {
    ...
    protected val entityToIndex = mutableMapOf<EntityId, Int>()
    protected val indexToEntity = mutableListOf<EntityId>()
...
class DenseComponentStorage<T : Component>(
    ...
    private var components = Array<Any?>(initialCapacity) { null }
    private var size = 0
...
class ECSWorld {
    ...
    private val entitySignatures = mutableMapOf<EntityId, MutableSet<ComponentTypeId>>()
    ...
    private fun getOrCreateStorage(typeId: ComponentTypeId): ComponentStorage<*> {
...
abstract class ParallelSystem(vararg requiredComponents: ComponentTypeId) : ComponentSystem(*requiredComponents) {
    ...
    private fun getProcessorCount(): Int = 4 // TODO: Get actual processor count
}