...
class ProceduralMapGenerator(
    ...
    private val moistureMap = FloatArray(width * height)
    private val temperatureMap = FloatArray(width * height)
    private val biomeMap = Array(width * height) { Biome.PLAINS }
    
    // Noise generators
    private val elevationNoise = PerlinNoise(random.nextLong())
    private val moistureNoise = PerlinNoise(random.nextLong())
    private val temperatureNoise = PerlinNoise(random.nextLong())
    ...
    private fun simulateErosion(iterations: Int) {
        repeat(iterations) {
    ...
    private fun generateMoistureMap() {
    ...
    private fun generateTemperatureMap() {
        for (y in 0 until height) {
            for (x in 0 until width) {
    ...
    private fun placeFeatures(): List<MapFeature> {
    ...
    private fun placeResources(features: MutableList<MapFeature>) {
    ...
    private fun placeStrategicPoints(features: MutableList<MapFeature>) {
    ...
    private fun placeObstacles(features: MutableList<MapFeature>) {
        // Rock formations
        for (i in 0 until 50) {
    ...
    private fun generateSpawnPoints(): List<SpawnPoint> {
    ...
    private fun isValidSpawnLocation(x: Int, y: Int): Boolean {
        val biome = getBiome(x, y)
        val elevation = getHeight(x, y)
    ...
    private fun findChokepoints(): List<Vec2> {
    ...
    private fun isChokepoint(x: Int, y: Int): Boolean {
    ...
    private fun isHighGround(x: Int, y: Int): Boolean {
    ...
    private fun blur(map: FloatArray, radius: Int) {
    ...
    private fun getHeight(x: Int, y: Int): Float {
        if (x < 0 || x >= width || y < 0 || y >= height) return 0f
        return heightMap[y * width + x]
    }
    
    private fun setHeight(x: Int, y: Int, value: Float) {
        if (x >= 0 && x < width && y >= 0 && y < height) {
            heightMap[y * width + x] = value.coerceIn(0f, 1f)
        }
    }
    
    private fun getBiome(x: Int, y: Int): Biome {
