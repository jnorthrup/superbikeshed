...
class GameUnit(
    ...
    private fun processMove(deltaTime: Double) {
        val dx = targetX - x
        val dy = targetY - y
    ...
    private fun processAttack(simulation: rtsgame.core.Simulation, deltaTime: Double) {
    ...
    private fun processBuild(simulation: rtsgame.core.Simulation, deltaTime: Double) {
    ...
    private fun performAttack(target: GameUnit, simulation: rtsgame.core.Simulation) {
        // Create projectile
    ...
    private fun startConstruction(buildCommand: BuildCommand, simulation: rtsgame.core.Simulation) {
        // Check resources
    ...
    private fun executeAutonomousBehavior(simulation: rtsgame.core.Simulation) {
    ...
    private fun findNearbyEnemies(simulation: Simulation, range: Double): Indexed<GameUnit> {
    ...
    private fun canAfford(resources: TeamResourcesExtended, buildingType: BuildingCost): Boolean {
        return resources.mass >= buildingType.mass && 
               resources.energy >= buildingType.energy
    }
    
    private fun deductResources(resources: TeamResourcesExtended, buildingType: BuildingCost) {
        resources.mass -= buildingType.mass
        resources.energy -= buildingType.energy
    }
    
    private fun getBuildingType(typeName: String): BuildingCost? {
...
private val Simulation.units: Indexed<GameUnit>
    get() = entityManager.units.size j { i: Int -> entityManager.units[i] }

private val Simulation.nextEntityId: Int
