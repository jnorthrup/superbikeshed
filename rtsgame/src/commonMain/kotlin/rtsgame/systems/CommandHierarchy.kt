...
class EnhancedCommandHierarchy(
    val team: String,
    private val computroniumManager: ComputroniumManager
) {
    
    // Command structure
    private val commandNodes = mutableMapOf<Int, CommandNode>()
    private val commandLinks = mutableListOf<CommandLink>()
    private var rootCommanderId: Int? = null
    
    // Authority levels
    private val authorityLevels = mutableMapOf<Int, AuthorityLevel>()
    private val delegationChains = mutableMapOf<Int, MutableList<Int>>()
    
    // Command fitness tracking
    private val commandFitness = mutableMapOf<Int, CommandFitnessData>()
    private var lastFitnessUpdate: Long = 0
    
    // Decision override system
    private val overrideQueue = mutableListOf<CommandOverride>()
    private val decisionHistory = mutableListOf<CommandDecision>()
    ...
    private fun updateCommandNodes(simulation: rtsgame.core.Simulation) {
    ...
    private fun establishCommandLinks(units: List<GameUnit>) {
        commandLinks.clear()
    ...
    private fun findBestCommander(unit: GameUnit): CommandNode? {
    ...
    private fun calculateDistance(entity1: Any, entity2: Any): Double {
        val (x1, y1) = getEntityPosition(entity1)
        val (x2, y2) = getEntityPosition(entity2)
        return sqrt((x1 - x2).pow(2) + (y1 - y2).pow(2))
    }
    
    private fun getEntityPosition(entity: Any): Pair<Double, Double> {
        return when (entity) {
    ...
    private fun calculateCommandLatency(commander: CommandNode, unit: GameUnit): Double {
    ...
    private fun calculateLinkReliability(commander: CommandNode, unit: GameUnit): Double {
        val distance = calculateDistance(commander.entity, unit)
    ...
    private fun determineAuthorityLevel(commander: CommandNode, unit: GameUnit): AuthorityLevel {
        val distance = calculateDistance(commander.entity, unit)
        val maxRange = commander.authorityRange
    ...
    private fun updateCommandFitness(simulation: rtsgame.core.Simulation) {
    ...
    private fun calculateCommandFitness(commander: CommandNode, simulation: rtsgame.core.Simulation): CommandFitnessData {
    ...
    private fun calculateCombatEffectiveness(commander: CommandNode, simulation: rtsgame.core.Simulation): Double {
    ...
    private fun considerDelegation(commander: CommandNode, simulation: rtsgame.core.Simulation) {
    ...
    private fun promoteToFieldCommander(unit: GameUnit, parentCommander: CommandNode) {
    ...
    private fun addDelegationChain(parentId: Int, delegateId: Int) {
        delegationChains.getOrPut(parentId) { mutableListOf() }.add(delegateId)
    }
    
    private fun processDelegationChains(simulation: rtsgame.core.Simulation) {
    ...
    private fun syncStrategicDecisions(parent: CommandNode, delegate: CommandNode, simulation: rtsgame.core.Simulation) {
        // Transfer high-level strategy from parent to delegate
        // This would integrate with the strategic AI system
    }
    
    private fun processOverrides(simulation: rtsgame.core.Simulation) {
    ...
    private fun executeOverride(override: CommandOverride, simulation: rtsgame.core.Simulation): Boolean {
    ...
    private fun executeAutonomousCommands(simulation: rtsgame.core.Simulation) {
    ...
    private fun grantAutonomousAuthority(commander: CommandNode, simulation: rtsgame.core.Simulation) {
    ...
    private fun recordDecision(override: CommandOverride, unit: GameUnit) {
