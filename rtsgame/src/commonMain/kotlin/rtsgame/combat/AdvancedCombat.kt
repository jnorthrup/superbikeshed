...
class AdvancedCombatSystem : System {
    private val projectilePool = ProjectilePool(1024)
    private val damageEvents = mutableListOf<DamageEvent>()
    ...
    private fun processWeaponFiring(world: ECSWorld, deltaTime: Float) {
    ...
    private fun updateProjectiles(world: ECSWorld, deltaTime: Float) {
    ...
    private fun onProjectileHit(world: ECSWorld, projectile: Projectile, target: EntityId) {
        if (projectile.areaOfEffect > 0) {
            // Explosive damage
            createExplosion(world, projectile)
        } else {
    ...
    private fun createExplosion(world: ECSWorld, projectile: Projectile) {
    ...
    private fun checkAreaCollisions(world: ECSWorld, projectile: Projectile) {
    ...
    private fun processDamageEvents(world: ECSWorld) {
    ...
    private fun updateExplosions(world: ECSWorld, deltaTime: Float) {
    ...
    private fun onUnitDeath(world: ECSWorld, unit: EntityId, killer: EntityId?) {
    ...
    private fun findEntitiesInRange(
        world: ECSWorld,
        x: Float,
        y: Float,
        range: Float
...
class ProjectilePool(initialCapacity: Int) {
    private val pool = mutableListOf<Projectile>()
    private val active = mutableListOf<Projectile>()
...
object BallisticsCalculator {
    ...
    private fun calculateLaunchAngle(range: Float, velocity: Float, gravity: Float): Float? {
