 ...
 */
class AStar(private val terrainSystem: TerrainSystem) {
    ...
    private val openSet = mutableSetOf<PathNode>()
    private val closedSet = mutableSetOf<PathNode>()
    private val nodeGrid = Array(GRID_SIZE) { x ->
        Array(GRID_SIZE) { y ->
    ...
    private fun findNearestWalkableNode(x: Int, y: Int): PathNode? {
        val searchRadius = 10
    ...
    private fun calculateDistance(nodeA: PathNode, nodeB: PathNode): Double {
    ...
    private fun smoothPath(path: List<Pair<Double, Double>>): Indexed<Pair<Double, Double>> {
        if (path.size <= 2) {
    ...
    private fun hasLineOfSight(x1: Double, y1: Double, x2: Double, y2: Double): Boolean {
        val dx = x2 - x1
        val dy = y2 - y1
    ...
    private fun createDirectPath(startX: Double, startY: Double, targetX: Double, targetY: Double): Indexed<Pair<Double, Double>> {
        return 2 j { i ->
            if (i == 0) startX to startY else targetX to targetY
        }
    }
    
    private fun resetNodeCosts() {
