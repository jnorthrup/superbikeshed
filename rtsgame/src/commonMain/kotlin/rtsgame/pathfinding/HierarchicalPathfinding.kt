...
class HierarchicalPathfinder(
    ...
    private val clustersX = (mapWidth + clusterSize - 1) / clusterSize
    private val clustersY = (mapHeight + clusterSize - 1) / clusterSize
    private val clusters = Array(clustersX * clustersY) { Cluster(it) }
    private val clusterGraph = ClusterGraph()
    
    // Flow field cache per destination cluster
    private val flowFieldCache = mutableMapOf<Int, FlowField>()
    ...
    private fun findPortals(cluster1: Cluster, cluster2: Cluster, horizontal: Boolean) {
    ...
    private fun findClusterPath(start: Cluster, end: Cluster): List<Cluster> {
    ...
    private fun heuristic(a: Cluster, b: Cluster): Float {
        val dx = (a.x - b.x).toFloat()
        val dy = (a.y - b.y).toFloat()
        return sqrt(dx * dx + dy * dy)
    }
    
    private fun reconstructPath(cameFrom: Map<Cluster, Cluster>, current: Cluster): List<Cluster> {
    ...
    private fun refinePath(start: PositionComponent, end: PositionComponent, 
    ...
    private fun smoothPath(waypoints: List<PositionComponent>): List<PositionComponent> {
    ...
    private fun hasLineOfSight(a: PositionComponent, b: PositionComponent): Boolean {
    ...
    private fun findLocalPath(start: PositionComponent, end: PositionComponent): Path {
        // Simple A* for intra-cluster pathfinding
        return Path(listOf(start, end)) // Simplified
    }
    
    private fun calculateFlowField(destination: PositionComponent): FlowField {
    ...
    private fun generatePathFromFlowField(
 ...
 */
class ClusterGraph {
    private val connections = mutableMapOf<Int, MutableMap<Int, MutableList<Portal>>>()
...
class FlowField(val width: Int, val height: Int) {
    private val flowX = FloatArray(width * height)
    private val flowY = FloatArray(width * height)
