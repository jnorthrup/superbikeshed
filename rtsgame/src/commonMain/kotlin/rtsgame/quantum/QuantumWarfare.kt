...
class QuantumWarfareSystem : System {
    private val quantumStates = mutableMapOf<EntityId, QuantumState>()
    private val entanglements = mutableMapOf<EntityId, Set<EntityId>>()
    private val waveCollapse = WaveCollapseCalculator()
    ...
    private fun updateQuantumStates(world: ECSWorld, deltaTime: Float) {
    ...
    private fun processQuantumTunneling(world: ECSWorld, deltaTime: Float) {
        world.forEach<QuantumComponent>(ComponentTypes.QUANTUM) { entity, quantum ->
    ...
    private fun processEntanglements(world: ECSWorld) {
    ...
    private fun propagateQuantumEffect(world: ECSWorld, source: EntityId, target: EntityId) {
    ...
    private fun collapseObservedStates(world: ECSWorld) {
    ...
    private fun collapseWavefunction(world: ECSWorld, entity: EntityId, state: QuantumState) {
    ...
    private fun createQuantumTunnelEffect(world: ECSWorld, from: PositionComponent, to: PositionComponent) {
    ...
    private fun createCollapseEffect(world: ECSWorld, pos: PositionComponent) {
        // Shockwave effect
        repeat(30) {
    ...
    private fun hasObstacle(world: ECSWorld, from: PositionComponent, to: PositionComponent): Boolean {
        // Check if path is blocked
        return false // Simplified
    }
    
    private fun distance(a: PositionComponent, b: PositionComponent): Float {
        val dx = b.x - a.x
        val dy = b.y - a.y
...
class TemporalWarfareSystem : System {
    private val timeFields = mutableListOf<TimeField>()
    private val chronoAnchors = mutableMapOf<EntityId, ChronoAnchor>()
    ...
    private fun updateTimeFields(deltaTime: Float) {
    ...
    private fun applyTemporalEffects(world: ECSWorld, deltaTime: Float) {
    ...
    private fun processChronoAnchors(world: ECSWorld, deltaTime: Float) {
    ...
    private fun rewindTime(world: ECSWorld, entity: EntityId, anchor: ChronoAnchor, seconds: Float) {
    ...
    private fun createTemporalShockwave(world: ECSWorld, center: PositionComponent, strength: Float) {
...
class DimensionalWarfareSystem : System {
    private val dimensions = Array(3) { Dimension(it) }
    private val phaseShifts = mutableMapOf<EntityId, PhaseShift>()
    private val dimensionalRifts = mutableListOf<DimensionalRift>()
    ...
    private fun updatePhaseShifts(world: ECSWorld, deltaTime: Float) {
    ...
    private fun completePhaseShift(world: ECSWorld, entity: EntityId, shift: PhaseShift) {
    ...
    private fun updatePhasedVisibility(world: ECSWorld, entity: EntityId, shift: PhaseShift) {
    ...
    private fun processRifts(world: ECSWorld, deltaTime: Float) {
    ...
    private fun pullUnitsIntoRift(world: ECSWorld, rift: DimensionalRift) {
    ...
    private fun transportThroughRift(world: ECSWorld, entity: EntityId, rift: DimensionalRift) {
        val pos = world.getComponent<PositionComponent>(entity, ComponentTypes.POSITION) ?: return
    ...
    private fun processCrossDimensionalCombat(world: ECSWorld) {
    ...
    private fun collapseRift(world: ECSWorld, rift: DimensionalRift) {
        // Explosion in both dimensions
        createDimensionalExplosion(world, rift.x, rift.y, rift.radius * 2f)
    }
    
    private fun createDimensionalExplosion(world: ECSWorld, x: Float, y: Float, radius: Float) {
...
class ChronoAnchor {
    private val stateHistory = mutableListOf<TemporalSnapshot>()
    var isActive = true
    var elapsedTime = 0f
