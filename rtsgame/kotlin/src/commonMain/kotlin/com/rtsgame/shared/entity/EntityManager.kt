...
class EntityManager {
    ...
    private val componentStores = mutableMapOf<Class<*>, MutableMap<Long, Any>>()
    ...
    private val systemPriorities = mutableMapOf<GameSystem, Int>()
    
    // Entity lifecycle tracking
    private val activeEntities = mutableSetOf<Long>()
    private val pendingRemovals = mutableSetOf<Long>()
    private val pendingAdditions = mutableListOf<GameEntity>()
     ...
     */
    private fun removeEntityInternal(entityId: Long) {
...
private class SpatialIndex {
    private data class SpatialEntity(
        val id: Long,
        var x: Double,
        var y: Double
    )
    
    private val entities = mutableMapOf<Long, SpatialEntity>()
    private val gridSize = 100.0 // Grid cell size
    private val grid = mutableMapOf<Pair<Int, Int>, MutableSet<Long>>()
    ...
    private fun addToGrid(id: Long, x: Double, y: Double) {
    ...
    private fun removeFromGrid(id: Long, x: Double, y: Double) {
        val gridX = (x / gridSize).toInt()
        val gridY = (y / gridSize).toInt()
        val cell = Pair(gridX, gridY)
        grid[cell]?.remove(id)
