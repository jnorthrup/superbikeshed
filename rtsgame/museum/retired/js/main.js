...
class RTSEngine {
    constructor() {
        ...
        this.projectiles = [];
        this.captions = [];
        this.rng = new deterministicRNG_js_1.DeterministicRNG(12345); // Seeded for deterministic replay
        ...
        this.camera = {
            x: gameConstants_js_1.WORLD_SIZE / 2,
            y: gameConstants_js_1.WORLD_SIZE / 2,
            zoom: 0.5,
    ...
    initializeGame() {
        ...
        // Generate terrain
        (0, terrain_js_1.generateTerrain)({ terrain: this.terrain, resourceNodes: this.resourceNodes });
        this.battleLogger.addEntry('system', `Generated ${gameConstants_js_1.GRID_SIZE}x${gameConstants_js_1.GRID_SIZE} terrain with ${this.resourceNodes.length} resource nodes`);
        // Setup build lists
    ...
    spawnInitialUnits() {
        ...
        const blueStart = (0, terrain_js_1.findLandPosition)({ terrain: this.terrain, resourceNodes: this.resourceNodes }, gameConstants_js_1.WORLD_SIZE * 0.2, gameConstants_js_1.WORLD_SIZE * 0.5, 5);
        const redStart = (0, terrain_js_1.findLandPosition)({ terrain: this.terrain, resourceNodes: this.resourceNodes }, gameConstants_js_1.WORLD_SIZE * 0.8, gameConstants_js_1.WORLD_SIZE * 0.5, 5);
        ...
        // Spawn commanders
        this.units.push(new unit_js_1.Unit(blueStart.x, blueStart.y, 'blue', unitTypes_js_1.UNIT_TYPES.commander, gameContext));
        this.units.push(new unit_js_1.Unit(redStart.x, redStart.y, 'red', unitTypes_js_1.UNIT_TYPES.commander, gameContext));
    ...
    updateResourceIncome() {
        for (const team of ['blue', 'red']) {
            const extractors = this.buildings.filter(b => b.team === team && b.type.resourceGeneration);
            let massIncome = 0;
            let energyIncome = 0;
    ...
    renderTerrain() {
        const cellSize = gameConstants_js_1.TILE_SIZE;
        const startX = Math.max(0, Math.floor((this.camera.x - this.canvas.width / (2 * this.camera.zoom)) / cellSize));
        const endX = Math.min(gameConstants_js_1.GRID_SIZE, Math.ceil((this.camera.x + this.canvas.width / (2 * this.camera.zoom)) / cellSize));
        const startY = Math.max(0, Math.floor((this.camera.y - this.canvas.height / (2 * this.camera.zoom)) / cellSize));
        const endY = Math.min(gameConstants_js_1.GRID_SIZE, Math.ceil((this.camera.y + this.canvas.height / (2 * this.camera.zoom)) / cellSize));
    ...
    renderResourceNodes() {
        for (const node of this.resourceNodes) {
            if (!this.isInView(node.x, node.y, 20))
                continue;
    ...
    renderUnit(unit) {
        if (!this.isInView(unit.x, unit.y, unit.type.size))
            return;
        this.ctx.save();
        ...
        if (unit.selected) {
            ...
            this.ctx.lineWidth = 2;
            this.ctx.strokeRect(unit.x - unit.type.size / 2 - 5, unit.y - unit.type.size / 2 - 5, unit.type.size + 10, unit.type.size + 10);
        }
    }
    renderBuilding(building) {
        if (!this.isInView(building.x, building.y, building.type.size))
            return;
        // Building body
        this.ctx.fillStyle = building.team === 'blue' ? '#2266aa' : '#aa2222';
        this.ctx.fillRect(building.x - building.type.size / 2, building.y - building.type.size / 2, building.type.size, building.type.size);
        // Building type indicator
        this.ctx.fillStyle = building.type.color || '#ffffff';
        this.ctx.fillRect(building.x - building.type.size / 4, building.y - building.type.size / 4, building.type.size / 2, building.type.size / 2);
        // Health bar
    ...
    centerView() {
        this.camera.x = gameConstants_js_1.WORLD_SIZE / 2;
        this.camera.y = gameConstants_js_1.WORLD_SIZE / 2;
    }
    handleZoom(e) {
        e.preventDefault();
