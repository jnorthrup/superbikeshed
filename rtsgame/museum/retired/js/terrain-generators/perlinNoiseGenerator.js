...
exports.generatePerlinNoiseTerrain = generatePerlinNoiseTerrain;
...
class PerlinNoiseGenerator {
    ...
    noise(x, y) {
        ...
        return this.lerp(this.lerp(this.dot(this.gradients[this.permutation[A]], x, y), this.dot(this.gradients[this.permutation[B]], x - 1, y), u), this.lerp(this.dot(this.gradients[this.permutation[A + 1]], x, y - 1), this.dot(this.gradients[this.permutation[B + 1]], x - 1, y - 1), u), v);
    }
 ...
 */
function generatePerlinNoiseTerrain(gameContext, retries = 0) {
    ...
    for (let x = 0; x < gameConstants_js_1.GRID_SIZE; x++) {
        ...
        for (let y = 0; y < gameConstants_js_1.GRID_SIZE; y++) {
            ...
            if (elevation < SEA_LEVEL - 0.1) {
                ...
                terrainType = gameConstants_js_1.TERRAIN_TYPES.MOUNTAIN;
            }
            else {
                terrainType = gameConstants_js_1.TERRAIN_TYPES.LAND;
            }
            ...
            gameContext.mobilityMesh[x][y] = {
                cost: movementCost,
                passable: terrainType !== gameConstants_js_1.TERRAIN_TYPES.WATER
            };
        }
    }
    ...
    for (let x = 0; x < gameConstants_js_1.GRID_SIZE; x++) {
        for (let y = 0; y < gameConstants_js_1.GRID_SIZE; y++) {
            if (gameContext.terrain[x][y].type === gameConstants_js_1.TERRAIN_TYPES.LAND) {
                landTiles++;
            }
        }
    }
    const totalTiles = gameConstants_js_1.GRID_SIZE * gameConstants_js_1.GRID_SIZE;
    ...
    if (landPercentage < gameConstants_js_1.MIN_LAND_PERCENTAGE && retries < gameConstants_js_1.MAX_TERRAIN_RETRIES) {
        console.log(`Land percentage ${landPercentage.toFixed(2)} is less than ${gameConstants_js_1.MIN_LAND_PERCENTAGE}. Regenerating terrain (attempt ${retries + 1}/${gameConstants_js_1.MAX_TERRAIN_RETRIES}).`);
        return generatePerlinNoiseTerrain(gameContext, retries + 1);
    }
    else if (landPercentage < gameConstants_js_1.MIN_LAND_PERCENTAGE) {
        console.warn(`Failed to achieve minimum land percentage after ${gameConstants_js_1.MAX_TERRAIN_RETRIES} attempts. Current land: ${landPercentage.toFixed(2)}. Using current terrain anyway.`);
    }
    ...
    for (let attempts = 0; attempts < maxResources * 10 && resourcesPlaced < maxResources; attempts++) {
        const x = Math.floor(gameContext.seedRandom ? gameContext.seedRandom.random() * gameConstants_js_1.GRID_SIZE : Math.random() * gameConstants_js_1.GRID_SIZE);
        const y = Math.floor(gameContext.seedRandom ? gameContext.seedRandom.random() * gameConstants_js_1.GRID_SIZE : Math.random() * gameConstants_js_1.GRID_SIZE);
        if (gameContext.terrain[x] && gameContext.terrain[x][y] &&
            gameContext.terrain[x][y].type === gameConstants_js_1.TERRAIN_TYPES.LAND &&
            ...
            if (!tooClose) {
                gameContext.terrain[x][y].type = gameConstants_js_1.TERRAIN_TYPES.RESOURCE;
