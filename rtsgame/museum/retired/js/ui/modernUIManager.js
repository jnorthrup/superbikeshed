...
class ModernUIManager {
    ...
    updateResourceBar(gameContext) {
        ...
        if (resources && resources.blue) {
            ...
            if (massEl)
                massEl.textContent = Math.floor(resources.blue.mass || 0);
            if (massIncomeEl)
                massIncomeEl.textContent = Math.floor(resources.blue.massIncome || 0);
            // Energy
            const energyEl = document.getElementById('blue-energy');
            const energyIncomeEl = document.getElementById('blue-energy-income');
            if (energyEl)
                energyEl.textContent = Math.floor(resources.blue.energy || 0);
            if (energyIncomeEl)
                energyIncomeEl.textContent = Math.floor(resources.blue.energyIncome || 0);
            // Computronium
            ...
            if (compEl)
                compEl.textContent = (resources.blue.computronium || 0).toFixed(1);
            if (compIncomeEl)
                compIncomeEl.textContent = (resources.blue.computroniumIncome || 0).toFixed(1);
        }
        ...
        if (resources && resources.red) {
            ...
            if (this.redMassDisplay)
                this.redMassDisplay.textContent = '0';
            if (this.redMassIncomeDisplay)
                this.redMassIncomeDisplay.textContent = '0';
            if (this.redEnergyDisplay)
                this.redEnergyDisplay.textContent = '0';
            if (this.redEnergyIncomeDisplay)
                this.redEnergyIncomeDisplay.textContent = '0';
            if (this.redComputroniumDisplay)
                this.redComputroniumDisplay.textContent = '0';
            if (this.redComputroniumIncomeDisplay)
                this.redComputroniumIncomeDisplay.textContent = '0';
        }
    }
    updateGameInfo(gameContext) {
        ...
        if (units && Array.isArray(units)) {
            ...
            if (blueEl)
                blueEl.textContent = blueUnits;
            if (redEl)
                redEl.textContent = redUnits;
        }
    }
    updateTeamStatus(gameContext) {
        ...
        const blueCommander = blueUnits.find(u => u.type === unitTypes_js_1.UNIT_TYPES.commander);
        ...
        if (blueArmySizeEl)
            blueArmySizeEl.textContent = blueUnits.length;
        if (blueBuildingsEl)
            blueBuildingsEl.textContent = blueBuildings.length;
        // Red team
        const redUnits = units.filter(u => u.team === 'red');
        const redBuildings = buildings.filter(b => b.team === 'red');
        const redCommander = redUnits.find(u => u.type === unitTypes_js_1.UNIT_TYPES.commander);
        ...
        if (redArmySizeEl)
            redArmySizeEl.textContent = redUnits.length;
        if (redBuildingsEl)
            redBuildingsEl.textContent = redBuildings.length;
    }
    updateMinimap(gameContext) {
        ...
        const minimapRenderContext = {
            ...
            terrain: (terrain === null || terrain === void 0 ? void 0 : terrain.terrain) || null,
            resourceNodes: resourceNodes || [],
            units: units || [],
            buildings: buildings || [],
            camera: camera || {}
        };
        (0, minimap_canvas2d_js_1.drawMinimap)(minimapRenderContext);
    }
    ...
    updateAlerts(gameContext) {
        ...
        if (now - this.lastAlertTime > 5000) { // Check every 5 seconds
            ...
            const blueCommander = units.find(u => u.team === 'blue' && u.type === unitTypes_js_1.UNIT_TYPES.commander);
            const redCommander = units.find(u => u.team === 'red' && u.type === unitTypes_js_1.UNIT_TYPES.commander);
        ...
        // Update DOM
        alertContainer.innerHTML = this.alertQueue.map(alert => `<div class="alert ${alert.type}">${alert.message}</div>`).join('');
    }
...
exports.ModernUIManager = ModernUIManager;
