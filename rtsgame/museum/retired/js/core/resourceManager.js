...
class ResourceManager {
    constructor(gameContext) {
        ...
        this.economicPhase = gameConstants_js_1.ECONOMIC_PHASES.SURVIVAL;
    ...
    determineResourceType(position, terrain) {
        ...
        switch (terrainType) {
            case 3: // Grass
                return gameConstants_js_1.RESOURCE_TYPES.MASS;
            case 4: // Forest
                return gameConstants_js_1.RESOURCE_TYPES.ENERGY;
            case 6: // Hill
                return Math.random() < 0.7 ? gameConstants_js_1.RESOURCE_TYPES.MASS : gameConstants_js_1.RESOURCE_TYPES.ENERGY;
            default:
                return gameConstants_js_1.RESOURCE_TYPES.MASS;
        }
    }
    ...
    calculateResourceYield(type) {
        const baseYield = type === gameConstants_js_1.RESOURCE_TYPES.MASS ? 2 : 3;
    ...
    updateEconomicPhase() {
        ...
        if (extractorCount >= 7) {
            this.economicPhase = gameConstants_js_1.ECONOMIC_PHASES.EXPERIMENTAL;
        }
        else if (extractorCount >= 5) {
            this.economicPhase = gameConstants_js_1.ECONOMIC_PHASES.ADVANCED;
        }
        else if (extractorCount >= 3) {
            this.economicPhase = gameConstants_js_1.ECONOMIC_PHASES.EXPANSION;
        }
        else {
            this.economicPhase = gameConstants_js_1.ECONOMIC_PHASES.SURVIVAL;
        }
    ...
    depleteResourceNode(nodeKey) {
        ...
        if (node && node.extractor) {
            ...
            if (this.gameContext.addEvent) {
                this.gameContext.addEvent('resource_depletion', `Resource node depleted at (${node.position.x}, ${node.position.y})`, 1);
            }
        }
    }
    /**
