...
class WeaponSystem {
    ...
    update(deltaTime, entities, gameContext) {
        ...
        for (const entity of entities) {
            ...
            if (entity.weapon.capacitorCharge < entity.weapon.capacitorSize) {
                ...
                entity.weapon.capacitorCharge = Math.min(entity.weapon.capacitorSize, entity.weapon.capacitorCharge + rechargeAmount);
            }
    ...
    fireWeapon(entity, target, gameContext) {
        if (!entity.weapon || !entity.weapon.canFire)
            return null;
        // Check energy cost
        if (entity.weapon.capacitorCharge < entity.weapon.energyCost)
            return null;
        // Create projectile
        const projectile = {
            x: entity.position.x,
