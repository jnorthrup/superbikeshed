...
class ProgressionManager {
    ...
    checkTierProgression(gameContext) {
        ...
        for (const [tier, requirements] of Object.entries(this.tierThresholds)) {
            if (currentTier === tier)
                continue;
            const canProgress = resources.mass >= requirements.mass &&
    ...
    processPromotion(unit, veterancyData, gameContext) {
        ...
        if (gameContext.captions) {
            gameContext.captions.push(new gameContext.Caption(unit.x, unit.y, `Promoted to ${veterancyData.level}`, '#ff4', 14));
        }
        // Log promotion event
        if (gameContext.addEvent) {
            gameContext.addEvent(gameContext, 'promotion', `${unit.team} ${unit.type.name} promoted to ${veterancyData.level}`, 2);
        }
    }
    isUnderFire(unit, gameContext) {
        return gameContext.projectiles.some(p => p.target === unit &&
            p.team !== unit.team);
    }
}
