...
class BattleSimulator {
    ...
    runBattleSimulation() {
        return __awaiter(this, arguments, void 0, function* (config = {}) {
            const simulation = {
                ...
                startTime: Date.now(),
                config: Object.assign({ duration: config.duration || 600, startingResources: config.startingResources || { mass: 100, energy: 150 }, terrainType: config.terrainType || 'random', aiDifficulty: config.aiDifficulty || 'normal' }, config),
                events: [],
                metrics: {
                    gamePhases: [],
                    combatEvents: [],
            ...
            const resources = {
                blue: Object.assign(Object.assign({}, simulation.config.startingResources), { massIncome: 0, energyIncome: 0 }),
                red: Object.assign(Object.assign({}, simulation.config.startingResources), { massIncome: 0, energyIncome: 0 })
            };
            const units = [];
            const buildings = [];
            // Generate terrain
                ...
                for (let i = units.length - 1; i >= 0; i--) {
                    ...
                    if (unit.hp <= 0) {
                        if (unit.type === unitTypes_js_1.UNIT_TYPES.commander) {
                            ...
                            this.logEvent(simulation, 'COMMANDER_DESTROYED', `${unit.team} commander eliminated! ${gameState.winner} victory!`);
    ...
    generateCustomTerrain(terrain, resourceNodes, terrainType) {
        ...
        for (let i = 0; i < 20; i++) {
            const x = Math.random() * gameConstants_js_1.WORLD_SIZE;
            const y = Math.random() * gameConstants_js_1.WORLD_SIZE;
    ...
    generateIslandTerrain(terrain) {
        const centerX = gameConstants_js_1.GRID_SIZE / 2;
        const centerY = gameConstants_js_1.GRID_SIZE / 2;
        for (let x = 0; x < gameConstants_js_1.GRID_SIZE; x++) {
            for (let y = 0; y < gameConstants_js_1.GRID_SIZE; y++) {
                const distToCenter = Math.sqrt(Math.pow((x - centerX), 2) + Math.pow((y - centerY), 2));
                if (distToCenter > gameConstants_js_1.GRID_SIZE * 0.3) {
                    terrain[x][y] = gameConstants_js_1.TERRAIN_TYPES.WATER;
                }
            }
        }
    }
    generateChokepointTerrain(terrain) {
        ...
        const chokeY = gameConstants_js_1.GRID_SIZE / 2;
        for (let x = 0; x < gameConstants_js_1.GRID_SIZE; x++) {
            for (let y = chokeY - 2; y <= chokeY + 2; y++) {
                if (y >= 0 && y < gameConstants_js_1.GRID_SIZE) {
                    if (Math.abs(x - gameConstants_js_1.GRID_SIZE / 2) < 3) {
                        terrain[x][y] = gameConstants_js_1.TERRAIN_TYPES.LAND; // Passage
                    }
                    else {
                        terrain[x][y] = gameConstants_js_1.TERRAIN_TYPES.MOUNTAIN;
                    }
                }
            }
        }
    ...
    detectBalanceIssues(simulation, units, buildings, resources) {
        ...
        if (Math.abs(blueUnits - redUnits) > 5) {
            simulation.metrics.balanceData.push({
                time: ((_a = simulation.gameState) === null || _a === void 0 ? void 0 : _a.gameTime) || 0,
        ...
        if (Math.abs(blueBuildings - redBuildings) > 3) {
            simulation.metrics.balanceData.push({
                time: ((_b = simulation.gameState) === null || _b === void 0 ? void 0 : _b.gameTime) || 0,
    ...
    calculateOutcomeQuality(simulation, gameState) {
        ...
        if (duration >= 180 && duration <= 360)
            quality += 20; // Good duration
        if (simulation.customerExperience.excitementPeaks.length > 2)
            quality += 15; // Multiple exciting moments
        if (simulation.customerExperience.boredomScore < 10)
            quality += 10; // Not boring
        if (simulation.customerExperience.frustrationScore < 20)
            quality += 5; // Not frustrating
    ...
    runBattleAnalysisSuite() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            for (const scenario of scenarios) {
                ...
                const result = yield this.runBattleSimulation(scenario);
                results.push(result);
    ...
    generateCustomerExperienceReport() {
        ...
        if (avgEngagement < 60)
            console.log(`   ⚠️  LOW ENGAGEMENT - Games not exciting enough`);
        if (avgFrustration > 25)
            console.log(`   ⚠️  HIGH FRUSTRATION - Players getting stuck/bored`);
        if (avgActionIntensity < 5)
            console.log(`   ⚠️  LOW ACTION - Games too slow-paced`);
    }
