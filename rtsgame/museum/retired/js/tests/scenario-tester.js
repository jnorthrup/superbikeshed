...
class ScenarioTester {
    ...
    createTestTerrain(scenario) {
        ...
        if (scenario.waterBarrier) {
            ...
            const barrierX = Math.floor(gameConstants_js_1.GRID_SIZE * 0.4);
            for (let y = 0; y < gameConstants_js_1.GRID_SIZE; y++) {
                for (let x = barrierX; x < barrierX + scenario.waterBarrier.width; x++) {
                    if (x < gameConstants_js_1.GRID_SIZE) {
        ...
        if (scenario.mountainRange) {
            ...
            const mountainY = Math.floor(gameConstants_js_1.GRID_SIZE * 0.5);
            for (let x = 0; x < gameConstants_js_1.GRID_SIZE; x++) {
                for (let y = mountainY; y < mountainY + scenario.mountainRange.height; y++) {
                    if (y < gameConstants_js_1.GRID_SIZE) {
        ...
        scenario.resourcePlacements.forEach(placement => {
            const worldX = placement.x * gameConstants_js_1.TILE_SIZE;
            const worldY = placement.y * gameConstants_js_1.TILE_SIZE;
            resourceNodes.push({
                x: worldX,
                y: worldY,
                type: placement.type,
    ...
    testScenario(name, config) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const blueStart = { x: config.blueSpawn.x * gameConstants_js_1.TILE_SIZE, y: config.blueSpawn.y * gameConstants_js_1.TILE_SIZE };
            const redStart = { x: config.redSpawn.x * gameConstants_js_1.TILE_SIZE, y: config.redSpawn.y * gameConstants_js_1.TILE_SIZE };
            ...
            resourceNodes.forEach((node, i) => {
                const distToBlue = Math.sqrt(Math.pow((node.x - blueStart.x), 2) + Math.pow((node.y - blueStart.y), 2));
                const distToRed = Math.sqrt(Math.pow((node.x - redStart.x), 2) + Math.pow((node.y - redStart.y), 2));
    ...
    calculateTeamStats(team, units, buildings, resources) {
        ...
        return {
            ...
            commander: ((_a = teamUnits.find(u => u.type === unitTypes_js_1.UNIT_TYPES.commander)) === null || _a === void 0 ? void 0 : _a.hp) || 0
        };
    }
    calculatePathingEfficiency(config, results) {
        var _a;
        // Analyze how terrain affected expansion efficiency
        const firstExtractorTime = ((_a = results.timeline.find(e => e.event.includes('extractor'))) === null || _a === void 0 ? void 0 : _a.time) || 999;
        const resourceDistance = Math.min(...config.resourcePlacements.map(r => Math.sqrt(Math.pow((r.x * gameConstants_js_1.TILE_SIZE - config.blueSpawn.x * gameConstants_js_1.TILE_SIZE), 2) +
            Math.pow((r.y * gameConstants_js_1.TILE_SIZE - config.blueSpawn.y * gameConstants_js_1.TILE_SIZE), 2))));
        return {
            firstExtractorTime,
    ...
    runStandardTests() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            yield this.testScenario('CLOSE_RESOURCES', {
                blueSpawn: { x: 10, y: 25 },
                redSpawn: { x: 40, y: 25 },
                resourcePlacements: [
            ...
            yield this.testScenario('DISTANT_RESOURCES', {
                blueSpawn: { x: 10, y: 25 },
                redSpawn: { x: 40, y: 25 },
                resourcePlacements: [
            ...
            yield this.testScenario('WATER_BARRIER', {
                blueSpawn: { x: 10, y: 25 },
                redSpawn: { x: 40, y: 25 },
            ...
            yield this.testScenario('RESOURCE_SCARCITY', {
                startingMass: 50,
                startingEnergy: 75,
                blueSpawn: { x: 10, y: 25 },
                redSpawn: { x: 40, y: 25 },
...
exports.ScenarioTester = ScenarioTester;
// CLI interface
