...
function drawMinimap(minimapRenderContext) {
    const { minimapCtx, terrain, resourceNodes, units, buildings, camera } = minimapRenderContext;
    if (!minimapCtx) {
        return;
    }
    const MINIMAP_SIZE = 200;
    const scale = MINIMAP_SIZE / gameConstants_js_1.WORLD_SIZE;
...
function drawTerrainGrid(ctx, terrain, scale, size) {
    ...
    const gridStep = Math.max(1, Math.floor(gameConstants_js_1.GRID_SIZE / 50)); // Optimize drawing
    for (let x = 0; x < gameConstants_js_1.GRID_SIZE; x += gridStep) {
        for (let y = 0; y < gameConstants_js_1.GRID_SIZE; y += gridStep) {
            ...
            const pixelX = (x / gameConstants_js_1.GRID_SIZE) * size;
            const pixelY = (y / gameConstants_js_1.GRID_SIZE) * size;
            const pixelSize = (gridStep / gameConstants_js_1.GRID_SIZE) * size;
