...
const createMockUnit = (config) => {
    ...
    return Object.assign({ id: unitId, team: 'blue', type: UNIT_TYPES_mock.tank, hp: (config.type && config.type.hp) || UNIT_TYPES_mock.tank.hp, x: 0, y: 0, target: null, patrolTarget: null, commandGroup: null }, config);
};
...
class CommandGroup {
    ...
    getRank(unit) {
        ...
        if (unit.type.damage > 30 || unit.type.support)
            return CommandRank.CAPTAIN;
        if (unit.type.speed > 2.5)
    ...
    getUnitRole(unit) {
        ...
        if (unit.type.domain === 'air')
            return UnitRole.AIR;
        if (unit.type.name.includes('AA') || unit.type.name.includes('Anti-Air'))
            return UnitRole.ANTI_AIR;
        if (unit.type.movementType === 'amphibious')
    ...
    findNearestEnemy(gameContext) {
        ...
        if (this._mockNearestEnemy)
            return this._mockNearestEnemy;
        return null;
    }
    update(gameContext) {
        ...
        if (!this.members.includes(this.leader))
            this.leader = this.members[0] || null; // Basic leader maintenance
        if (this.leader)
            this.executeMission(gameContext); // Call executeMission if leader exists
        return true;
    }
    ...
    scoreStrikeTarget(target, gameContext) {
        ...
        const aaThreats = gameContext.units.filter(u => u.team !== this.leader.team &&
            ...
            Math.sqrt(Math.pow(u.x - target.x, 2) + Math.pow(u.y - target.y, 2)) < 300).length;
...
function runCommandHierarchyTests() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        test('CommandGroup.executeHunt: Avoid Overkill - Weak Target', () => {
            ...
            let focusedCount = 0;
            group.members.forEach(m => { if (m.target === weakTarget)
                focusedCount++; });
            ...
            group.members.forEach((m, idx) => {
                if (m.target !== weakTarget)
                    assert(m.patrolTarget !== null, `Member ${idx} not targeting weak unit should have patrol target`);
            });
        });
        test('CommandGroup.executeHunt: Avoid Overkill - Commander Target', () => {
            ...
            let focusedCount = 0;
            group.members.forEach(m => { if (m.target === weakCommander)
                focusedCount++; });
        ...
        if (failed_ch > 0)
            throw new Error(`${failed_ch} tests failed in commandHierarchy.js overall.`);
    });
}
// Trigger test run
