...
exports.CommandHierarchy = exports.CommandGroup = exports.UnitRole = exports.MissionType = exports.CommandRank = void 0;
const strategicAI_js_1 = require("./strategicAI.js");
// calculateBasePower is used by calculateAssetScore, so it doesn't need to be explicitly imported.
const unitTypes_js_1 = require("../config/unitTypes.js"); // Added for UNIT_TYPES.commander access
const strategicAI_js_2 = require("./strategicAI.js"); // Needed for groupPower calculation
...
class CommandGroup {
    constructor(leader, mission = exports.MissionType.SEARCH, priority = 1) {
    ...
    getMaxSizeForMission(mission) {
        switch (mission) {
            case exports.MissionType.SEARCH: return 3; // Small, fast scout groups
            case exports.MissionType.HUNT: return 6; // Medium attack squads
            case exports.MissionType.COLLECT: return 8; // Large resource protection
            case exports.MissionType.DEFEND: return 10; // Large defensive formations
            case exports.MissionType.ASSAULT: return 12; // Major attack force
            case exports.MissionType.SUPPORT: return 4; // Engineering teams
            case exports.MissionType.SORTIE: return 5; // Air strike squadrons
            default: return 5;
        }
    }
    requiresDiversity(mission) {
        // Missions that benefit from rock-paper-scissors balance
        return [exports.MissionType.DEFEND, exports.MissionType.ASSAULT, exports.MissionType.HUNT].includes(mission);
    }
    getOptimalComposition(mission) {
        // Define optimal unit role distribution for different missions
        const compositions = {
            [exports.MissionType.DEFEND]: {
                [exports.UnitRole.ANTI_AIR]: 0.3, // Strong AA defense
                [exports.UnitRole.GROUND]: 0.4, // Ground defense backbone
                [exports.UnitRole.AIR]: 0.2, // Air patrol
                [exports.UnitRole.SUPPORT]: 0.1 // Repairs and support
            },
            [exports.MissionType.ASSAULT]: {
                [exports.UnitRole.AIR]: 0.3, // Air superiority
                [exports.UnitRole.GROUND]: 0.5, // Main assault force
                [exports.UnitRole.ANTI_AIR]: 0.2 // AA protection
            },
            [exports.MissionType.HUNT]: {
                [exports.UnitRole.AIR]: 0.4, // Fast pursuit
                [exports.UnitRole.GROUND]: 0.4, // Ground pursuit
                [exports.UnitRole.ANTI_AIR]: 0.2 // Counter enemy air
            },
            [exports.MissionType.SORTIE]: {
                [exports.UnitRole.AIR]: 1.0 // Pure air mission
            },
            [exports.MissionType.SEARCH]: {
                [exports.UnitRole.AIR]: 0.6, // Fast reconnaissance
                [exports.UnitRole.GROUND]: 0.4 // Ground confirmation
            }
        };
        return compositions[mission] || {
            [exports.UnitRole.GROUND]: 0.6,
            [exports.UnitRole.AIR]: 0.2,
            [exports.UnitRole.ANTI_AIR]: 0.2
        };
    }
    getUnitRole(unit) {
        if (!unit.type)
            return exports.UnitRole.GROUND;
        if (unit.type.domain === 'air') {
            return exports.UnitRole.AIR;
        }
        else if (unit.type.name.includes('AA') || unit.type.name.includes('Anti-Air')) {
            return exports.UnitRole.ANTI_AIR;
        }
        else if (unit.type.movementType === 'amphibious') {
            return exports.UnitRole.AMPHIBIOUS;
        }
        else if (unit.type.support) {
            return exports.UnitRole.SUPPORT;
        }
        else {
            return exports.UnitRole.GROUND;
        }
    ...
    getRank(unit) {
        if (!unit.type)
            return exports.CommandRank.PRIVATE;
        if (unit.type.name === 'Commander' || unit.type.name === 'ACU') {
            return exports.CommandRank.COMMANDER;
        }
        else if (unit.type.tier >= 3) {
            return exports.CommandRank.COLONEL;
        }
        else if (unit.type.tier >= 2) {
            return exports.CommandRank.MAJOR;
        }
        else if (unit.type.damage > 30 || unit.type.support) {
            return exports.CommandRank.CAPTAIN;
        }
        else if (unit.type.speed > 2.5) {
            return exports.CommandRank.LIEUTENANT;
        }
        else {
            return exports.CommandRank.PRIVATE;
        }
    ...
    update(gameContext) {
        if (this.members.length === 0)
            return false; // Disband empty group
        // Remove dead members
    ...
    executeMission(gameContext) {
        switch (this.mission) {
            case exports.MissionType.SEARCH:
                this.executeSearch(gameContext);
                break;
            case exports.MissionType.HUNT:
                this.executeHunt(gameContext);
                break;
            case exports.MissionType.COLLECT:
                this.executeCollect(gameContext);
                break;
            case exports.MissionType.DEFEND:
                this.executeDefend(gameContext);
                break;
            case exports.MissionType.ASSAULT:
                this.executeAssault(gameContext);
                break;
            case exports.MissionType.SUPPORT:
                this.executeSupport(gameContext);
                break;
            case exports.MissionType.SORTIE:
    ...
    executeHunt(gameContext) {
        ...
        if (this.target) {
            const targetAssetScore = (0, strategicAI_js_1.calculateAssetScore)(this.target);
            const groupPower = this.members.reduce((sum, member) => sum + (0, strategicAI_js_2.calculateBasePower)(member), 0);
            // Avoid pointless attack if group is significantly weaker than the target, unless target is Commander
            if (this.target.type !== unitTypes_js_1.UNIT_TYPES.commander &&
                ...
                this.mission = exports.MissionType.SEARCH; // Revert to search to find a more suitable engagement
            }
            // Proceed with target assignment if target is still valid (not cleared by pointless attack check)
            if (this.target) {
                const targetMaxHP = (0, strategicAI_js_1.getUnitMaxHP)(this.target);
                const targetRH = (0, strategicAI_js_1.calculateRelativeHealth)(this.target, targetMaxHP);
                if (this.members.length >= MIN_GROUP_SIZE_FOR_OVERKILL_ADJUSTMENT &&
                    ...
                    this.target.type && this.target.type.name !== unitTypes_js_1.UNIT_TYPES.commander.name // Avoid overkill logic for commanders
                ) {
    ...
    executeCollect(gameContext) {
        ...
        const friendlyNodes = resourceNodes.filter(node => node.occupied && this.isNearFriendlyBase(node, gameContext));
    ...
    executeDefend(gameContext) {
        ...
        const importantBuildings = gameContext.buildings.filter(b => b.team === this.leader.team &&
    ...
    executeSupport(gameContext) {
        ...
        const damagedBuildings = gameContext.buildings.filter(b => b.team === this.leader.team && b.hp < b.maxHp);
    ...
    initializeSortie(gameContext) {
        ...
        if (this.strikeTarget) {
            ...
            this.mission = exports.MissionType.SEARCH;
        }
    }
    sortieStagingPhase(gameContext) {
        ...
        const allReady = this.members.every(member => {
            const distance = Math.sqrt(Math.pow((member.x - this.homeBase.x), 2) + Math.pow((member.y - this.homeBase.y), 2));
            return distance < 150;
        });
        if (allReady) {
            this.sortiePhase = 'approach';
    ...
    sortieApproachPhase(gameContext) {
        ...
        const leaderDistance = Math.sqrt(Math.pow((this.leader.x - this.strikeTarget.x), 2) +
            Math.pow((this.leader.y - this.strikeTarget.y), 2));
    ...
    sortieReturnPhase(gameContext) {
        ...
        if (allReturned) {
            ...
            this.mission = Math.random() < 0.6 ? exports.MissionType.SEARCH : exports.MissionType.HUNT;
            this.sortiePhase = 'staging';
            this.status = 'reformed';
        }
    }
    findAirBase(gameContext) {
        const airFactories = gameContext.buildings.filter(b => b.team === this.leader.team && b.type.name.includes('Air'));
        if (airFactories.length > 0) {
            // Find closest air factory to leader
            let closest = airFactories[0];
            let closestDistance = Math.sqrt(Math.pow((closest.x - this.leader.x), 2) + Math.pow((closest.y - this.leader.y), 2));
            for (const factory of airFactories) {
                const distance = Math.sqrt(Math.pow((factory.x - this.leader.x), 2) + Math.pow((factory.y - this.leader.y), 2));
    ...
    scoreStrikeTarget(target, gameContext) {
        let newScore = (0, strategicAI_js_1.calculateAssetScore)(target);
        ...
        const aaThreats = gameContext.units.filter(u => u.team !== this.leader.team &&
            this.getUnitRole(u) === exports.UnitRole.ANTI_AIR &&
            Math.sqrt(Math.pow((u.x - target.x), 2) + Math.pow((u.y - target.y), 2)) < 300 // AA check radius
        ).length;
    ...
    findBestTargetForAirUnit(airUnit, gameContext) {
        const enemies = gameContext.units.filter(u => u.team !== airUnit.team &&
            u.hp > 0 && // Ensure target is alive
            Math.sqrt(Math.pow((u.x - airUnit.x), 2) + Math.pow((u.y - airUnit.y), 2)) < (airUnit.type.range || 100) * 1.5 // Use unit's range
        );
        if (enemies.length === 0)
            return null;
        let bestTarget = null;
        ...
        for (const enemy of enemies) {
            let newScore = (0, strategicAI_js_1.calculateAssetScore)(enemy);
            const enemyRole = this.getUnitRole(enemy);
            // Apply role-based adjustments
            if (enemyRole === exports.UnitRole.GROUND) {
            ...
            else if (enemyRole === exports.UnitRole.ANTI_AIR) {
                newScore *= 0.5; // Significantly reduce score for AA threats
            }
            else if (enemyRole === exports.UnitRole.AIR) {
    ...
    findNearestEnemy(gameContext) {
        ...
        for (const unit of gameContext.units) {
            if (unit.team !== this.leader.team) {
                const distance = Math.sqrt(Math.pow((unit.x - leaderPos.x), 2) + Math.pow((unit.y - leaderPos.y), 2));
    ...
    isNearFriendlyBase(node, gameContext) {
        ...
        return friendlyBuildings.some(building => {
            const distance = Math.sqrt(Math.pow((building.x - node.x), 2) + Math.pow((building.y - node.y), 2));
            return distance < 300;
        });
    }
    ...
    maintainFormation(gameContext) {
        ...
        this.members.forEach(member => {
            if (member === this.leader)
                return;
            const distance = Math.sqrt(Math.pow((member.x - leaderPos.x), 2) + Math.pow((member.y - leaderPos.y), 2));
...
class CommandHierarchy {
    constructor(team) {
        ...
        this.missionWeights = {
            [exports.MissionType.SEARCH]: 0.25,
            [exports.MissionType.HUNT]: 0.35,
            [exports.MissionType.COLLECT]: 0.15,
            [exports.MissionType.DEFEND]: 0.15,
            [exports.MissionType.ASSAULT]: 0.10
        };
    }
    update(gameContext) {
        const now = Date.now();
    ...
    selectMission(leader, gameContext) {
        ...
        if (leader.type.name === 'Commander') {
            return exports.MissionType.ASSAULT;
        }
        else if (leader.type.support) {
            return exports.MissionType.SUPPORT;
        }
        else if (rank >= exports.CommandRank.MAJOR) {
            return Math.random() < 0.6 ? exports.MissionType.HUNT : exports.MissionType.ASSAULT;
        }
        else if (leader.type.speed > 2.5) {
            return exports.MissionType.SEARCH;
        }
        else {
            // Random based on current needs
            const missions = [exports.MissionType.HUNT, exports.MissionType.COLLECT, exports.MissionType.DEFEND];
    ...
    scoreCandidateForGroup(unit, group) {
        ...
        const distance = Math.sqrt(Math.pow((unit.x - group.leader.x), 2) + Math.pow((unit.y - group.leader.y), 2));
        score += Math.max(0, 3 - distance / 100);
        return score;
    }
    balanceMissionDistribution(gameContext) {
        const missionCounts = {};
        Object.values(exports.MissionType).forEach(mission => missionCounts[mission] = 0);
    ...
    getDistribution() {
        ...
        // Initialize counters
        Object.values(exports.MissionType).forEach(mission => distribution.missions[mission] = 0);
        Object.values(exports.CommandRank).forEach(rank => distribution.ranks[rank] = 0);
...
exports.CommandHierarchy = CommandHierarchy;
