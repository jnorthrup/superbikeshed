...
class InputManager {
    ...
    handleLeftClick(worldX, worldY, shiftKey = false) {
        ...
        if (clickedEntity) {
            ...
            if (!clickedEntity.id)
                clickedEntity.id = entityId; // Assign temporary ID if missing for this example
            const command = {
                type: commands_js_1.GameCommandTypes.PLAYER_COMMAND_SELECT_UNIT,
                payload: {
                    unitId: entityId,
            ...
            if (!shiftKey) {
                const command = { type: commands_js_1.GameCommandTypes.PLAYER_COMMAND_DESELECT_ALL, payload: {} };
                this.simulation.dispatchCommand(command);
                this.selectedUnitIds.clear();
    ...
    handleRightClick(worldX, worldY) {
        ...
        if (this.selectedUnitIds.size > 0) {
            ...
            const command = {
                type: commands_js_1.GameCommandTypes.PLAYER_COMMAND_MOVE_UNITS, // Changed from PLAYER_COMMAND_UNIT_MOVE
                payload: {
        ...
        switch (key.toLowerCase()) {
            case 'p': // Example: Toggle Pause
                ...
                this.simulation.dispatchCommand({
                    type: commands_js_1.GameCommandTypes.SET_PAUSED,
    ...
    _findClickedEntity(worldX, worldY) {
        ...
        for (const entity of entities) {
            const size = ((_a = entity.type) === null || _a === void 0 ? void 0 : _a.size) || 20; // Default size if not specified
