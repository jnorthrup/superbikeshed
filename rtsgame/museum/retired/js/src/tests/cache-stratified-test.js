...
const spatialIndex_js_1 = require("../core/spatialIndex.js");
const batchProcessor_js_1 = require("../core/batchProcessor.js");
...
const spatialIndex = new spatialIndex_js_1.SpatialIndex(gameConstants_js_1.WORLD_SIZE, 64); // 64x64 cell size as per README
...
const batchProcessor = new batchProcessor_js_1.BatchProcessor();
...
for (let i = 0; i < 10; i++) {
    ...
    if (i < 2) {
        ...
        units.push(new unit_js_1.Unit(x, y, team, unitTypes_js_1.UNIT_TYPES.commander));
    }
    else {
        // Regular units
        units.push(new unit_js_1.Unit(x, y, team, unitTypes_js_1.UNIT_TYPES.tank || unitTypes_js_1.UNIT_TYPES.infantry));
    }
}
// Add some buildings
for (let i = 0; i < 5; i++) {
    ...
    const y = Math.random() * gameConstants_js_1.WORLD_SIZE;
    const team = i % 2 === 0 ? 'blue' : 'red';
    buildings.push(new building_js_1.Building(x, y, team, buildingTypes_js_1.BUILDING_TYPES.landFactory));
}
...
const gameContext = {
    ...
    mainGameGlobals: { resources, units, Unit: unit_js_1.Unit, Building: building_js_1.Building, addEvent: () => { } }
};
