...
const gameConstants_js_1 = require("../config/gameConstants.js"); // Import necessary constants
const unitTypes_js_1 = require("../config/unitTypes.js");
const ui_js_1 = require("./ui.js"); // Import updateUI
...
function drawMinimap(gameContext) {
    ...
    const scale = 200 / gameConstants_js_1.GRID_SIZE;
    for (let x = 0; x < gameConstants_js_1.GRID_SIZE; x += 2) {
    ...
    for (const node of resourceNodes) {
        if (node.amount > 0) {
            ...
            const mx = (node.x / gameConstants_js_1.WORLD_SIZE) * 200;
            const my = (node.y / gameConstants_js_1.WORLD_SIZE) * 200;
    ...
    for (const unit of units) {
        ...
        const mx = (unit.x / gameConstants_js_1.WORLD_SIZE) * 200;
        const my = (unit.y / gameConstants_js_1.WORLD_SIZE) * 200;
        minimapCtx.fillRect(mx - 1, my - 1, 2, 2);
    }
    // Draw buildings on minimap
    for (const building of buildings) {
        minimapCtx.fillStyle = building.team === 'blue' ? '#88f' : '#f88';
        const mx = (building.x / gameConstants_js_1.WORLD_SIZE) * 200;
        const my = (building.y / gameConstants_js_1.WORLD_SIZE) * 200;
    ...
    for (const unit of units) {
        if (unit.type === unitTypes_js_1.UNIT_TYPES.commander) { // UNIT_TYPES.commander is defined
    ...
    const viewLeft = ((camera.x - camera.canvasWidth / 2 / camera.zoom) / gameConstants_js_1.WORLD_SIZE) * 200;
    const viewTop = ((camera.y - camera.canvasHeight / 2 / camera.zoom) / gameConstants_js_1.WORLD_SIZE) * 200;
    const viewWidth = (camera.canvasWidth / camera.zoom / gameConstants_js_1.WORLD_SIZE) * 200;
    const viewHeight = (camera.canvasHeight / camera.zoom / gameConstants_js_1.WORLD_SIZE) * 200;
...
function render(gameContext) {
    ...
    // Draw terrain
    const startX = Math.floor((camera.x - camera.canvasWidth / 2 / camera.zoom) / gameConstants_js_1.TILE_SIZE);
    const endX = Math.ceil((camera.x + camera.canvasWidth / 2 / camera.zoom) / gameConstants_js_1.TILE_SIZE);
    const startY = Math.floor((camera.y - camera.canvasHeight / 2 / camera.zoom) / gameConstants_js_1.TILE_SIZE);
    const endY = Math.ceil((camera.y + camera.canvasHeight / 2 / camera.zoom) / gameConstants_js_1.TILE_SIZE);
    for (let x = Math.max(0, startX); x < Math.min(gameConstants_js_1.GRID_SIZE, endX); x++) {
        for (let y = Math.max(0, startY); y < Math.min(gameConstants_js_1.GRID_SIZE, endY); y++) {
            if (!terrain[x] || terrain[x][y] === undefined)
                continue;
            const screenX = (x * gameConstants_js_1.TILE_SIZE - camera.x) * camera.zoom + camera.canvasWidth / 2;
            const screenY = (y * gameConstants_js_1.TILE_SIZE - camera.y) * camera.zoom + camera.canvasHeight / 2;
            const size = gameConstants_js_1.TILE_SIZE * camera.zoom;
            switch (terrain[x][y]) {
                case gameConstants_js_1.TERRAIN_TYPES.WATER:
    ...
    (0, ui_js_1.updateUI)(gameContext);
