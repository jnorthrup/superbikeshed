...
const simulation_js_1 = require("../core/simulation.js"); // Updated import path
function updateUI(gameContext) {
    ...
    if (blueUnitsEl)
        blueUnitsEl.textContent = blueUnits;
    const redUnitsEl = document.getElementById('redUnits');
    if (redUnitsEl)
        redUnitsEl.textContent = redUnits;
    ...
    if (blueMassEl)
        blueMassEl.textContent = Math.floor(resources.blue.mass || 0);
    const blueMassIncomeEl = document.getElementById('blue-mass-income');
    if (blueMassIncomeEl)
        blueMassIncomeEl.textContent = Math.floor(resources.blue.massIncome || 0);
    const blueEnergyEl = document.getElementById('blue-energy');
    if (blueEnergyEl)
        blueEnergyEl.textContent = Math.floor(resources.blue.energy || 0);
    const blueEnergyIncomeEl = document.getElementById('blue-energy-income');
    if (blueEnergyIncomeEl)
        blueEnergyIncomeEl.textContent = Math.floor(resources.blue.energyIncome || 0);
    const redMassEl = document.getElementById('red-mass');
    if (redMassEl)
        redMassEl.textContent = Math.floor(resources.red.mass || 0);
    const redMassIncomeEl = document.getElementById('red-mass-income');
    if (redMassIncomeEl)
        redMassIncomeEl.textContent = Math.floor(resources.red.massIncome || 0);
    const redEnergyEl = document.getElementById('red-energy');
    if (redEnergyEl)
        redEnergyEl.textContent = Math.floor(resources.red.energy || 0);
    const redEnergyIncomeEl = document.getElementById('red-energy-income');
    if (redEnergyIncomeEl)
        redEnergyIncomeEl.textContent = Math.floor(resources.red.energyIncome || 0);
    ...
    if (gameTimeEl)
        gameTimeEl.textContent = (0, simulation_js_1.formatTime)(gameState.gameTime);
    // Zoom level
...
function updateStatusWindow(gameContext) {
    ...
    const html = `
        <div class="status-line">
            ...
            <span class="status-value">${(0, simulation_js_1.formatTime)(gameState.gameTime)}</span>
        </div>
...
function getTeamStats(team, units, buildings, resources) {
    ...
    const commander = teamUnits.find(u => u.type === unitTypes_js_1.UNIT_TYPES.commander);
