...
exports.initThreeRenderer = initThreeRenderer;
const THREE = __importStar(require("three"));
const GLTFLoader_js_1 = require("three/addons/loaders/GLTFLoader.js");
const gameConstants_js_1 = require("../config/gameConstants.js"); // Added GRID_SIZE
...
class ThreeRenderer {
    constructor(canvas) {
        ...
        const frustumSize = gameConstants_js_1.WORLD_SIZE * 1.2; // Tighter viewing area for pixel-perfect rendering
        const aspect = canvas.width / canvas.height;
        this.camera = new THREE.OrthographicCamera(-frustumSize * aspect / 2, // left
        frustumSize * aspect / 2, // right
        frustumSize / 2, // top
        -frustumSize / 2, // bottom
        1, // near plane
        gameConstants_js_1.WORLD_SIZE * 4 // far plane - large enough to see entire world
        );
        // Position camera high overhead for perfect top-down RTS view
        this.camera.position.set(0, 0, gameConstants_js_1.WORLD_SIZE * 2); // High enough to see everything
        ...
        directionalLight.shadow.camera.far = gameConstants_js_1.WORLD_SIZE * 3;
        directionalLight.shadow.camera.left = -gameConstants_js_1.WORLD_SIZE;
        directionalLight.shadow.camera.right = gameConstants_js_1.WORLD_SIZE;
        directionalLight.shadow.camera.top = gameConstants_js_1.WORLD_SIZE;
        directionalLight.shadow.camera.bottom = -gameConstants_js_1.WORLD_SIZE;
    ...
    loadModelManifest() {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                this.modelManifest = yield response.json();
    ...
    updateTerrain(terrainData) {
        ...
        const segments = gameConstants_js_1.GRID_SIZE;
        const geometry = new THREE.PlaneGeometry(gameConstants_js_1.WORLD_SIZE, gameConstants_js_1.WORLD_SIZE, segments - 1, segments - 1);
        ...
        const elevationScaleFactor = gameConstants_js_1.TILE_SIZE * 8; // Reduced for more reasonable terrain height
        // Adjust vertices and assign colors based on terrain type
        for (let i = 0; i < vertices.length; i += 3) {
            const x = Math.floor((vertices[i] + gameConstants_js_1.WORLD_SIZE / 2) / gameConstants_js_1.TILE_SIZE);
            const y = Math.floor((vertices[i + 1] + gameConstants_js_1.WORLD_SIZE / 2) / gameConstants_js_1.TILE_SIZE);
            if (terrainData[x] && terrainData[x][y] !== undefined) {
                ...
                switch (tile.type) {
                    case 0: // WATER
                        ...
                        vertices[i + 2] = Math.min(vertices[i + 2], -gameConstants_js_1.TILE_SIZE); // Water below ground
                        break;
                    case 1: // LAND
                    ...
                    case 2: // MOUNTAIN
                        ...
                        vertices[i + 2] += gameConstants_js_1.TILE_SIZE * 4; // Mountains higher
                        break;
                    case 3: // RESOURCE
    ...
    addEnvironmentalObjects(terrainData) {
        ...
        for (let x = 0; x < gameConstants_js_1.GRID_SIZE; x += 2) { // Sample every other tile for performance
            for (let y = 0; y < gameConstants_js_1.GRID_SIZE; y += 2) {
                if (terrainData[x] && terrainData[x][y]) {
                    const tile = terrainData[x][y];
                    const worldX = x * gameConstants_js_1.TILE_SIZE - gameConstants_js_1.WORLD_SIZE / 2;
                    const worldY = y * gameConstants_js_1.TILE_SIZE - gameConstants_js_1.WORLD_SIZE / 2;
                    const worldZ = tile.elevation * gameConstants_js_1.TILE_SIZE * 8;
                    ...
                    if (tile.type === 2 && Math.random() < 0.25) {
                        this.addRock(worldX, worldY, worldZ + gameConstants_js_1.TILE_SIZE * 4);
                        objectCount.rocks++;
                    }
    ...
    addTree(x, y, z) {
        ...
        const trunkGeometry = new THREE.CylinderGeometry(gameConstants_js_1.TILE_SIZE * 0.3, gameConstants_js_1.TILE_SIZE * 0.5, gameConstants_js_1.TILE_SIZE * 2);
        const trunkMaterial = new THREE.MeshLambertMaterial({ color: 0x8B4513 }); // Brown
        const trunk = new THREE.Mesh(trunkGeometry, trunkMaterial);
        const leavesGeometry = new THREE.SphereGeometry(gameConstants_js_1.TILE_SIZE * 1.2);
        const leavesMaterial = new THREE.MeshLambertMaterial({ color: 0x228B22 }); // Green
        const leaves = new THREE.Mesh(leavesGeometry, leavesMaterial);
        leaves.position.y = gameConstants_js_1.TILE_SIZE * 1.5;
        const tree = new THREE.Group();
        tree.add(trunk);
        tree.add(leaves);
        tree.position.set(x, y, z + gameConstants_js_1.TILE_SIZE);
        tree.castShadow = true;
        this.terrainGroup.add(tree);
    }
    addRock(x, y, z) {
        const geometry = new THREE.SphereGeometry(gameConstants_js_1.TILE_SIZE * 0.8, 6, 4); // Low-poly rock
        const material = new THREE.MeshLambertMaterial({ color: 0x696969 }); // Dark gray
        const rock = new THREE.Mesh(geometry, material);
        rock.position.set(x, y, z + gameConstants_js_1.TILE_SIZE * 0.4);
        rock.castShadow = true;
        this.terrainGroup.add(rock);
    }
    addDebris(x, y, z) {
        const geometry = new THREE.BoxGeometry(gameConstants_js_1.TILE_SIZE * 0.3, gameConstants_js_1.TILE_SIZE * 0.3, gameConstants_js_1.TILE_SIZE * 0.2);
        const material = new THREE.MeshLambertMaterial({ color: 0x4A4A4A }); // Dark gray
        const debris = new THREE.Mesh(geometry, material);
        debris.position.set(x, y, z + gameConstants_js_1.TILE_SIZE * 0.1);
        debris.rotation.y = Math.random() * Math.PI;
        this.terrainGroup.add(debris);
    }
    addResourceNode(x, y, z, type) {
        const geometry = new THREE.CylinderGeometry(gameConstants_js_1.TILE_SIZE * 0.8, gameConstants_js_1.TILE_SIZE * 0.8, gameConstants_js_1.TILE_SIZE * 1.5);
        ...
        resource.position.set(x, y, z + gameConstants_js_1.TILE_SIZE * 0.75);
    ...
    updateCamera(cameraData) {
        ...
        const frustumSize = (gameConstants_js_1.WORLD_SIZE * 1.5) / zoom;
        ...
        const camX = (cameraData.x || 0) - gameConstants_js_1.WORLD_SIZE / 2;
        const camY = (cameraData.y || 0) - gameConstants_js_1.WORLD_SIZE / 2;
        ...
        const baseHeight = gameConstants_js_1.WORLD_SIZE * 2;
        const tiltDistance = gameConstants_js_1.WORLD_SIZE * 0.5; // Distance from target when tilted
        if (tiltAngle > 0) {
            // 3D perspective mode
            ...
            this.camera.position.set(camX + offsetX, camY + offsetY, height);
            ...
            if (rotation !== 0) {
                const upVector = new THREE.Vector3(Math.sin(rotation), Math.cos(rotation), 0);
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (this.modelManifest) {
                ...
                if (modelId && this.modelManifest[modelId]) {
                    try {
                        const gltf = yield this.gltfLoader.loadAsync(this.modelManifest[modelId].path);
    ...
    setMeshPosition(mesh, entity) {
        let terrainZ = 0;
        if (this.terrainMesh) {
            const worldX = entity.x - gameConstants_js_1.WORLD_SIZE / 2;
            const worldY = entity.y - gameConstants_js_1.WORLD_SIZE / 2;
            const halfWorld = gameConstants_js_1.WORLD_SIZE / 2;
            const segmentSize = gameConstants_js_1.WORLD_SIZE / (gameConstants_js_1.GRID_SIZE - 1);
            const xIndex = Math.floor((worldX + halfWorld) / segmentSize);
            const yIndex = Math.floor((worldY + halfWorld) / segmentSize);
            const vertexIndex = (yIndex * gameConstants_js_1.GRID_SIZE + xIndex) * 3;
        ...
        mesh.position.set(entity.x - gameConstants_js_1.WORLD_SIZE / 2, entity.y - gameConstants_js_1.WORLD_SIZE / 2, terrainZ);
    }
    createBuildingMesh(building) {
        ...
        for (const [key, value] of Object.entries(modelDefaults_js_1.BUILDING_MODELS)) {
    ...
    render(simulation, gameContext) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (!this.terrainMesh && (simulation === null || simulation === void 0 ? void 0 : simulation.terrain)) {
                this.updateTerrain(simulation.terrain);
            }
            // Handle units with proper caching
            if ((_a = simulation === null || simulation === void 0 ? void 0 : simulation.entityManager) === null || _a === void 0 ? void 0 : _a.units) {
                for (const unit of simulation.entityManager.units) {
                    ...
                    if (!mesh) {
                        ...
                        mesh = yield this.createUnitMesh(unit);
            ...
            if ((_b = simulation === null || simulation === void 0 ? void 0 : simulation.entityManager) === null || _b === void 0 ? void 0 : _b.buildings) {
    ...
    resize(width, height) {
        ...
        const frustumSize = gameConstants_js_1.WORLD_SIZE * 1.5;
        this.camera.left = -frustumSize * aspect / 2;
        this.camera.right = frustumSize * aspect / 2;
        this.camera.top = frustumSize / 2;
        this.camera.bottom = -frustumSize / 2;
...
function initThreeRenderer(canvas) {
    return __awaiter(this, void 0, void 0, function* () {
        const renderer = new ThreeRenderer(canvas);
        yield renderer.manifestPromise; // Ensure manifest is loaded before renderer is considered fully initialized
        return renderer;
    });
}
