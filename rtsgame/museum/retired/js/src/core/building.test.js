...
const unit_js_1 = require("./unit.js"); // Needed because buildings create Units
...
describe('Building', () => {
    ...
    beforeEach(() => {
        ...
        buildingTypes_js_1.BUILDING_TYPES.testBuilding = testBuildingType;
        unitTypes_js_1.UNIT_TYPES.testUnit = testProducibleUnitType; // For production
        // For resource generation test
        buildingTypes_js_1.BUILDING_TYPES.massExtractor = Object.assign(Object.assign({}, baseBuildingConfig), { name: 'Mass Extractor', resourceGeneration: { type: 'mass', amount: 1 } // 1 mass per some interval (original code implies per update tick, not per second)
         });
        buildingTypes_js_1.BUILDING_TYPES.energyPlant = Object.assign(Object.assign({}, baseBuildingConfig), { name: 'Energy Plant', resourceGeneration: { type: 'energy', amount: 10 } });
        // For production test
        buildingTypes_js_1.BUILDING_TYPES.factory = Object.assign(Object.assign({}, baseBuildingConfig), { name: 'Factory', produces: [unitTypes_js_1.UNIT_TYPES.testUnit] // Factory can produce testUnit
         });
        // Clear mocks
    ...
    describe('Constructor', () => {
        it('should initialize basic properties', () => {
            const building = new building_js_1.Building(100, 150, 'blue', buildingTypes_js_1.BUILDING_TYPES.testBuilding, mockSim);
            expect(building.x).toBe(100);
            expect(building.y).toBe(150);
            expect(building.team).toBe('blue');
            expect(building.type).toEqual(buildingTypes_js_1.BUILDING_TYPES.testBuilding);
            expect(building.hp).toBe(buildingTypes_js_1.BUILDING_TYPES.testBuilding.maxHp);
    ...
    describe('setRallyPoint', () => {
        ...
        it('should set rally point to a valid land position near the building', () => {
            ...
            mockSim.terrain[Math.floor((160 + 100) / gameConstants_js_1.TILE_SIZE)][Math.floor(160 / gameConstants_js_1.TILE_SIZE)] = gameConstants_js_1.TERRAIN_TYPES.LAND;
            const building = new building_js_1.Building(160, 160, 'blue', buildingTypes_js_1.BUILDING_TYPES.testBuilding, mockSim);
        ...
        it('should fallback to building position if no suitable land rally point is found', () => {
            ...
            for (let i = -4; i <= 4; i++) {
                for (let j = -4; j <= 4; j++) {
                    const xTile = Math.floor(160 / gameConstants_js_1.TILE_SIZE) + i;
                    const yTile = Math.floor(160 / gameConstants_js_1.TILE_SIZE) + j;
                    if (xTile >= 0 && xTile < gameConstants_js_1.GRID_SIZE && yTile >= 0 && yTile < gameConstants_js_1.GRID_SIZE) {
                        mockSim.terrain[xTile][yTile] = gameConstants_js_1.TERRAIN_TYPES.WATER;
                    }
                }
            }
            const building = new building_js_1.Building(160, 160, 'blue', buildingTypes_js_1.BUILDING_TYPES.testBuilding, mockSim);
    ...
    describe('Update: Resource Generation', () => {
        it('should generate mass if it is a mass extractor', () => {
            const extractor = new building_js_1.Building(100, 100, 'blue', buildingTypes_js_1.BUILDING_TYPES.massExtractor, mockSim);
            const initialMass = mockSim.gameState.resources.blue.mass;
            const generationAmount = buildingTypes_js_1.BUILDING_TYPES.massExtractor.resourceGeneration.amount;
        ...
        it('should generate energy if it is an energy plant', () => {
            const plant = new building_js_1.Building(100, 100, 'red', buildingTypes_js_1.BUILDING_TYPES.energyPlant, mockSim);
            const initialEnergy = mockSim.gameState.resources.red.energy;
            const generationAmount = buildingTypes_js_1.BUILDING_TYPES.energyPlant.resourceGeneration.amount;
    ...
    describe('Update: Production', () => {
        let factory;
        const unitToProduce = unitTypes_js_1.UNIT_TYPES.testUnit; // Has cost {mass:25, energy:25}, buildTime: 5s
        beforeEach(() => {
            factory = new building_js_1.Building(100, 100, 'blue', buildingTypes_js_1.BUILDING_TYPES.factory, mockSim);
        ...
        it('should progress production and spawn unit when complete', () => {
            ...
            expect(mockSim.entityManager.addUnit).toHaveBeenCalledWith(expect.any(unit_js_1.Unit)); // Check instance of Unit
            ...
            expect(mockSim.entityManager.addCaption).toHaveBeenCalledWith(expect.objectContaining({ text: `${unitToProduce.name} ready!` }));
        });
        it('should show production progress caption', () => {
            ...
            expect(mockSim.entityManager.addCaption).toHaveBeenCalledWith(expect.objectContaining({ text: `Building ${unitToProduce.name} ${progressPercent}%` }));
            expect(factory.captionCooldown).toBe(1.5); // Cooldown set
        });
        it('should not auto-queue if produces list is empty or undefined', () => {
            buildingTypes_js_1.BUILDING_TYPES.testBuildingNoProduce = Object.assign(Object.assign({}, testBuildingType), { produces: [] });
            const buildingNoProduce = new building_js_1.Building(100, 100, 'blue', buildingTypes_js_1.BUILDING_TYPES.testBuildingNoProduce, mockSim);
            buildingNoProduce.update(mockSim, 0.1);
            expect(buildingNoProduce.productionQueue.length).toBe(0);
            buildingTypes_js_1.BUILDING_TYPES.testBuildingNoProduce2 = Object.assign(Object.assign({}, testBuildingType), { produces: undefined });
            const buildingNoProduce2 = new building_js_1.Building(100, 100, 'blue', buildingTypes_js_1.BUILDING_TYPES.testBuildingNoProduce2, mockSim);
