...
const combatConstants_js_1 = require("../config/combatConstants.js");
class CombatSystem {
    constructor() {
    ...
    update(entity, gameContext) {
        if (!entity.combat || !entity.position || !entity.type)
            return;
    ...
    attack(attacker, target, gameContext) {
        ...
        const damageType = type.inflictsDamageType || combatConstants_js_1.DAMAGE_TYPES.KINETIC;
        // Create projectile
        const projectile = {
            x: position.x,
            y: position.y,
        ...
        if (gameContext.captions) {
            gameContext.captions.push(new gameContext.Caption(position.x, position.y, 'Attack!', '#f44', 12));
        }
    }
        ...
        for (let i = gameContext.projectiles.length - 1; i >= 0; i--) {
            ...
            if (distance < 5) { // Projectile reaches target vicinity
                if (projectile.target && projectile.target.health && projectile.target.type) {
                    ...
                    const armorType = targetType.armorType || combatConstants_js_1.ARMOR_TYPES.STANDARD;
                    ...
                    let multiplier = 1.0;
                    if (combatConstants_js_1.DAMAGE_ARMOR_INTERACTIONS[projectileDamageType] &&
                        combatConstants_js_1.DAMAGE_ARMOR_INTERACTIONS[projectileDamageType][armorType] !== undefined) {
                        multiplier = combatConstants_js_1.DAMAGE_ARMOR_INTERACTIONS[projectileDamageType][armorType];
                    }
                    ...
                    let finalDamage = damageAfterMultiplier * (1 - (armorValue / (armorValue + combatConstants_js_1.ARMOR_DAMAGE_REDUCTION_CONSTANT_K)));
                    // 3. Ensure Minimum Damage
                    const minDamage = projectile.initialDamage * combatConstants_js_1.MINIMUM_DAMAGE_PERCENTAGE;
                    ...
                    if (projectileDamageType === combatConstants_js_1.DAMAGE_TYPES.EMP) {
                        ...
                        if (projectile.target.currentEnergy !== undefined) {
                            projectile.target.currentEnergy = Math.max(0, projectile.target.currentEnergy - combatConstants_js_1.EMP_ENERGY_DRAIN_AMOUNT);
                    ...
                    const isDead = projectile.target.health.takeDamage(finalDamage, projectile.attacker, // Pass the original attacker
                    gameContext, projectileDamageType // Pass damage type to takeDamage for shield interaction
                    );
                    if (isDead) {
    ...
    handleTargetDeath(target, gameContext) {
        ...
        if (gameContext.captions && target.position) {
            gameContext.captions.push(new gameContext.Caption(target.position.x, target.position.y, 'Destroyed!', '#f44', 14));
        }
    }
}
