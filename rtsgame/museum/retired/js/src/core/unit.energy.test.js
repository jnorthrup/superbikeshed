...
const createMockUnitType = (config = {}) => (Object.assign({ maxHp: 100, speed: 50, range: 100, attackSpeed: 1.0, damage: 10, 
    // Energy related defaults
    batteryCapacity: 100, generatorOutput: 10, weaponEnergyCost: 10, 
    // Shield related defaults
    maxEnergyShields: 0, shieldRegenRate: DEFAULT_SHIELD_REGEN_RATE, 
    // Weight related defaults
    unitWeight: 0, 
    // Other common properties
    shields: 0, shieldRegen: 0, size: 10, movementType: 'land' }, config));
const createMockUnit = (typeConfig = {}, unitConfig = {}, mockSim = getMockSimulation()) => {
    ...
    unit.getCurrentSpeed = function (simulation = mockSim) {
        var _a, _b;
        // For more complex amphibious tests, this would need the terrain logic from unit.js
        const terrain = simulation.terrain || {};
        const tileX = Math.floor(this.x / (((_a = simulation.gameContext) === null || _a === void 0 ? void 0 : _a.TILE_SIZE) || 32)); // Assuming TILE_SIZE
        const tileY = Math.floor(this.y / (((_b = simulation.gameContext) === null || _b === void 0 ? void 0 : _b.TILE_SIZE) || 32));
    ...
    unit.attack = function (target, simulation = mockSim) {
...
function runUnitEnergyTests() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        test('Unit Update: Weapon Energy Consumption - Sufficient Energy', () => {
            const attacker = createMockUnit({ weaponEnergyCost: 20, attackSpeed: 1.0, range: 50 }, { currentEnergy: 50 });
        ...
        test('Unit Update: Weapon Energy Consumption - Insufficient Energy', () => {
            const attacker = createMockUnit({ weaponEnergyCost: 20, attackSpeed: 1.0, range: 50 }, { currentEnergy: 10 });
        ...
        test('Unit Update: Weapon Energy Consumption - Zero Cost Weapon', () => {
            const attacker = createMockUnit({ weaponEnergyCost: 0, attackSpeed: 1.0, range: 50 }, { currentEnergy: 5 });
            const target = createMockUnit({}, { hp: 100, x: 10, y: 0 });
            attacker.target = target;
            attacker.cooldown = 0;
            attacker.update();
        ...
        test('Unit takeDamage: Energy Shields + Old HP Shields Interaction', () => {
            const target = createMockUnit({ maxEnergyShields: 10, shields: 20 }, // type: 10 ES, 20 HP-Shield
        ...
        if (failed_ue > 0)
            throw new Error(`${failed_ue} tests failed in unit.energy.test.js.`);
    });
}
