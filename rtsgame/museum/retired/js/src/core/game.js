...
exports.performAoeDamage = performAoeDamage;
...
function addEvent(gameContext, type, message, importance = 1) {
    const event = {
        type: type,
        message: message,
...
function placeFactoriesAroundCommander(gameContext, commanderPos, team) {
    ...
    landPos = (0, terrain_js_1.findLandPosition)(gameContext, commanderPos.x + offset, commanderPos.y, factoryAreaSize);
    if (!landPos)
        landPos = (0, terrain_js_1.findLandPosition)(gameContext, commanderPos.x - offset, commanderPos.y, factoryAreaSize);
    if (!landPos) {
    ...
    gameContext.buildings.push(new building_js_1.Building(landPos.x, landPos.y, team, buildingTypes_js_1.BUILDING_TYPES.landFactory, gameContext));
    airPos = (0, terrain_js_1.findLandPosition)(gameContext, commanderPos.x - offset, commanderPos.y - offset, factoryAreaSize);
    if (!airPos)
        airPos = (0, terrain_js_1.findLandPosition)(gameContext, commanderPos.x + offset, commanderPos.y + offset, factoryAreaSize);
    if (!airPos) {
    ...
    gameContext.buildings.push(new building_js_1.Building(airPos.x, airPos.y, team, buildingTypes_js_1.BUILDING_TYPES.airFactory, gameContext));
    navalPos = (0, terrain_js_1.findWaterPosition)(gameContext, commanderPos.x, commanderPos.y + offset * 1.5);
    if (navalPos && navalPos.x !== undefined && navalPos.y !== undefined) {
        ...
        if (distToCommander < 700) {
            gameContext.buildings.push(new building_js_1.Building(navalPos.x, navalPos.y, team, buildingTypes_js_1.BUILDING_TYPES.navalFactory, gameContext));
        }
        else {
    ...
    energyPos1 = (0, terrain_js_1.findLandPosition)(gameContext, commanderPos.x, commanderPos.y - offset, factoryAreaSize);
    if (!energyPos1) {
    ...
    gameContext.buildings.push(new building_js_1.Building(energyPos1.x, energyPos1.y, team, buildingTypes_js_1.BUILDING_TYPES.energyExtractor, gameContext));
}
function initGame(gameContext) {
    ...
    function printTerrainMap(gameContext) {
        if (gameContext.HEADLESS_MODE)
            return; // Skip printing in headless mode
        console.log("=== TERRAIN MAP (L=Land, W=Water, M=Mountain) ===");
        let mapStr = "";
        for (let y = 0; y < gameConstants_js_1.GRID_SIZE; y += 2) { // Show every 2nd row for readability
            let row = "";
            for (let x = 0; x < gameConstants_js_1.GRID_SIZE; x += 2) { // Show every 2nd column
                if (gameContext.terrain[x] && gameContext.terrain[x][y] !== undefined) {
                    const terrain = gameContext.terrain[x][y];
                    if (terrain === gameConstants_js_1.TERRAIN_TYPES.LAND)
                        row += "L";
                    else if (terrain === gameConstants_js_1.TERRAIN_TYPES.WATER)
                        row += "W";
                    else if (terrain === gameConstants_js_1.TERRAIN_TYPES.MOUNTAIN)
                        row += "M";
                    else
                        row += "?";
                }
    ...
    for (let i = 0; i < MAX_INIT_RETRIES; i++) {
        ...
        (0, terrainManager_js_1.generateTerrain)(gameContext, 'perlinNoiseGenerator'); // Specify the terrain generator
        blueStart = (0, terrain_js_1.findLandPosition)(gameContext, gameConstants_js_1.WORLD_SIZE * 0.2, gameConstants_js_1.WORLD_SIZE * 0.5, COMMANDER_START_AREA_SIZE);
        redStart = (0, terrain_js_1.findLandPosition)(gameContext, gameConstants_js_1.WORLD_SIZE * 0.8, gameConstants_js_1.WORLD_SIZE * 0.5, COMMANDER_START_AREA_SIZE);
    ...
    if (!blueStart) {
        ...
        for (let i = 0; i < FOCUSED_PLACEMENT_RETRIES; i++) {
            ...
            (0, terrainManager_js_1.generateTerrain)(gameContext, 'perlinNoiseGenerator');
            blueStart = (0, terrain_js_1.findLandPosition)(gameContext, gameConstants_js_1.WORLD_SIZE * 0.2, gameConstants_js_1.WORLD_SIZE * 0.5, COMMANDER_START_AREA_SIZE);
            if (blueStart) {
    ...
    if (!redStart) {
        ...
        for (let i = 0; i < FOCUSED_PLACEMENT_RETRIES; i++) {
            ...
            (0, terrainManager_js_1.generateTerrain)(gameContext, 'perlinNoiseGenerator');
            if (blueStart && !(0, terrain_js_1.findLandPosition)(gameContext, blueStart.x, blueStart.y, COMMANDER_START_AREA_SIZE)) {
    ...
    if (!blueStart) {
        ...
        blueStart = { x: gameConstants_js_1.WORLD_SIZE * 0.2, y: gameConstants_js_1.WORLD_SIZE * 0.5 };
    }
    if (!redStart) {
        ...
        redStart = { x: gameConstants_js_1.WORLD_SIZE * 0.8, y: gameConstants_js_1.WORLD_SIZE * 0.5 };
    }
    if (unitTypes_js_1.UNIT_TYPES.commander) {
        gameContext.units.push(new unit_js_1.Unit(blueStart.x, blueStart.y, 'blue', unitTypes_js_1.UNIT_TYPES.commander, gameContext)); // Pass gameContext to Unit constructor
        gameContext.units.push(new unit_js_1.Unit(redStart.x, redStart.y, 'red', unitTypes_js_1.UNIT_TYPES.commander, gameContext)); // Pass gameContext to Unit constructor
        addEvent(gameContext, 'strategic', 'Commanders deployed!', 3);
    }
    else {
        console.error("UNIT_TYPES.commander is not defined! Cannot spawn ACUs.");
    }
    gameContext.camera.x = gameConstants_js_1.WORLD_SIZE / 2;
    gameContext.camera.y = gameConstants_js_1.WORLD_SIZE / 2;
...
function update(gameContext) {
    if (gameContext.gameState.paused || gameContext.gameState.winner)
        return;
    ...
    for (let i = gameContext.units.length - 1; i >= 0; i--) {
        ...
        if (unit.hp <= 0) {
            if (unit.type === unitTypes_js_1.UNIT_TYPES.commander) {
                ...
                gameContext.captions.push(new caption_js_1.Caption(unit.x, unit.y, `${unit.team.toUpperCase()} COMMANDER DESTROYED!`, '#ff0', 20));
    ...
    for (let i = gameContext.buildings.length - 1; i >= 0; i--) {
        ...
        if (building.hp <= 0) {
            if (building.type.produces || building.type.resourceGeneration) {
                gameContext.captions.push(new caption_js_1.Caption(building.x, building.y, 'Structure lost!', '#f88', 12));
    ...
    (0, strategicAI_js_1.makeStrategicDecisions)(gameContext); // This function will need to use seedRandom.random()
    (0, strategicAI_js_1.coordinateAttacks)(gameContext); // This function will need to use seedRandom.random()
...
function gameLoop(timestamp, gameContext) {
    if (!gameContext.lastFpsTime)
        gameContext.lastFpsTime = 0;
    if (!gameContext.frameCount)
...
function performAoeDamage(centerX, centerY, radius, damageAmount, ownerTeam, gameContext) {
