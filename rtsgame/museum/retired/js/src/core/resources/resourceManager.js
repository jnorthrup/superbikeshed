...
class ResourceManager {
    ...
    initialize(gameContext) {
        this.resourceNodes = gameContext.terrain
            .flatMap((row, y) => row.map((tile, x) => tile === 'RESOURCE' ? { x, y, type: 'mass' } : null))
    ...
    findNearestResourceNode(unit) {
        ...
        return (_a = this.resourceNodes
            ...
            return !nearest || distance < nearest.distance
                ? { node, distance }
                : nearest;
        }, null)) === null || _a === void 0 ? void 0 : _a.node;
    }
    ...
    buildExtractor(engineer, node, gameContext) {
        ...
        if (gameContext.captions) {
            gameContext.captions.push(new gameContext.Caption(node.x, node.y, 'Extractor Built', '#4f4', 12));
        }
    }
