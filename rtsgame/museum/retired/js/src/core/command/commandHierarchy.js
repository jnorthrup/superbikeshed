...
exports.CommandHierarchy = void 0;
const authoritySystem_js_1 = require("./authoritySystem.js");
class CommandHierarchy {
    constructor() {
        this.authoritySystem = new authoritySystem_js_1.AuthoritySystem();
        this.commandRadius = {
    ...
    getCommandRadius(unit) {
        ...
        return baseRadius + (((_a = veterancyData === null || veterancyData === void 0 ? void 0 : veterancyData.bonuses) === null || _a === void 0 ? void 0 : _a.commandRadius) || 0);
    }
    getBaseCommandRadius(unit) {
        const authority = this.authoritySystem.calculateEffectiveAuthority(unit).effectiveAuthority;
        if (authority >= 30)
            return this.commandRadius.SUPREME;
        if (authority >= 20)
            return this.commandRadius.FIELD;
        if (authority >= 15)
            return this.commandRadius.SQUAD;
    ...
    findCommander(unit, sortedUnits) {
        ...
        for (const potentialCommander of sortedUnits) {
            if (potentialCommander.id === unit.id)
                continue;
            ...
            if (distance > commanderNode.commandRadius)
                continue;
            // Check if commander has capacity
            if (commanderNode.subordinates.size >= this.getMaxSubordinates(potentialCommander))
                continue;
            // Check authority
    ...
    getMaxSubordinates(unit) {
        const authority = this.authoritySystem.calculateEffectiveAuthority(unit).effectiveAuthority;
        if (authority >= 30)
            return 50; // Supreme Commander
        if (authority >= 20)
            return 25; // Field Commander
        if (authority >= 15)
            return 10; // Squad Leader
        return 0; // Individual
    }
