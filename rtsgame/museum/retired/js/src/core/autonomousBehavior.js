...
exports.AutonomousBehavior = void 0;
...
class AutonomousBehavior {
    ...
    updateEngineerBehavior() {
        ...
        const resourceNode = resourceManager.findNearestResourceNode(this.unit.position, gameConstants_js_1.RESOURCE_TYPES.MASS);
        if (resourceNode) {
    ...
    setBehavior(type, state) {
        this.currentBehavior = Object.assign({ type }, state);
    ...
    findDamagedBuilding() {
        ...
        return buildings.find(building => building.team === this.unit.team &&
    ...
    findCommander() {
        ...
        return units.find(unit => unit.team === this.unit.team &&
            unit.type === 'commander');
    }
    /**
     * Find patrol target
    ...
    executeEvadeBehavior(deltaTime) {
        ...
        const angle = Math.atan2(this.unit.y - target.y, this.unit.x - target.x);
        const distance = 200;
    ...
    executeExploreBehavior(deltaTime) {
        ...
        this.unit.moveTo(this.behaviorState.targetPosition.x, this.behaviorState.targetPosition.y);
    }
    /**
...
exports.AutonomousBehavior = AutonomousBehavior;
