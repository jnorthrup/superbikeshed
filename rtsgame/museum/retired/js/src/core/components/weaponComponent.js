...
class WeaponComponent {
    ...
    fire(source, target, gameContext) {
        ...
        if (accuracyRoll > this.accuracy) {
            ...
            const targetAngle = Math.atan2(target.position.y - source.position.y, target.position.x - source.position.x);
    ...
    update(deltaTime) {
        // Recharge capacitor
        this.capacitorCharge = Math.min(this.capacitorSize, this.capacitorCharge + this.energyCost * deltaTime);
    }
    toJSON() {
        return {
            id: this.id,
