...
const unit_js_1 = require("./unit.js"); // Added import
const caption_js_1 = require("./entities/caption.js"); // Added import
...
class Building {
    ...
    setRallyPoint(buildingX, buildingY, simulation) {
        ...
        if (terrain) {
            ...
            for (const pos of rallyPositions) {
                const tileX = Math.floor(pos.x / gameConstants_js_1.TILE_SIZE);
                const tileY = Math.floor(pos.y / gameConstants_js_1.TILE_SIZE);
                if (tileX >= 0 && tileX < gameConstants_js_1.GRID_SIZE && tileY >= 0 && tileY < gameConstants_js_1.GRID_SIZE &&
                    terrain[tileX] && terrain[tileX][tileY] === gameConstants_js_1.TERRAIN_TYPES.LAND) {
                    this.rallyx = pos.x;
                    this.rallyy = pos.y;
                    return;
                }
    ...
    update(simulation, deltaTime) {
        ...
        if (this.type.produces && this.productionQueue.length === 0) {
            const unitTypesToBuild = this.type.produces.filter(unitType => {
                var _a, _b, _c;
                return resources[this.team].mass >= (((_a = unitType.cost) === null || _a === void 0 ? void 0 : _a.mass) || 0) &&
                    resources[this.team].energy >= (((_b = unitType.cost) === null || _b === void 0 ? void 0 : _b.energy) || 0) &&
                    resources[this.team].computronium >= (((_c = unitType.cost) === null || _c === void 0 ? void 0 : _c.computronium) || 0);
            });
        ...
        if (this.productionQueue.length > 0) {
            ...
            if (this.captionCooldown <= 0 && seedRandom && seedRandom.random() < 0.01) {
                ...
                entityManager.addCaption(new caption_js_1.Caption(this.x, this.y - this.type.size, `Building ${this.productionQueue[0].name} ${progressPercent}%`, '#ff0', 10));
            ...
            if (this.productionProgress >= this.productionQueue[0].buildTime) {
                ...
                const newUnit = new unit_js_1.Unit(this.rallyx, this.rallyy, this.team, unitType, simulation);
                entityManager.addUnit(newUnit);
                this.productionProgress = 0;
                entityManager.addCaption(new caption_js_1.Caption(this.x, this.y - this.type.size, `${unitType.name} ready!`, '#0f0', 12));
                if (unitType.name === 'Experimental' || unitType.tier === 3) {
                    const event = addEvent('build', `${this.team.toUpperCase()} completed ${unitType.name}!`, 3);
    ...
    draw(ctx, camera) {
        ...
        if (this.productionQueue.length > 0) {
            ctx.fillStyle = '#ff0';
            ctx.fillRect(screenX - size / 2, screenY + size / 2 + 5, size * (this.productionProgress / this.productionQueue[0].buildTime), 3);
        }
