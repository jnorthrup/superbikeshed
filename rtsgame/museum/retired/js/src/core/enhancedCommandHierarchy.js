...
exports.EnhancedCommandHierarchy = exports.CommandNode = void 0;
...
class CommandNode {
    ...
    calculateCommandLatency(computroniumManager) {
        let latency = gameConstants_js_1.COMPUTRONIUM_CONFIG.C2_LATENCY_BASE;
...
class EnhancedCommandHierarchy {
    ...
    autoAssignToHierarchy(node) {
        ...
        for (const [entityId, candidateNode] of this.commandNodes) {
            if (candidateNode === node || candidateNode.rank <= node.rank)
                continue;
    ...
    unregisterEntity(entity) {
        ...
        if (node.superior) {
            ...
            if (node.subordinates.length > 0) {
                const replacement = node.subordinates.reduce((best, current) => current.rank > best.rank ? current : best);
            ...
            for (let y = cellY - cellRange; y <= cellY + cellRange; y++) {
                ...
                if (cellUnits) {
                    for (const unitId of cellUnits) {
                        const otherUnit = (_a = this.commandNodes.get(unitId)) === null || _a === void 0 ? void 0 : _a.entity;
    ...
    checkPromotionOpportunities(node) {
        ...
        if (node.entity.veterancyLevel === 'HERO')
            return;
    ...
    getCommandRange(node) {
        let baseRange = gameConstants_js_1.COMPUTRONIUM_CONFIG.COMMAND_RANGES.STRATEGIC;
        ...
        // Veterancy bonus
        if (node.entity.veterancyLevel === 'VETERAN')
            baseRange *= 1.1;
        if (node.entity.veterancyLevel === 'ELITE')
            baseRange *= 1.2;
        if (node.entity.veterancyLevel === 'HERO')
            baseRange *= 1.3;
    ...
    calculateDistanceToObjective(node) {
        ...
        return Math.min(...objectives.map(obj => this.calculateDistance(node.entity, obj)));
    }
    /**
    ...
    findBestFieldCommander(sector) {
        ...
        const commanders = unitsInSector.filter(u => u.type.isCommandUnit &&
    ...
    findUnitsInSector(sector) {
        return Array.from(this.commandNodes.values())
            ...
            .filter(unit => unit.x >= sector.x1 && unit.x <= sector.x2 &&
...
exports.EnhancedCommandHierarchy = EnhancedCommandHierarchy;
