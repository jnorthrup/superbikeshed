...
export class FormationSystem {
    ...
    createFormation(leader, shape = 'LINE', options = {}) {
        ...
        const formation = {
            ...
            followers: new Set(),
            options: {
                spacing: options.spacing || COMMAND_CONFIG.COMMAND_RANGES.INDIVIDUAL,
    ...
    updateFormation(formation, gameContext) {
        if (!formation || !formation.leader) return;
        
        const now = Date.now();
        if (now - formation.lastUpdate < 16) return; // Cap at ~60fps
    ...
    calculateSteering(unit, target, gameContext) {
        ...
        steering.x += separation.x * COMMAND_CONFIG.STEERING_WEIGHTS.SEPARATION;
        steering.y += separation.y * COMMAND_CONFIG.STEERING_WEIGHTS.SEPARATION;
        
        // Terrain avoidance
        const avoidance = this.calculateTerrainAvoidance(unit, gameContext);
        steering.x += avoidance.x * COMMAND_CONFIG.STEERING_WEIGHTS.TERRAIN_AVOIDANCE;
        steering.y += avoidance.y * COMMAND_CONFIG.STEERING_WEIGHTS.TERRAIN_AVOIDANCE;
    ...
    calculateSeparation(unit, gameContext) {
        ...
        const neighborRadius = unit.type.size * COMMAND_CONFIG.NEIGHBOR_RADIUS_FACTOR;
    ...
    calculateTerrainAvoidance(unit, gameContext) {
        ...
        const feelerLength = unit.type.size * COMMAND_CONFIG.STEERING_FEELER_LENGTH_FACTOR;
    ...
    changeFormationShape(formation, newShape) {
        if (!formation || !this.formationShapes[newShape]) return false;
