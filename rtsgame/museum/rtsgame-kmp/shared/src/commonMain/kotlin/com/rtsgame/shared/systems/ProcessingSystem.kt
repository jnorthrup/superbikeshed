...
class ProcessingSystem(private val gameMap: GameMap) {
    private val recipes = mutableMapOf<String, ProcessingRecipe>()
    ...
    private fun processBuilding(building: Building): Building {
    ...
    private fun hasRequiredResources(building: Building, recipe: ProcessingRecipe): Boolean {
        return recipe.inputResources.all { (resource, amount) ->
            building.storedResources >= amount
        }
    }

    private fun consumeResources(building: Building, recipe: ProcessingRecipe): Float {
