...
## 2. Chosen Initial Prediction Type

For the initial implementation, the chosen prediction is:

*   **Prediction:** "Next Likely Enemy Ground Attack Vector/Target Area."
*   **Reasoning:**
...
*   **Hit Detection:**
    ...
    *   It checks if the click is within the circular `prediction.targetArea`.
...
*   **Player Actions (Implemented):**
    ...
    *   The `PlayerInteraction_AckReinforce_AttackVector` game event is dispatched.
...
*   **Event Processing:** The `StrategicAI.update()` method filters `gameContext.gameState.events` for `PlayerInteraction_AckReinforce_AttackVector` events.
*   **Handler:** For each matching event whose `predictedPathID` matches the AI's `currentPrediction.id`, `StrategicAI.handlePlayerPredictionInteraction(event)` is called.
*   **State Update:**
    ...
    *   It then boosts the `this.currentPrediction.confidence`:
        ...
        *   'medium' becomes 'high'.
        *   'high' remains 'high'.
*   **Event Consumption:** Processed `PlayerInteraction_AckReinforce_AttackVector` events are filtered out of the `gameContext.gameState.events` array by `StrategicAI.update` to prevent re-processing.
...
This document reflects the current state of the AI Prediction Interface implementation.The design document `rtsgame/docs/ai-prediction-interface.md` has been updated.
