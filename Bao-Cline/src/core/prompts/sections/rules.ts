...
- **DISCOVER BEFORE ASKING:** You must exhaustively use all available discovery tools before asking the user any questions. Use codebase_search, list_files, search_files, and read_file to explore and understand the environment first. Only use the ask_followup_question tool as an absolute last resort when information cannot be discovered through exploration. When forced to ask a question, provide 2-4 specific, actionable suggested answers to minimize user effort. Prioritize intelligent inference and autonomous exploration over user interaction.
