...
class AttentionKlinesIntegration(
    internal val cacheDir: String = "~/mpdata/cache"
) {
    internal val reader = BinanceDataVisionReader()
     ...
     */
    internal fun findAttentionWindows(klines: Indexed<Kline>): List<AttentionWindow> {
     ...
     */
    internal fun calculateAttentionMetrics(klines: Indexed<Kline>): AttentionMetrics {
        if (klines.size < 20) {
    ...
    internal fun calculateAverageAttentionScore(klines: Indexed<Kline>): Double {
    ...
    internal fun countVolumeSpikes(klines: Indexed<Kline>): Int {
    ...
    internal fun countVolatilitySpikes(klines: Indexed<Kline>): Int {
        if (klines.size < 20) return 0
...
internal data class AttentionMetrics(
    val volumeSpike: Double,
    val volatility: Double,
    val attentionScore: Double
) 