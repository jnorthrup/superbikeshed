...
class ATRIndicator(
    ...
    // Internal caches
    internal val trueRangeResults = mutableListOf<BigDecimal?>() // Stores raw TR values
    internal val atrResults = mutableListOf<BigDecimal?>()       // Stores smoothed ATR values
    internal var calculatedUpToIndex = -1

    internal val calculationScale = 8 // Internal calculation precision
    // Result scale for ATR usually matches price scale or a bit more
    internal val resultScale = klineSeries.firstOrNull()?.closePrice?.scale()?.let { it + 2 } ?: 4
