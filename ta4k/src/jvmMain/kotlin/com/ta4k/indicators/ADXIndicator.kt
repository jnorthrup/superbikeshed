...
class ADXIndicator(
    ...
    internal val plusDMResults = mutableListOf<BigDecimal?>()    // Raw +DM
    internal val minusDMResults = mutableListOf<BigDecimal?>()   // Raw -DM
    internal val trueRangeResults = mutableListOf<BigDecimal?>() // Raw TR (used for ADX's internal TR smoothing)

    internal val smoothedPlusDM = mutableListOf<BigDecimal?>()
    internal val smoothedMinusDM = mutableListOf<BigDecimal?>()
    internal val smoothedTR = mutableListOf<BigDecimal?>()

    internal val plusDIResults = mutableListOf<BigDecimal?>()
    internal val minusDIResults = mutableListOf<BigDecimal?>()
    internal val dxResults = mutableListOf<BigDecimal?>()
    val adxResults = mutableListOf<BigDecimal?>() // Smoothed ADX values

    internal val calculationScale = 8
    internal val resultScale = 2 // Standard scale for ADX, DI values

    internal var calculatedUpToIndex = -1

    internal fun ensureListSize(list: MutableList<BigDecimal?>, size: Int) {
        while (list.size < size) { // Use < instead of <= to make it size of klineSeries
            list.add(null)
        }
    }

    internal fun preAllocateListsIfNeeded() {
