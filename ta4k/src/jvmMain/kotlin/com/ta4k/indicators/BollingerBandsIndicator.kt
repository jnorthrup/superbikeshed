...
import borg.trikeshed.lib.j      // Import infix j for creating Indexed
...
class BollingerBandsIndicator(
    internal val klineSeries: Indexed<Kline>, // Changed
    internal val period: Int,
    internal val standardDeviationMultiplier: BigDecimal = BigDecimal("2.0"),
    internal val klinePropertySelector: (Kline) -> BigDecimal = { it.closePrice }
) {
    init {
    ...
    // Internal caches
    internal val upperBandResults = mutableListOf<BigDecimal?>()
    internal val lowerBandResults = mutableListOf<BigDecimal?>()
    internal val stdDevResults = mutableListOf<BigDecimal?>()
    internal var calculatedUpToIndex = -1

    internal val calculationScale = 8
    // Result scale for bands usually matches price scale. Default to 2 if series is empty or first kline has no scale.
    internal val resultScale = klineSeries.firstOrNull()?.let { kline ->
    ...
    internal fun Indexed<Kline>.firstOrNull(): Kline? = if (this.size > 0) this[0] else null

    internal fun ensureCalculatedUpTo(targetIndex: Int) {
        ...
        if (calculatedUpToIndex == -1) {
            ...
            while (stdDevResults.size < klineSeries.size) stdDevResults.add(null)
        }

        // Ensure middle band (SMA) is calculated up to the target index.
        // SMAIndicator's getValue will handle its own Indexed interaction and caching.
    ...
    val middleBandValues: Indexed<BigDecimal?> // Renamed for consistency
        get() {
            ...
            // Access the SMA's values Indexed, then re-scale if necessary for BBands context.
