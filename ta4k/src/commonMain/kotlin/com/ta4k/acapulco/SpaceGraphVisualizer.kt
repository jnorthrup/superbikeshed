...
class SpaceGraphVisualizer {
    companion object {
        internal val DATE_FORMATTER = DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss")
        ...
        internal fun formatKlineLabel(kline: Kline): String {
            val time = DATE_FORMATTER.format(Instant.ofEpochMilli(kline.openTimeMillis))
            return "$time\nO: ${kline.openPrice}\nH: ${kline.highPrice}\nL: ${kline.lowPrice}\nC: ${kline.closePrice}"
        }
        
        internal fun getKlineColor(kline: Kline): String {
        ...
        internal fun calculateEdgeWeight(prev: Kline, curr: Kline): Double {
