...
class BinanceDataDownloader(
    internal val cacheDir: String = System.getProperty("user.home") + "/mpdata/cache",
    internal val importDir: String = System.getProperty("user.home") + "/mpdata/import"
) {
    
    companion object {
        internal const val BINANCE_BASE_URL = "https://data.binance.vision/data/spot"
        internal const val MAX_CONCURRENT_DOWNLOADS = 5
    }
    ...
    internal fun generateDownloadUrls(symbol: String, timeUnit: String): List<String> {
    ...
    internal suspend fun downloadFiles(urls: List<String>, cachePath: String) {
    ...
    internal suspend fun downloadFile(url: String, cachePath: String) {
        try {
    ...
    internal suspend fun processAndCombineFiles(
        cachePath: String, 
        targetPath: String, 
        symbol: String, 
        timeUnit: String
    ...
    internal fun extractZipFiles(cachePath: String, tempDir: File) {
    ...
    internal fun combineCsvFiles(tempDir: File, finalFile: String, symbol: String, timeUnit: String) {
    ...
    internal fun cleanupCache(cachePath: String) {
 ...
 */
internal suspend fun <T> kotlinx.coroutines.sync.Semaphore.withPermit(block: suspend () -> T): T {
    acquire()
    try {
        return block()
    } finally {
