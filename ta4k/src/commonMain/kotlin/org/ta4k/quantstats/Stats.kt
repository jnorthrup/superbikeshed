...
// Placeholder for data series type (e.g., ta4k's Series<T> or a simple List<Double>)
 ...
 * It will be adapted to use ta4k's Series<T> and its 'α' operator once available.
 *
 * @param returns A collection (e.g., List<Double> or eventually ta4k's Series<Double>) of returns.
...
fun comp(returns: Any): Double {
    ...
    // In a typed environment with ta4k's Series<T>, this check would be different or handled by the type system.
    if (returns !is List<*>) {
    ...
    // CLAUDE.md mandates using 'α' for transformations on Series<T>.
    // If 'numericReturns' were a ta4k.Series<Double>, the logic would be:
    // val plusOneSeries = numericReturns.α { it + 1.0 }
    // val product = plusOneSeries.fold(1.0) { acc, value -> acc * value } // Or a specific 'prod' operation on Series.
 ...
 *         Eventually, this should return a ta4k's Series<Double>.
 */
fun compsum(returns: Any): List<Double> { // Return type changed to List<Double> for clarity.
                                         // Eventually ta4k's Series<Double>.
    if (returns !is List<*>) {
    ...
    // CLAUDE.md mandates using 'α' for transformations on Series<T>.
    // If 'numericReturns' were a ta4k.Series<Double>, the logic might involve:
    // val plusOneSeries = numericReturns.α { it + 1.0 }
...
fun maxDrawdown(prices: Any): Double {
    println("WARN: maxDrawdown(prices) is a stub. Requires implementation with ta4k's Series type and _prepare_prices from Utils.")
...
fun skew(returns: Any, prepareReturns: Boolean = true): Double {
    println("WARN: skew(returns) is a stub. Pandas .skew() equivalent needed for ta4k Series type.")
    return 0.0
}
...
fun kurtosis(returns: Any, prepareReturns: Boolean = true): Double {
    println("WARN: kurtosis(returns) is a stub. Pandas .kurtosis() equivalent needed for ta4k Series type.")
    return 0.0
}
...
fun volatility(returns: Any, periods: Int = 252, annualize: Boolean = true, prepareReturns: Boolean = true): Double {
    ...
    // CLAUDE.md: If 'numericReturns' were ta4k.Series<Double>, it should have its own .std() or equivalent.
