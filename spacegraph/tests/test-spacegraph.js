...
describe('Fractal Blackboard Layout', () => {
    let space;
    const beforeEachTest = () => {
        setupMockDOM();
        space = new S.SpaceGraph(mockContainer, {
    ...
    // Helper: recursively add fractal nodes
    function addFractalNodes(parentId, depth, breadth) {
        if (depth === 0) return;
        for (let i = 0; i < breadth; i++) {
            const nodeId = `${parentId}_child${i}`;
            const node = new S.ShapeNode(nodeId, {x: Math.random()*100, y: Math.random()*100, z: 0}, {label: `Node ${nodeId}`, shape: 'sphere', size: 30});
            space.addNode(node);
            space.addEdge(space.getNodeById(parentId), node);
            addFractalNodes(nodeId, depth-1, breadth);
        }
    }

    it('should create a fractal blackboard layout with correct structure', () => {
        beforeEachTest();
        // Root node
        const root = new S.ShapeNode('root', {x:0, y:0, z:0}, {label: 'Blackboard Root', shape: 'sphere', size: 50});
        space.addNode(root);
        // Fractal: depth 3, breadth 2 (root → 2 children → each 2 children)
        addFractalNodes('root', 3, 2);
        // Check node and edge counts: 1 (root) + 2 + 4 = 7 nodes, 2 + 4 = 6 edges
        assertEqual(space.nodes.size, 7, 'Should have 7 nodes in fractal layout');
        assertEqual(space.edges.size, 6, 'Should have 6 edges in fractal layout');
        // Check parent-child relationships
        const child1 = space.getNodeById('root_child0');
        const child2 = space.getNodeById('root_child1');
        assertNotNull(child1, 'First child should exist');
        assertNotNull(child2, 'Second child should exist');
        // Each child should have 2 children
        assertNotNull(space.getNodeById('root_child0_child0'), 'Grandchild should exist');
        assertNotNull(space.getNodeById('root_child1_child1'), 'Grandchild should exist');
        afterEachTest();
    });
});
