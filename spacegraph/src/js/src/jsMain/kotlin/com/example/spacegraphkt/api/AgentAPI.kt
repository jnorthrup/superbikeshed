...
@JsExport
class AgentAPI(internal val spaceGraph: SpaceGraph) {

    internal data class JsEventCallbackInfo(
        val eventType: String,
        val callback: (dynamic) -> Unit,
        val subscriptionId: String
    )

    internal val eventSubscriptions: MutableList<JsEventCallbackInfo> = arrayListOf()
    internal var nextSubscriptionId = 0
    ...
    internal fun parseConstraintParams(type: String?, params: dynamic): EdgeConstraintParams {
        return when (type) {
            "rigid" -> RigidParams(
    ...
    internal fun convertNodeToJs(node: BaseNode): JsAny? {
       try {
            return jsObject {
                this.id = node.id
    ...
    internal fun convertEdgeToJs(edge: Edge): JsAny? {
        try {
            return jsObject {
                this.id = edge.id
    ...
    internal fun convertConstraintParamsToJs(params: EdgeConstraintParams): JsAny {
        return when (params) {
