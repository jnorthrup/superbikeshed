...
object JsonBBCursive {
    ...
    private fun whitespace(): JsonParser = JsonParser { buffer, pos ->
        var p = pos
        while (p < buffer.a) {
            val c = buffer.b(p)
    ...
    // String parser
    private fun string(): JsonParser = JsonParser { buffer, pos ->
    ...
    // Number parser
    private fun number(): JsonParser = JsonParser { buffer, pos ->
    ...
    // Literal parsers
    private fun literal(text: String, element: JsonElement): JsonParser = JsonParser { buffer, pos ->
    ...
    private fun null_(): JsonParser = literal("null", JsonElement.Null)
    private fun true_(): JsonParser = literal("true", JsonElement.Bool(true))
    private fun false_(): JsonParser = literal("false", JsonElement.Bool(false))
    
    // Forward declarations for recursive parsers
    private lateinit var valueParser: JsonParser
    
    // Array parser
    private fun array(): JsonParser = JsonParser { buffer, pos ->
        var p = pos
    ...
    // Object parser
    private fun object_(): JsonParser = JsonParser { buffer, pos ->
        var p = pos
    ...
    private fun value(): JsonParser = JsonParser { buffer, pos ->
