...
class KzranGzipReader(
    internal val httpClient: HttpRangeClient,
    internal val indexCache: KzranIndexCache = InMemoryKzranCache()
) {
    companion object {
     ...
     */
    internal suspend fun decompressFromSyncPoint(
     ...
     */
    internal fun estimateCompressedSize(uncompressedSize: Long): Long {
     ...
     */
    internal fun extractJsonObjects(
    ...
    internal fun getStructuralBits(bits: UByteArray, position: Int): Int {
 ...
 */
class SlidingWindow(internal val size: Int) {
    internal val buffer = ByteArray(size)
    ...
    fun append(data: ByteArray, offset: Int, length: Int) {
        ...
        if (copyLength < size - position) {
            ...
            data.copyInto(buffer, 0, offset + startOffset + firstPart, offset + startOffset + copyLength)
    ...
    fun getWindow(): ByteArray {
        ...
        if (position == 0) {
            ...
            buffer.copyInto(result, 0, position, size)
            buffer.copyInto(result, size - position, 0, position)
        }
        return result
    }
}
...
class InMemoryKzranCache : KzranIndexCache {
    internal val cache = mutableMapOf<String, KzranIndex>()
 ...
 */
class ZlibInflater {
    internal var input: ByteArray = ByteArray(0)
    internal var finished = false
    internal var bytesProcessed = 0
