 ...
 */
class RemoteFileAttention(internal val httpClient: HttpRangeClient) {
    ...
    internal fun parseLocalFileHeader(headerData: Indexed<Byte>): LocalFileHeader {
    ...
    internal fun findEocdRecord(buffer: Indexed<Byte>): Long {
    ...
    internal fun parseEocd(eocdData: Indexed<Byte>): EocdRecord {
        return EocdRecord(
    ...
    internal fun parseCentralDirectory(directoryData: Indexed<Byte>): Indexed<ZipEntry> {
        val entries = mutableListOf<ZipEntry>()
        var offset = 0
    ...
    internal fun parseCentralDirectoryEntry(entryData: Indexed<Byte>): ZipEntry {
    ...
    internal data class EocdRecord(
    ...
    companion object {
        internal const val EOCD_RECORD_SIZE = 22
        internal const val LOCAL_FILE_HEADER_SIZE = 30
    }

    // Helper functions to read little-endian values from a Indexed<Byte>
    internal fun Indexed<Byte>.getShort(offset: Int): Short {
        return ((this[offset + 1].toInt() and 0xFF) shl 8 or (this[offset].toInt() and 0xFF)).toShort()
    }

    internal fun Indexed<Byte>.getInt(offset: Int): Int {
