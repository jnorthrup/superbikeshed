...
class BitmapStreamingProcessor(
    ...
    private fun streamSmallArrayBitmap(json: String): Flow<JsonElement> = flow {
        val provider = BitmapJsonProvider(enableParallel = false)
        val result = provider.parse(json)
    ...
    private fun streamLargeArrayBitmap(json: String): Flow<JsonElement> = channelFlow {
...
class BitmapStreamingCursor(
    private val processor: BitmapStreamingProcessor = BitmapStreamingProcessor()
) {
    ...
    private fun elementToRow(element: JsonElement): List<Any?> = when (element) {
        is JsonElement.Obj -> {
...
class BitmapISAMStreaming(
    ...
    private suspend fun writeBatchToISAM(
        rows: List<List<Any?>>,
        maxColumns: Int,
        outputPath: String,
    ...
    private fun inferTypeFromValues(rows: List<List<Any?>>, colIndex: Int): IOMemento {
...
class ReactiveBitmapStreaming(
    ...
    private val _events = MutableSharedFlow<BitmapStreamEvent>()
