...
object VectorAPIBitmapOps {
    ...
    internal val vectorApiAvailable by lazy {
        try {
    ...
    internal fun scanWithVectorAPI(input: ByteArray): BitmapSeries {
        try {
    ...
    internal fun scanChunkVectorized(input: ByteArray, offset: Int): Int {
...
class ForkJoinBitmapProcessor(
    internal val threshold: Int = 1024,
    internal val parallelism: Int = Runtime.getRuntime().availableProcessors()
) {
    
    internal val forkJoinPool = ForkJoinPool(parallelism)
...
internal class BitmapScanTask(
    ...
    internal fun mergeStructuralSeries(
 ...
 */
internal class ArrayProcessTask(
    internal val array: JsonElement.Arr,
    internal val start: Int,
    internal val end: Int,
    internal val threshold: Int,
    internal val processor: (JsonElement) -> Unit
) : RecursiveAction() {
...
class MemoryMappedBitmapProcessor {
    ...
    internal fun processJsonChunk(chunk: String, processor: (JsonElement) -> Unit) {
...
class ConcurrentBitmapCounter {
    internal val structuralCount = AtomicLong(0)
    internal val quoteCount = AtomicLong(0)
    internal val objectCount = AtomicLong(0)
    internal val arrayCount = AtomicLong(0)
    ...
    internal fun analyzeChunk(chunk: String) {
...
class ParallelBitmapToCursor(
    internal val workerThreads: Int = Runtime.getRuntime().availableProcessors()
) {
    
    internal val executor = Executors.newFixedThreadPool(workerThreads)
    ...
    internal suspend fun convertArrayToCursorParallel(
    ...
    internal fun processArrayChunk(
        array: JsonElement.Arr,
        start: Int,
        end: Int
    ): List<List<Any?>> {
