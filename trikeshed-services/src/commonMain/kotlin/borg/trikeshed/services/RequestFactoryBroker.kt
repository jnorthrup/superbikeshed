...
object RequestFactoryBroker {
    ...
    class Client(internal val transport: Transport) {
        internal val entityVersions = mutableMapOf<EntityProxyId, Long>()
        internal val pendingRequests = mutableMapOf<MethodToken, (Response) -> Unit>()
        ...
        internal suspend fun sendRequest(request: Request, callback: (Response) -> Unit) {
            pendingRequests[request.methodToken] = callback
            val json = serializeRequest(request)
            transport.send(json.play.toByteArray().toIndexed())
        }
    ...
    class Server(internal val serviceInvoker: PlatformServiceInvoker) {
        internal val services = mutableMapOf<ServiceToken, Any>()
        
        internal val entityVersions = mutableMapOf<EntityProxyId, Long>()
        internal val validators = mutableMapOf<MethodToken, (Indexed<Any?>) -> Boolean>()
        ...
        suspend fun handleRequest(requestBytes: Indexed<Byte>): Indexed<Byte> {
            ...
            return JsonImpl.stringify(json).encodeToByteArray().toIndexed()
        }

        internal fun handleInvoke(request: Request.Invoke): Response {
        ...
        internal fun handleCreate(request: Request.Create): Response {
            val service = services[request.serviceToken] ?: return Response.Failure("Service not found")
        ...
        internal fun handleUpdate(request: Request.Update): Response {
        ...
        internal fun handleDelete(request: Request.Delete): Response {
            val currentVersion = entityVersions[request.version.id]
                ?: return Response.Failure("Entity not found")
    ...
    internal fun parseRequest(json: Any?): Request {
    ...
    internal fun parseResponse(json: Any?): Response {
        val obj = json as? Map<*, *> ?: throw IllegalArgumentException("Expected JSON object")
    ...
    internal fun serializeRequest(request: Request): Map<String, Any?> {
        val base = mapOf(
    ...
    internal fun serializeResponse(response: Response): Map<String, Any?> {
    ...
    internal fun getStringField(obj: Any?, field: String): String {
