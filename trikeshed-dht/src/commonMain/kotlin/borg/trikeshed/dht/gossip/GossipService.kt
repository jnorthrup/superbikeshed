...
class GossipService(
    ...
    internal val subnetManager: SubnetManager,
    internal val sendToNode: suspend (nodeId: NUID, message: GossipMessage) -> Boolean
) {
    // Message cache for duplicate detection
    internal val messageCache = MessageCache(maxSize = 10000, ttlMillis = 300000) // 5 minutes
    
    // Active subscriptions
    internal val subscriptions = mutableListOf<GossipSubscription>()
    
    // Gossip flow for reactive processing
    internal val gossipFlow = MutableSharedFlow<GossipMessage>(
        replay = 0,
        extraBufferCapacity = 1000
    )
    
    // Configuration
    internal val defaultFanout = 8
    internal val maxHops = 6
     ...
     */
    internal suspend fun propagate(message: GossipMessage, ttl: Int): Boolean {
     ...
     */
    internal fun selectPeersForGossip(message: GossipMessage): List<NUID> {
     ...
     */
    internal fun deliverToSubscribers(message: GossipMessage) {
 ...
 */
internal data class GossipSubscription(
...
internal class MessageCache(
    ...
    internal val ttlMillis: Long
) {
    internal val cache = mutableMapOf<NUID, Long>()
    ...
    fun markAsSeen(messageId: NUID) {
        cleanExpired()
        cache[messageId] = kotlinx.datetime.Clock.System.now().toEpochMilliseconds()
