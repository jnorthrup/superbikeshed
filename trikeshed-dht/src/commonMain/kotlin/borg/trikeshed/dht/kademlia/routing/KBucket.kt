...
class KBucket(
    ...
    internal val nodes = mutableListOf<NodeInfo>()
    internal val replacementCache = mutableListOf<NodeInfo>()
    
    // Destruction notification callbacks
    internal val destructionListeners = mutableListOf<(NodeInfo, String) -> Unit>()
    
    fun addDestructionListener(listener: (NodeInfo, String) -> Unit) {
        destructionListeners.add(listener)
    }
    
    internal fun notifyDestruction(node: NodeInfo, reason: String) {
    ...
    fun addNode(node: NodeInfo): Boolean {
        ...
        if (existingIndex >= 0) {
            ...
            nodes.add(node.copy(lastSeen = kotlinx.datetime.Clock.System.now().toEpochMilliseconds()))
