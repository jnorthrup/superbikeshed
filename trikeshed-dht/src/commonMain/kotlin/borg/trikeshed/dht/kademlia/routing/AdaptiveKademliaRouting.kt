...
class AdaptiveKademliaRouting(
    ...
    internal val k: Int = 20 // k-bucket size
) {
    // Multi-dimensional routing metrics
    internal val routingTable = RoutingTable(nodeId, k)
    internal val nodeMetrics = mutableMapOf<NodeID, NodeMetrics>()
    
    // Learned weight vectors for different optimization goals
    internal val weightVectors = WeightVectors()
    ...
    class WeightVectors {
        ...
        internal val learningRate = 0.01
     ...
     */
    internal fun updateAvailabilityPattern(nodeId: NodeID, success: Boolean) {
    ...
    internal fun normalizeDistance(distance: NodeID): Double {
    ...
    internal fun normalizeLatency(latencyMs: Double): Double {
        // Normalize latency to 0-1 (lower is better)
        return 1.0 - (1.0 / (1.0 + latencyMs / 100))
    }
    
    internal fun normalizeBandwidth(bps: Long): Double {
...
internal fun NodeID.toBitString(): String {
