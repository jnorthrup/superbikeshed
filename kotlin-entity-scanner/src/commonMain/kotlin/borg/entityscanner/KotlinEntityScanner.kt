...
object KotlinEntityScanner {
    ...
    internal fun isClassNode(depType: DependencyToken): Boolean {
        return depType.depType == DependencyToken.INTERNAL_REFERENCE ||
               depType.depType == DependencyToken.INHERITANCE
    }
    
    internal fun extractDependencyCoordinate(line: String): Join<String, String>? {
    ...
    internal fun generateEntityName(entityToken: EntityToken, roleToken: RoleToken): String {
    ...
    internal fun calculatePosition(source: String, entityName: String): Int {
        // Simple position calculation - would be enhanced with actual parsing
        return entityName.hashCode() % source.length
    }
    
    internal fun calculateLineNumber(lines: List<String>, position: Int): Int {
...
object K2ScriptIntegration {
    ...
    internal fun extractDependencyFromLine(line: String): Join<String, String>? {
        val regex = """@file:DependsOn\("([^:]+):([^:]+)(?::([^"]+))?"\)""".toRegex()
        val match = regex.find(line)
