...
object KotlinScriptLexer {
    ...
    fun tokenize(source: String): TokenSeries {
        ...
        // For example: fun <T> List<T>.toSeries(): Indexed<T> = \1 j { \2: Int -> this[index] }
    ...
    internal fun createBoundedToken(value: String, typeInstance: TokenType, startIndex: Int): BoundedToken {
    ...
    internal fun readWhile(source: String, startIndex: Int, predicate: (Char) -> Boolean): Pair<String, Int> {
    ...
    internal fun readUntil(source: String, startIndex: Int, predicate: (Char) -> Boolean): Pair<String, Int> {
        var currentIndex = startIndex
        while (currentIndex < source.length && !predicate(source[currentIndex])) {
            currentIndex++
        }
    ...
    internal fun keywordToTokenType(value: String): TokenType? {
        return when (value) {
    ...
    internal fun symbolToTokenType(char: Char): TokenType {
        return when (char) {
...
fun <T> List<T>.toIndexed(): Indexed<T> = \1 j { \2: Int -> this[i] }
