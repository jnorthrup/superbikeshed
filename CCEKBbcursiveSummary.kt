...
object ProtocolCapabilities {
    ...
    private fun loadProtocolRules(type: String): Indexed<TransformationRule> = EmptyIndexed
    private fun loadProtocolConstraints(type: String): Indexed<Constraint> = EmptyIndexed
    private fun getProtocolValidator(type: String): (Any) -> Boolean = { true }
    private fun createCcekContext(phase: ExecutionPhase): CcekContext = CcekContext(
        Control("demo", phase),
        Context("session"),
        Environment("DEMO", Unit),
        Knowledge(EmptyIndexed, EmptyIndexed) { true }
    )
    private suspend fun bbcursiveParse(data: ByteArray): Any = data
    private suspend fun validateParsedData(data: Any): Any = data
    private suspend fun transformData(data: Any): Any = data
    private suspend fun serializeResult(data: Any): ByteArray = ByteArray(0)
    private suspend fun parseHttp(data: ByteArray): Any = "HTTP parsed"
    private suspend fun parseBinary(data: ByteArray): Any = "Binary parsed"
    private suspend fun parseFrame(data: ByteArray): Any = "Frame parsed"
}
