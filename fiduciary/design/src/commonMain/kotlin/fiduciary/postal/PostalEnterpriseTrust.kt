...
class PostalEnterpriseTrust {
     ...
     */
    class TechTree {
        private val nodes = mutableMapOf<FundingPhase, TechNode>()
    ...
    class PostalServiceImpl : PostalServiceAPI {
        ...
        private fun generateTrackingNumber(serviceType: ServiceType): String =
            when (serviceType) {
    ...
    // Extension functions
    private fun SpecialEndorsement.toMarking(): String =
        when (type) {
    ...
    // Utility functions
    private fun generatePurchaseId(): String = "PUR-${System.currentTimeMillis()}"
    private fun generateLabelId(): String = "LBL-${System.currentTimeMillis()}"
    private fun generateUPUId(): String = "UPU-${System.currentTimeMillis()}"
    private fun generateDepositId(): String = "DEP-${System.currentTimeMillis()}"
    
    private fun getStampDutyRate(type: TransactionType, jurisdiction: String): Double =
        when (type) {
    ...
    private fun determineStampDutyType(transaction: PostalTransaction): StampDutyType =
        when (transaction.type) {
            TransactionType.TRUST_FUNDING → StampDutyType.TRUST_CREATION
            else → StampDutyType.POSTAL_SERVICE
        }
    
    private fun createTerminalDuesAccount(): TerminalDuesAccount =
        TerminalDuesAccount(
    ...
    private fun standardQualityObligations(): Indexed<QualityObligation> = b0.a
    
    private fun calculateSpecialDepositRate(terms: DepositTerms): Double =
    ...
    private fun calculateMaturityDate(terms: DepositTerms): LocalDate =
        Clock.System.now().toLocalDateTime(TimeZone.UTC).date + terms.termLength
    
    private fun generateBarcodes(
        shipment: Shipment, 
        endorsements: Indexed<SpecialEndorsement>
