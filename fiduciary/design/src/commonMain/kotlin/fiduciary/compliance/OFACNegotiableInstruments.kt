...
class OFACNegotiableInstruments {
    ...
    class ADHDComplianceWorkflow {
        ...
        private fun createImmediateTask(
    ...
    class OFACScreener {
        ...
        private suspend fun checkExemptions(
            instrument: NegotiableInstrument
        ): ExemptionStatus? {
        ...
        private fun determineReporting(
            instrument: NegotiableInstrument,
    ...
    // Utility functions
    private fun isPersonalRemittance(instrument: NegotiableInstrument): Boolean {
        // Check memo line or purpose
        return true // Simplified
    }
    
    private fun hasLegalServicesAuthorization(instrument: NegotiableInstrument): Boolean {
        // Check for legal services authorization
        return instrument.payee.partyType == PartyType.LAW_FIRM
    }
    
    private suspend fun searchSDNList(party: Party): List<SDNEntry> {
        // Would connect to OFAC SDN API or database
        return emptyList()
    }
    
    private fun calculateMatchConfidence(matches: List<SDNEntry>): Double {
        return matches.maxOfOrNull { it.score } ?: 0.0
    }
    
    private fun identifyFalsePositives(
    ...
    private fun mapExemptionReason(type: ExemptionType): ExemptionReason =
        when (type) {
    ...
    private fun <T> idx(vararg items: T): Indexed<T> = b0.a
}