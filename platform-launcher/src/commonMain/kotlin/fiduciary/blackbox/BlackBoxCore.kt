...
typealias BlackBoxOp<T> = Indexed<SealedOperation<T>>

// Encrypted results indexed by operation ID  
typealias EncryptedResults = Indexed<EncryptedResult<Any>>

// Compliance proofs indexed by proof ID
typealias ProofChain = Indexed<CryptographicProof>
...
object BlackBoxFactory {
     ...
     */
    fun createProofChain(capacity: Int = 1000): ProofChain {
...
abstract class BaseBlackBoxOperation<T> : SealedOperation<T>() {
    ...
    internal fun generateOperationId(): OpId {
        return "op_${timestamp.toEpochMilliseconds()}_${hashCode()}"
    }
    
    internal fun generateEncryptedId(): EncryptedId {
        return "enc_${timestamp.toEpochMilliseconds()}_${hashCode()}"
    }
    
    internal fun generateProofId(): ProofId {
...
class BlackBoxContext(
    ...
    internal val signingKey: ByteArray,
    internal val accessControl: AccessControlList
) {
    /**
