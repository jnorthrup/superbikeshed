...
inline infix fun <X, C, V : Indexed<X>> V.alpha(crossinline xform: (X) -> C): Indexed<C> = 
    this α xform

// Conversion alias (more explicit name)
inline infix fun <X, C, V : Indexed<X>> V.conversion(crossinline xform: (X) -> C): Indexed<C> = 
    this α xform
 ...
 */
inline infix fun <T, R> Indexed<T>.map(transform: (T) -> R): Indexed<R> = 
    this α transform
