...
// Core type aliases built on Join.
...
// Alpha (α) transformation operator for Indexed.
inline infix fun <X, C, V : Indexed<X>> V.α(crossinline xform: (X) -> C): Indexed<C> =
    this.a j { index: Int -> xform(this.b(index)) }

// Accessor properties for Indexed.
inline val <T> Indexed<T>.size: Int get() = a
inline operator fun <T> Indexed<T>.get(index: Int): T = b(index)

// 'play' operator to expose Indexed as a standard Kotlin Iterable.
@JvmInline
...
val <T> Indexed<T>.play: Iterable<T> get() = IterableIndexed(this)

// Empty Indexed singleton.
object EmptyIndexed : Indexed<Nothing> by (0 j { i -> throw IndexOutOfBoundsException("Access on EmptyIndexed at index $i") })
