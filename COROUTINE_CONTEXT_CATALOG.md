...
val quicContextLoadout = coroutineContext {
    ...
    + QuicCipherSuiteConfigContext { ringLevel ->
        when (ringLevel) {
            0 -> CipherSuite.TLS_AES_256_GCM_SHA384        // Core: strongest
            1 -> CipherSuite.TLS_AES_256_GCM_SHA384        // Processing: strong
            2 -> CipherSuite.TLS_AES_128_GCM_SHA256        // External: standard
            3 -> CipherSuite.TLS_CHACHA20_POLY1305_SHA256  // Untrusted: fast
            else -> throw IllegalArgumentException("Unknown ring level: $ringLevel")
        }
    }
}
...
data class QuicCipherSuiteConfigContext(
    val keyConfigForRing: (ringLevel: Int) -> CipherSuite
) : CoroutineContext.Element {
    companion object Key : CoroutineContext.Key<QuicCipherSuiteConfigContext>
